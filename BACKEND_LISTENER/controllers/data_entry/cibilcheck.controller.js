...
const cibilcheck = require('../../models/data_entry/cibilcheck.model')
...
if (!req.body.status) {
    ...
    const obj = new cibilcheck({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "6722092ddec2aeecb2d5c135" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
cibilscore: req.body.cibilscore,
            cibilscoreRhs: null,
cibilstatus: req.body.cibilstatus,
            cibilstatusRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await cibilcheck.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "cibilcheck" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving cibilcheck'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    cibilcheck
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving cibilcheck for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    cibilcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    cibilcheck.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    cibilcheck
        ...
        cibilcheck.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await cibilcheck.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await cibilcheck.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return cibilcheck.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
// cibilscoreRhs: req.body.cibilscoreRhs,
// cibilstatusRhs: req.body.cibilstatusRhs,
// verifiedbyRhs: req.body.verifiedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    cibilcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,numberRhs: req.body.numberRhs,dobRhs: req.body.dobRhs,cibilscoreRhs: req.body.cibilscoreRhs,cibilstatusRhs: req.body.cibilstatusRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    cibilcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,numberRhs: req.body.numberRhs,dobRhs: req.body.dobRhs,cibilscoreRhs: req.body.cibilscoreRhs,cibilstatusRhs: req.body.cibilstatusRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'cibilcheck Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'cibilcheck Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    cibilcheck.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await cibilcheck.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await cibilcheck.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await cibilcheck.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In cibilcheck Create for CDE")
...
if (!req.body.cibilscore) {
            res.status(400).json({ message: "cibilscore is Mandatory" })
        }
if (!req.body.cibilstatus) {
            res.status(400).json({ message: "cibilstatus is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In cibilcheck for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        cibilcheck.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
