const directorproprietoridentitycheck = require('../../models/data_entry/directorproprietoridentitycheck.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   if(!req.body.idtype){
       res.status(400).json({message:'ID Type required'})
   }
   if(!req.body.verifiedon){
       res.status(400).json({message:'Verified on required'})
   }
   if(!req.body.datasource){
       res.status(400).json({message:'Data Source required'})
   }
   ...
   const obj = new directorproprietoridentitycheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "645df385da4885b59f88fc47":req.body.component,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"directorproprietoridentitycheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving directorproprietoridentitycheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 directorproprietoridentitycheck
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving directorproprietoridentitycheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   directorproprietoridentitycheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       candidatename:req.body.candidatename,
       fathersname:req.body.fathersname,
       idtype:req.body.idtype,
       idnumber:req.body.idnumber,
...
exports.findOne=(req,res)=>{
   directorproprietoridentitycheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    directorproprietoridentitycheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    directorproprietoridentitycheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await directorproprietoridentitycheck.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await directorproprietoridentitycheck.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return directorproprietoridentitycheck.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await directorproprietoridentitycheck.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'directorproprietoridentitycheck Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
exports.deleteCheck=(req,res)=>{
   directorproprietoridentitycheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await directorproprietoridentitycheck.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await directorproprietoridentitycheck.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await directorproprietoridentitycheck.findOne({ _id: req.params.case_id })
