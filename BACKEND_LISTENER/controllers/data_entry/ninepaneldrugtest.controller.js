...
const ninepaneldrugtest = require('../../models/data_entry/ninepaneldrugtest.model')
...
if (!req.body.status) {
    ...
    const obj = new ninepaneldrugtest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "67220111dec2aeecb2d5a9dc" : req.body.component,
        checkId: checkId,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await ninepaneldrugtest.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "ninepaneldrugtest" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving ninepaneldrugtest'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    ninepaneldrugtest
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving ninepaneldrugtest for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    ninepaneldrugtest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    ninepaneldrugtest.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    ninepaneldrugtest
        ...
        ninepaneldrugtest.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await ninepaneldrugtest.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await ninepaneldrugtest.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return ninepaneldrugtest.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateFeVerificationStatus = (req, res) => {
    ninepaneldrugtest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {drugsRhs: req.body.drugsRhs,resultRhs: req.body.resultRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,nameRhs: req.body.nameRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    ninepaneldrugtest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        drugsRhs: req.body.drugsRhs,resultRhs: req.body.resultRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,nameRhs: req.body.nameRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await ninepaneldrugtest.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'ninepaneldrugtest Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         ninepaneldrugtest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {drugsRhs: req.body.data.drugsRhs,resultRhs: req.body.data.resultRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,nameRhs: req.body.data.nameRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'ninepaneldrugtest Details'</td><td>${data.drugs}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    ninepaneldrugtest.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await ninepaneldrugtest.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await ninepaneldrugtest.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await ninepaneldrugtest.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In ninepaneldrugtest Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In ninepaneldrugtest for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        ninepaneldrugtest.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
