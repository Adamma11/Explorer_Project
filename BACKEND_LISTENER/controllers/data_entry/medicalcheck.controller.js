...
const medicalcheck = require('../../models/data_entry/medicalcheck.model')
...
if (!req.body.status) {
    ...
    const obj = new medicalcheck({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "68d37671a9c6178d41437e01" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
hospitalname: req.body.hospitalname,
            hospitalnameRhs: null,
hospitaladdress: req.body.hospitaladdress,
            hospitaladdressRhs: null,
remarks: req.body.remarks,
            remarksRhs: null,
result: req.body.result,
            resultRhs: null,
dateofmedicalcheck: req.body.dateofmedicalcheck,
            dateofmedicalcheckRhs: null,
mov: req.body.mov,
            movRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await medicalcheck.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "medicalcheck" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving medicalcheck'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    medicalcheck
      // .find({ case: req.params.case })
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving medicalcheck for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    medicalcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    medicalcheck.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    medicalcheck
        ...
        medicalcheck.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
    try {
        ...
        const existing = await medicalcheck.findOne({ case: caseId, _id: componentId });
        ...
        if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
            ...
            const updated = await medicalcheck.findOneAndUpdate(
        ...
        if (req.body.status === 'DRAFT') {
            return medicalcheck.findOneAndUpdate(
                { case: caseId, _id: componentId },
                {
                    ...updateData,
                    stage: 'WIP'
...
// officernameRhs: req.body.officernameRhs,
// hospitalnameRhs: req.body.hospitalnameRhs,
// hospitaladdressRhs: req.body.hospitaladdressRhs,
// remarksRhs: req.body.remarksRhs,
// resultRhs: req.body.resultRhs,
// dateofmedicalcheckRhs: req.body.dateofmedicalcheckRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    medicalcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,officernameRhs: req.body.officernameRhs,hospitalnameRhs: req.body.hospitalnameRhs,hospitaladdressRhs: req.body.hospitaladdressRhs,remarksRhs: req.body.remarksRhs,resultRhs: req.body.resultRhs,dateofmedicalcheckRhs: req.body.dateofmedicalcheckRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'medicalcheck Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         medicalcheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,addressRhs: req.body.data.addressRhs,officernameRhs: req.body.data.officernameRhs,hospitalnameRhs: req.body.data.hospitalnameRhs,hospitaladdressRhs: req.body.data.hospitaladdressRhs,remarksRhs: req.body.data.remarksRhs,resultRhs: req.body.data.resultRhs,dateofmedicalcheckRhs: req.body.data.dateofmedicalcheckRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'medicalcheck Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    medicalcheck.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await medicalcheck.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await medicalcheck.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await medicalcheck.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In medicalcheck Create for CDE")
...
if (!req.body.officername) {
            res.status(400).json({ message: "officername is Mandatory" })
        }
if (!req.body.hospitalname) {
            res.status(400).json({ message: "hospitalname is Mandatory" })
        }
if (!req.body.hospitaladdress) {
            res.status(400).json({ message: "hospitaladdress is Mandatory" })
        }
if (!req.body.remarks) {
...
if (!req.body.dateofmedicalcheck) {
            res.status(400).json({ message: "dateofmedicalcheck is Mandatory" })
        }
if (!req.body.mov) {
...
exports.findOneForCde = (req, res) => {
    console.log("In medicalcheck for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        medicalcheck.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
