...
const dinverification = require('../../models/data_entry/dinverification.model')
...
if (!req.body.status) {
    ...
    const obj = new dinverification({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "67222997ff5ce4ee231ab85d" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
directorname: req.body.directorname,
            directornameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await dinverification.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "dinverification" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving dinverification'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    dinverification
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving dinverification for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    dinverification.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    dinverification.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    dinverification
        ...
        dinverification.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await dinverification.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await dinverification.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return dinverification.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
// directornameRhs: req.body.directornameRhs,
// dinnumberRhs: req.body.dinnumberRhs,
// dobRhs: req.body.dobRhs,
// sourceRhs: req.body.sourceRhs,
// verifiedbyRhs: req.body.verifiedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    dinverification.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,directornameRhs: req.body.directornameRhs,dinnumberRhs: req.body.dinnumberRhs,dobRhs: req.body.dobRhs,sourceRhs: req.body.sourceRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    dinverification.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,directornameRhs: req.body.directornameRhs,dinnumberRhs: req.body.dinnumberRhs,dobRhs: req.body.dobRhs,sourceRhs: req.body.sourceRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await dinverification.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'dinverification Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'dinverification Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    dinverification.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await dinverification.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await dinverification.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await dinverification.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In dinverification Create for CDE")
...
if (!req.body.directorname) {
            res.status(400).json({ message: "directorname is Mandatory" })
        }
...
if (!req.body.source) {
            res.status(400).json({ message: "source is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In dinverification for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        dinverification.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
