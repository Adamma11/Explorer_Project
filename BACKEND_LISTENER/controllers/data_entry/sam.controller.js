const sam = require('../../models/data_entry/sam.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
  ...
  checkId: checkId,
   component:req.body.component ==null ? "650ad488c176fe8818613137":req.body.component,
       name:req.body.name,
       nameRhs:null,
       dob:req.body.dob,
       dobRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"sam"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving sam'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving sam for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   sam.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       dob:req.body.dob,
       fathername:req.body.fathername,
       verifiedby:req.body.verifiedby,
...
exports.findOne=(req,res)=>{
   sam.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    sam.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await sam.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await sam.findOneAndUpdate(
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'sam Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
exports.deleteCheck=(req,res)=>{
   sam.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await sam.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await sam.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await sam.findOne({ _id: req.params.case_id })
