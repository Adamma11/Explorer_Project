...
exports.create = async (req, res) => {
    ...
    const caseObj = await await Case.findOne({_id: req.body.case})
...
if (!req.body.status) {
    ...
    const obj = new employmenttest({
        ...
        component: req.body.component == null ? "66e7e376fc5986b88671b12d" : req.body.component,
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await employmenttest.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await employmenttest.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return employmenttest.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await employmenttest.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'employmenttest Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
exports.getAllChecksAllocatedToMeForVerification = (req, res) => {
    ...
    employmenttest
        ...
        .find({ $or: [{ status: 'INPUTQC-ACCEPTED', verificationAllocatedTo: req.user.user_id }, { status: 'MENTOR-REVIEW-REJECTED', verificationAllocatedTo: req.user.user_id }, { status: 'ON-HOLD', verificationAllocatedTo: req.user.user_id }, { status: 'OUTPUTQC-REJECTED', verificationAllocatedTo: req.user.user_id }] })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await employmenttest.findOne({ _id: req.params._id })
