const idbglobalsolutions = require('../../models/data_entry/idbglobalsolutions.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   if(!req.body.idtypeverified){
       res.status(400).json({message:'ID Type Verified required'})
   }
   if(!req.body.idnumberverified){
       res.status(400).json({message:'ID NumberVerified required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new idbglobalsolutions({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "645dde66da4885b59f88f9a6":req.body.component,
       name:req.body.name,
       nameRhs:null,
       ...
       dateofbirthRhs:null,
       idtypeverified:req.body.idtypeverified,
       idtypeverifiedRhs:null,
       idnumberverified:req.body.idnumberverified,
       idnumberverifiedRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"idbglobalsolutions"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving idbglobalsolutions'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving idbglobalsolutions for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   idbglobalsolutions.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       entity:req.body.entity,
       modeofverification:req.body.modeofverification,
       datasource:req.body.datasource,
...
exports.findOne=(req,res)=>{
   idbglobalsolutions.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    idbglobalsolutions.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await idbglobalsolutions.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await idbglobalsolutions.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return idbglobalsolutions.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
//        entityRhs:req.body.entityRhs,
//        modeofverificationRhs:req.body.modeofverificationRhs,
//        datasourceRhs:req.body.datasourceRhs,
//        verifiedonRhs:req.body.verifiedonRhs,
//        fathersnameRhs:req.body.fathersnameRhs,
//        dateofbirthRhs:req.body.dateofbirthRhs,
//        idtypeverifiedRhs:req.body.idtypeverifiedRhs,
//        idnumberverifiedRhs:req.body.idnumberverifiedRhs,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await idbglobalsolutions.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'idbglobalsolutions Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//        entityRhs:req.body.data.entityRhs,
//        modeofverificationRhs:req.body.data.modeofverificationRhs,
//        datasourceRhs:req.body.data.datasourceRhs,
//        verifiedonRhs:req.body.data.verifiedonRhs,
//        fathersnameRhs:req.body.data.fathersnameRhs,
//        dateofbirthRhs:req.body.data.dateofbirthRhs,
//        idtypeverifiedRhs:req.body.data.idtypeverifiedRhs,
//        idnumberverifiedRhs:req.body.data.idnumberverifiedRhs,
...
exports.deleteCheck=(req,res)=>{
   idbglobalsolutions.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await idbglobalsolutions.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await idbglobalsolutions.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await idbglobalsolutions.findOne({ _id: req.params.case_id })
