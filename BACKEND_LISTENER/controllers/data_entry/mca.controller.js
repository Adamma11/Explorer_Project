...
if (!req.body.status) {
    ...
    const obj = new mca({
        ...
        component: req.body.component == null ? "6723cc361272f0f57d28001d" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await mca.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await mca.findOneAndUpdate(
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'mca Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
