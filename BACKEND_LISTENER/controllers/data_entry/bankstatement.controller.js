...
if (!req.body.status) {
    ...
    const obj = new bankstatement({
        ...
        component: req.body.component == null ? "67220888dec2aeecb2d5b816" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await bankstatement.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
...
exports.updateFeVerificationStatus = (req, res) => {
    bankstatement.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,banknameRhs: req.body.banknameRhs,numberRhs: req.body.numberRhs,branchRhs: req.body.branchRhs,startdateRhs: req.body.startdateRhs,enddateRhs: req.body.enddateRhs,salaryRhs: req.body.salaryRhs,salarydateRhs: req.body.salarydateRhs,verifiedbyRhs: req.body.verifiedbyRhs,VerifiedonRhs: req.body.VerifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
/*exports.updateMentorReviewStatus = (req, res) => {
        ...
        bankstatement.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,banknameRhs: req.body.data.banknameRhs,numberRhs: req.body.data.numberRhs,branchRhs: req.body.data.branchRhs,startdateRhs: req.body.data.startdateRhs,enddateRhs: req.body.data.enddateRhs,salaryRhs: req.body.data.salaryRhs,salarydateRhs: req.body.data.salarydateRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,VerifiedonRhs: req.body.data.VerifiedonRhs,movRhs: req.body.data.movRhs,
status: req.body.status,
                        ...
                        if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                            Case
                                ...
                                .then(caseData => {
                                    ...
                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'bankstatement Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.createForCde = (req, res) => {
    console.log("In bankstatement Create for CDE")
...
if (!req.body.bankname) {
            res.status(400).json({ message: "bankname is Mandatory" })
        }
if (!req.body.number) {
            res.status(400).json({ message: "number is Mandatory" })
        }
if (!req.body.branch) {
            res.status(400).json({ message: "branch is Mandatory" })
        }
if (!req.body.startdate) {
            res.status(400).json({ message: "startdate is Mandatory" })
        }
if (!req.body.enddate) {
            res.status(400).json({ message: "enddate is Mandatory" })
        }
if (!req.body.salary) {
            res.status(400).json({ message: "salary is Mandatory" })
        }
if (!req.body.salarydate) {
            res.status(400).json({ message: "salarydate is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In bankstatement for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        bankstatement.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await bankstatement.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await bankstatement.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return bankstatement.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let mailText = subclientNotificationData.template.content;
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'bankstatement Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
                let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails);
