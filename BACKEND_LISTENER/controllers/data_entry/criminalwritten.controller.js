...
const criminalwritten = require('../../models/data_entry/criminalwritten.model')
...
if (!req.body.status) {
    ...
    const obj = new criminalwritten({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "672227d3ff5ce4ee231aa921" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await criminalwritten.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "criminalwritten" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving criminalwritten'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    criminalwritten
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving criminalwritten for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    criminalwritten.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    criminalwritten.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    criminalwritten
        ...
        criminalwritten.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await criminalwritten.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await criminalwritten.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return criminalwritten.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
// verifedbyRhs: req.body.verifedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    criminalwritten.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,verifedbyRhs: req.body.verifedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    criminalwritten.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,verifedbyRhs: req.body.verifedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await criminalwritten.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'criminalwritten Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         criminalwritten.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,addressRhs: req.body.data.addressRhs,verifedbyRhs: req.body.data.verifedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'criminalwritten Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    criminalwritten.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await criminalwritten.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await criminalwritten.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await criminalwritten.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In criminalwritten Create for CDE")
...
if (!req.body.verifedby) {
            res.status(400).json({ message: "verifedby is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In criminalwritten for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        criminalwritten.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
