	...
	const idbgdb = require('../../models/data_entry/idbgdb.model')
	...
	if (!req.body.status) {
	    ...
	    const obj = new idbgdb({
		case: req.body.case,
		personalDetailsData: req.body.personalDetails,
		component: req.body.component == null ? "6723cb711272f0f57d27f38f" : req.body.component,
		checkId: checkId,
	name: req.body.name,
		    nameRhs: null,
	dob: req.body.dob,
	...
	status: req.body.status,
		...
		.then(data => {
			...
			.then(async caseData => {
			    await idbgdb.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
			//.then(caseData => {
			    let checksEntered = 0
		    ...
		    Case
			.update({ _id: req.body.case }, { $push: { actualComponents: "idbgdb" } })
			.then(data => {
		...
		.catch(err => {
		    res.status(500).json({
			message: err.message || 'Some error while saving idbgdb'
		    })
		})
	};
...
exports.findAllForACase = (req, res) => {
		...
		.catch(err => {
		    res.status(500).json({
			message: err.message || 'Some error while retrieving idbgdb for cases'
		    })
		})
	};
	...
	exports.update = (req, res) => {
	    ...
	    idbgdb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
	...
	exports.findOne = (req, res) => {
	    idbgdb.findOne({ case: req.params.caseId, _id: req.params.componentId })
		.then(data => {
		    res.json(data)
		})
		.catch(err => {
	...
	exports.findComponentsForVendorManager = (req, res) => {
		...
		idbgdb.find({ status: req.params.status })
		.sort({ _id: 1 })
		.skip(offSet)
		.limit(200)
		.populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
  ...
  try {
    ...
    const existing = await idbgdb.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await idbgdb.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return idbgdb.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
	...
	exports.updateFeVerificationStatus = (req, res) => {
	    idbgdb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,dobRhs: req.body.dobRhs,fathernameRhs: req.body.fathernameRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
	status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'idbgdb Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
	...
	// 	idbgdb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,dobRhs: req.body.data.dobRhs,fathernameRhs: req.body.data.fathernameRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
	// status: req.body.status,
	// 	mentorReviewCompletionDate: new Date(),
	// 		mentorReviewComments: req.body.mentorReviewComments,
	// 		mentorReviewCompletedBy: req.user.user_id
	...
	// 				    let mailText = subclientNotificationData.template.content
	// 				    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'idbgdb Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
	// 				    let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    idbgdb.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await idbgdb.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await idbgdb.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await idbgdb.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In idbgdb Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In idbgdb for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        idbgdb.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
