...
const identityanddigital = require('../../models/data_entry/identityanddigital.model')
...
if (!req.body.status) {
    ...
    const obj = new identityanddigital({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "6723cbc61272f0f57d27f8b9" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await identityanddigital.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "identityanddigital" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving identityanddigital'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    identityanddigital
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving identityanddigital for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    identityanddigital.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    identityanddigital.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    identityanddigital
        ...
        identityanddigital.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await identityanddigital.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await identityanddigital.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return identityanddigital.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
// photoRhs: req.body.photoRhs,
// resultRhs: req.body.resultRhs,
// discrepancyRhs: req.body.discrepancyRhs,
// verifiedbyRhs: req.body.verifiedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    identityanddigital.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,idtypeRhs: req.body.idtypeRhs,idnumberRhs: req.body.idnumberRhs,photoRhs: req.body.photoRhs,resultRhs: req.body.resultRhs,discrepancyRhs: req.body.discrepancyRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    identityanddigital.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,idtypeRhs: req.body.idtypeRhs,idnumberRhs: req.body.idnumberRhs,photoRhs: req.body.photoRhs,resultRhs: req.body.resultRhs,discrepancyRhs: req.body.discrepancyRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await identityanddigital.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'identityanddigital Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         identityanddigital.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,idtypeRhs: req.body.data.idtypeRhs,idnumberRhs: req.body.data.idnumberRhs,photoRhs: req.body.data.photoRhs,resultRhs: req.body.data.resultRhs,discrepancyRhs: req.body.data.discrepancyRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'identityanddigital Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    identityanddigital.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await identityanddigital.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await identityanddigital.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await identityanddigital.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In identityanddigital Create for CDE")
...
if (!req.body.photo) {
            res.status(400).json({ message: "photo is Mandatory" })
        }
if (!req.body.result) {
            res.status(400).json({ message: "result is Mandatory" })
        }
if (!req.body.discrepancy) {
            res.status(400).json({ message: "discrepancy is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In identityanddigital for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        identityanddigital.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
