const drivinglicensecheck = require('../../models/data_entry/drivinglicensecheck.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   if(!req.body.idnumber){
       res.status(400).json({message:'ID Number required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new drivinglicensecheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "645e0287da4885b59f88fdec":req.body.component,
       name:req.body.name,
       nameRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"drivinglicensecheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving drivinglicensecheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving drivinglicensecheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   drivinglicensecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       idtype:req.body.idtype,
       idnumber:req.body.idnumber,
       expirydate:req.body.expirydate,
...
exports.findOne=(req,res)=>{
   drivinglicensecheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    drivinglicensecheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    drivinglicensecheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await drivinglicensecheck.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await drivinglicensecheck.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return drivinglicensecheck.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await drivinglicensecheck.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'drivinglicensecheck Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//        expirydateRhs:req.body.data.expirydateRhs,
//        verifiedbyRhs:req.body.data.verifiedbyRhs,
//        verifiedonRhs:req.body.data.verifiedonRhs,
//        modeofverificationRhs:req.body.data.modeofverificationRhs,
// 	     status:req.body.status,
...
exports.deleteCheck=(req,res)=>{
   drivinglicensecheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await drivinglicensecheck.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await drivinglicensecheck.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await drivinglicensecheck.findOne({ _id: req.params._id })
