...
const identitytest = require('../../models/data_entry/identitytest.model')
...
if (!req.body.status) {
    ...
    const obj = new identitytest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "6548c3702b6cd2c20faf3124" : req.body.component,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await identitytest.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "identitytest" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving identitytest'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    identitytest
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving identitytest for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    identitytest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    identitytest.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    identitytest
        ...
        identitytest.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await identitytest.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await identitytest.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return identitytest.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
//         const identitytestData = await identitytest.findOne({ case: req.params.caseId, _id: req.params.componentId })
...
//         } else if (!identitytestData.secondInsufficiencyRaisedDate) {
...
//         } else if (!identitytestData.thirdInsufficiencyRaisedDate) {
...
//         } else if (!identitytestData.fourthInsufficiencyRaisedDate) {
...
//         } else if (!identitytestData.fifthInsufficiencyRaisedDate) {
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await identitytest.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'identitytest Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//     identitytest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
//          nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,idtypeRhs: req.body.data.idtypeRhs,idnumberRhs: req.body.data.idnumberRhs,resultRhs: req.body.data.resultRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// 	    status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
//         mentorReviewComments: req.body.mentorReviewComments,
//         mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'identitytest Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.approveInsuff2 = async (req, res) => {
    ...
    try {
        ...
        if (!identitytesttData.secondInsufficiencyRaisedDate) {

            const data = await identitytest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.searchACaseForInsuffForClient = (req, res) => {
    ...
    console.log("In search for insuff case identitytest, case id required is ", req.query.caseId)

    Case
        .findOne({ caseId: req.query.caseId })
        .then(caseData => {
            console.log("Case Data found for identitytest insuff")
            ...
            UserSubclientAccess.find({ user: req.user.user_id }, { _id: 0, subclient: 1 })
                .then(userSubclientData => {
                    console.log("In identitytest insuff user subclient access is ", userSubclientData)
                    identitytest
                        ...
                        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
                        .then(data => {
                            console.log("Found the identitytest insuff data ", data)
                            res.json(data)
                        })
                        .catch(err => {
                            res.status(500).json({
                                message: "Error searching insuff for identitytest"
                            })
                        })
                })
                .catch(err => {
...
exports.approveInsuffClearance = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == "INSUF-2-CLEARED") {
            ...
            if (!identitytestData.secondInsufficiencyClearedDate) {
            ...
            } else if (!identitytestData.thirdInsufficiencyClearedDate) {
            ...
            } else if (!identitytestData.fourthInsufficiencyClearedDate) {
            ...
            } else if (!identitytestData.fifthInsufficiencyClearedDate) {
...
exports.deleteCheck = (req, res) => {
    identitytest.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull = (req, res) => {
    identitytest
        ...
        .then(data => {
            data.forEach(item => {
                console.log("About to update subclient of identitytest", item._id)
                identitytest
                    .findOneAndUpdate({ _id: item._id }, { subclient: item.case.subclient._id, client: item.case.subclient.client._id })
                    .then(data => {
                    })
                    .catch(err => {
                        console.log("Error in updating identitytest ", err)
                    })
            })
            res.json({ message: "Completed" })
        })
        .catch(err => {
            console.log("Error reading identitytest", err)
            res.status(500).json({
...
exports.createForCde = (req, res) => {
    console.log("In identitytest Create for CDE")
...
if (!req.body.status) {
    ...
    console.log("Case Id for identitytest CDE ", caseId)
    if (caseId != null) {
        const obj = new identitytest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
...
status: req.body.status,
        ...
        console.log("Object for saving created in CDE for identitytest")
        if (req.body.status == 'CDE-COMPLETED') {
            obj.dataEntryCompletionDate = new Date()
        } else {
            obj.insufficiencyComments = req.body.insufficiencyComments
        }
        console.log("About to save identitytest for CDE")
        obj
            .save(obj)
            .then(data => {
            ...
            .catch(err => {
                console.log("Error Occurred while creating the identitytest for CDE ", err)
                res.status(500).json({
                    message: err.message || 'Some error while saving identitytest'
                })
            })
...
exports.findOneForCde = (req, res) => {
    console.log("In identitytest for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        identitytest.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await identitytest.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await identitytest.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await identitytest.findOne({ _id: req.params.case_id })
