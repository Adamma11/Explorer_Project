...
const directorproprietorcrc = require('../../models/data_entry/directorproprietorcrc.model')
...
if (!req.body.status) {
    ...
    const obj = new directorproprietorcrc({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "67222a46ff5ce4ee231abf9a" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await directorproprietorcrc.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "directorproprietorcrc" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving directorproprietorcrc'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    directorproprietorcrc
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving directorproprietorcrc for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    directorproprietorcrc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    directorproprietorcrc.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsFor = (req, res) => {
    ...
    directorproprietorcrc.find(query)
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    directorproprietorcrc
        ...
        directorproprietorcrc.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await directorproprietorcrc.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await directorproprietorcrc.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return directorproprietorcrc.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateFeVerificationStatus = (req, res) => {
    directorproprietorcrc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,posRhs: req.body.posRhs,proceedingsRhs: req.body.proceedingsRhs,searchRhs: req.body.searchRhs,sessioncourtRhs: req.body.sessioncourtRhs,civilcourtRhs: req.body.civilcourtRhs,magistrateRhs: req.body.magistrateRhs,appelsRhs: req.body.appelsRhs,highcourtRhs: req.body.highcourtRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await directorproprietorcrc.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'directorproprietorcrc Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         directorproprietorcrc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,addressRhs: req.body.data.addressRhs,posRhs: req.body.data.posRhs,proceedingsRhs: req.body.data.proceedingsRhs,searchRhs: req.body.data.searchRhs,sessioncourtRhs: req.body.data.sessioncourtRhs,civilcourtRhs: req.body.data.civilcourtRhs,magistrateRhs: req.body.data.magistrateRhs,appelsRhs: req.body.data.appelsRhs,highcourtRhs: req.body.data.highcourtRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'directorproprietorcrc Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    directorproprietorcrc.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await directorproprietorcrc.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await directorproprietorcrc.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await directorproprietorcrc.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In directorproprietorcrc Create for CDE")
...
if (!req.body.appels) {
            res.status(400).json({ message: "appels is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In directorproprietorcrc for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        directorproprietorcrc.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
