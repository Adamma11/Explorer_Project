...
const cvc = require('../../models/data_entry/cvc.model')
...
if (!req.body.status) {
    ...
    const obj = new cvc({
        ...
        component: req.body.component == null ? "67222976ff5ce4ee231ab494" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
magistrate: req.body.magistrate,
            magistrateRhs: null,
appeals: req.body.appeals,
            appealsRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await cvc.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "cvc" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving cvc'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    cvc
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving cvc for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    cvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    cvc.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    cvc
        ...
        cvc.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await cvc.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await cvc.findOneAndUpdate(
...
// numberofyearsRhs: req.body.numberofyearsRhs,
// resultRhs: req.body.resultRhs,
// dateRhs: req.body.dateRhs,
// commentsRhs: req.body.commentsRhs,
...
// magistrateRhs: req.body.magistrateRhs,
// appealsRhs: req.body.appealsRhs,
// highcourtRhs: req.body.highcourtRhs,
// supremeRhs: req.body.supremeRhs,
// verifiedbyRhs: req.body.verifiedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    cvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,numberofyearsRhs: req.body.numberofyearsRhs,resultRhs: req.body.resultRhs,dateRhs: req.body.dateRhs,commentsRhs: req.body.commentsRhs,proceedingsRhs: req.body.proceedingsRhs,searchRhs: req.body.searchRhs,civilcourtRhs: req.body.civilcourtRhs,sessioncourtRhs: req.body.sessioncourtRhs,magistrateRhs: req.body.magistrateRhs,appealsRhs: req.body.appealsRhs,highcourtRhs: req.body.highcourtRhs,supremeRhs: req.body.supremeRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'cvc Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         cvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,addressRhs: req.body.data.addressRhs,numberofyearsRhs: req.body.data.numberofyearsRhs,resultRhs: req.body.data.resultRhs,dateRhs: req.body.data.dateRhs,commentsRhs: req.body.data.commentsRhs,proceedingsRhs: req.body.data.proceedingsRhs,searchRhs: req.body.data.searchRhs,civilcourtRhs: req.body.data.civilcourtRhs,sessioncourtRhs: req.body.data.sessioncourtRhs,magistrateRhs: req.body.data.magistrateRhs,appealsRhs: req.body.data.appealsRhs,highcourtRhs: req.body.data.highcourtRhs,supremeRhs: req.body.data.supremeRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'cvc Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    cvc.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await cvc.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await cvc.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await cvc.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In cvc Create for CDE")
...
if (!req.body.numberofyears) {
            res.status(400).json({ message: "numberofyears is Mandatory" })
        }
if (!req.body.result) {
            res.status(400).json({ message: "result is Mandatory" })
        }
if (!req.body.date) {
            res.status(400).json({ message: "date is Mandatory" })
        }
if (!req.body.comments) {
            res.status(400).json({ message: "comments is Mandatory" })
        }
...
if (!req.body.magistrate) {
            res.status(400).json({ message: "magistrate is Mandatory" })
        }
if (!req.body.appeals) {
            res.status(400).json({ message: "appeals is Mandatory" })
        }
if (!req.body.highcourt) {
            res.status(400).json({ message: "highcourt is Mandatory" })
        }
if (!req.body.supreme) {
            res.status(400).json({ message: "supreme is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In cvc for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        cvc.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
