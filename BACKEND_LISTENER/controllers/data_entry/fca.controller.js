...
const fca = require('../../models/data_entry/fca.model')
...
if (!req.body.status) {
    ...
    const obj = new fca({
        ...
        component: req.body.component == null ? "6728614243b2217ca9758f87" : req.body.component,
        checkId: checkId,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await fca.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "fca" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving fca'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    fca
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving fca for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    fca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    fca.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    fca
        ...
        fca.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await fca.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await fca.findOneAndUpdate(
...
// 	};
	...
	exports.updateFeVerificationStatus = (req, res) => {
	    fca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {firstnameRhs: req.body.firstnameRhs,middlenameRhs: req.body.middlenameRhs,lastnameRhs: req.body.lastnameRhs,dobRhs: req.body.dobRhs,numberRhs: req.body.numberRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
	status: req.body.status,
	...
	exports.updateVendorVerificationStatus = (req, res) => {
	    fca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
		firstnameRhs: req.body.firstnameRhs,middlenameRhs: req.body.middlenameRhs,lastnameRhs: req.body.lastnameRhs,dobRhs: req.body.dobRhs,numberRhs: req.body.numberRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
	status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'fca Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
	...
	// 	fca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {firstnameRhs: req.body.data.firstnameRhs,middlenameRhs: req.body.data.middlenameRhs,lastnameRhs: req.body.data.lastnameRhs,dobRhs: req.body.data.dobRhs,numberRhs: req.body.data.numberRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
	// status: req.body.status,
	// 	mentorReviewCompletionDate: new Date(),
	// 		mentorReviewComments: req.body.mentorReviewComments,
	// 		mentorReviewCompletedBy: req.user.user_id
	...
	// 				    let mailText = subclientNotificationData.template.content
	// 				    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'fca Details'</td><td>${data.firstname}</td><td>${data.gradingComments}</td></tr></table>`
	// 				    let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    fca.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await fca.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await fca.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await fca.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In fca Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In fca for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        fca.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
