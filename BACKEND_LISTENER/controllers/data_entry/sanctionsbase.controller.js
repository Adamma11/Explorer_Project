const sanctionsbase = require('../../models/data_entry/sanctionsbase.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   const obj = new sanctionsbase({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "650ad80ec176fe8818613199":req.body.component,
       name:req.body.name,
       nameRhs:null,
       dob:req.body.dob,
       dobRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"sanctionsbase"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving sanctionsbase'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving sanctionsbase for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   sanctionsbase.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       dob:req.body.dob,
       fathername:req.body.fathername,
       verifiedby:req.body.verifiedby,
...
exports.findOne=(req,res)=>{
   sanctionsbase.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    sanctionsbase.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await sanctionsbase.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await sanctionsbase.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return sanctionsbase.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await sanctionsbase.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'sanctionsbase Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
exports.deleteCheck=(req,res)=>{
   sanctionsbase.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await sanctionsbase.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await sanctionsbase.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await sanctionsbase.findOne({ _id: req.params.case_id })
