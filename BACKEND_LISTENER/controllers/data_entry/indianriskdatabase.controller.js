...
const indianriskdatabase = require('../../models/data_entry/indianriskdatabase.model')
...
if (!req.body.status) {
    ...
    const obj = new indianriskdatabase({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "6723cbe31272f0f57d27fb42" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await indianriskdatabase.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "indianriskdatabase" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving indianriskdatabase'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    indianriskdatabase
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving indianriskdatabase for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    indianriskdatabase.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    indianriskdatabase.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    indianriskdatabase
        ...
        indianriskdatabase.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
vrifiedonRhs: req.body.vrifiedonRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    indianriskdatabase.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,verifiedbyRhs: req.body.verifiedbyRhs,vrifiedonRhs: req.body.vrifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    indianriskdatabase.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,verifiedbyRhs: req.body.verifiedbyRhs,vrifiedonRhs: req.body.vrifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
/*exports.updateMentorReviewStatus = (req, res) => {
        ...
        indianriskdatabase.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,vrifiedonRhs: req.body.data.vrifiedonRhs,movRhs: req.body.data.movRhs,
status: req.body.status,
                        ...
                        if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                            Case
                                ...
                                .then(caseData => {
                                    ...
                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'indianriskdatabase Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.deleteCheck = (req, res) => {
    indianriskdatabase.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await indianriskdatabase.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await indianriskdatabase.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await indianriskdatabase.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In indianriskdatabase Create for CDE")
...
if (!req.body.vrifiedon) {
            res.status(400).json({ message: "vrifiedon is Mandatory" })
        }
if (!req.body.mov) {
...
exports.findOneForCde = (req, res) => {
    console.log("In indianriskdatabase for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        indianriskdatabase.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await indianriskdatabase.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await indianriskdatabase.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return indianriskdatabase.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
//added code nov-07
        ...
        if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
            ...
            await indianriskdatabase.findOneAndUpdate(
                { case: caseId, _id: componentId },
                {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let mailText = subclientNotificationData.template.content;
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'indianriskdatabase Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
                let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails);
