...
const pvc = require('../../models/data_entry/pvc.model')
...
if (!req.body.status) {
    ...
    const obj = new pvc({
        ...
        component: req.body.component == null ? "6723e58df072dffc31fe5008" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
address: req.body.address,
            addressRhs: null,
liasoning: req.body.liasoning,
            liasoningRhs: null,
verification: req.body.verification,
            verificationRhs: null,
station: req.body.station,
            stationRhs: null,
stationaddrerss: req.body.stationaddrerss,
            stationaddrerssRhs: null,
numberofyear: req.body.numberofyear,
            numberofyearRhs: null,
pvcresult: req.body.pvcresult,
            pvcresultRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await pvc.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "pvc" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving pvc'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    pvc
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving pvc for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    pvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    pvc.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    pvc
        ...
        pvc.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await pvc.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await pvc.findOneAndUpdate(
...
// liasoningRhs: req.body.liasoningRhs,
// verificationRhs: req.body.verificationRhs,
// stationRhs: req.body.stationRhs,
// stationaddrerssRhs: req.body.stationaddrerssRhs,
// numberofyearRhs: req.body.numberofyearRhs,
// pvcresultRhs: req.body.pvcresultRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    pvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,addressRhs: req.body.addressRhs,liasoningRhs: req.body.liasoningRhs,verificationRhs: req.body.verificationRhs,stationRhs: req.body.stationRhs,stationaddrerssRhs: req.body.stationaddrerssRhs,numberofyearRhs: req.body.numberofyearRhs,pvcresultRhs: req.body.pvcresultRhs,commentsRhs: req.body.commentsRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
//added code nov-07
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'pvc Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         pvc.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,addressRhs: req.body.data.addressRhs,liasoningRhs: req.body.data.liasoningRhs,verificationRhs: req.body.data.verificationRhs,stationRhs: req.body.data.stationRhs,stationaddrerssRhs: req.body.data.stationaddrerssRhs,numberofyearRhs: req.body.data.numberofyearRhs,pvcresultRhs: req.body.data.pvcresultRhs,commentsRhs: req.body.data.commentsRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'pvc Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    pvc.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await pvc.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await pvc.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await pvc.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In pvc Create for CDE")
...
if (!req.body.liasoning) {
            res.status(400).json({ message: "liasoning is Mandatory" })
        }
if (!req.body.verification) {
            res.status(400).json({ message: "verification is Mandatory" })
        }
if (!req.body.station) {
            res.status(400).json({ message: "station is Mandatory" })
        }
if (!req.body.stationaddrerss) {
            res.status(400).json({ message: "stationaddrerss is Mandatory" })
        }
if (!req.body.numberofyear) {
            res.status(400).json({ message: "numberofyear is Mandatory" })
        }
if (!req.body.pvcresult) {
            res.status(400).json({ message: "pvcresult is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In pvc for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        pvc.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
