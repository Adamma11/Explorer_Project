const resumechecks = require('../../models/data_entry/resumechecks.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   const obj = new resumechecks({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "650ab5a5c176fe8818612dcf":req.body.component,
       name:req.body.name,
       nameRhs:null,
       ...
       job:req.body.job,
       jobRhs:null,
       gapsineducation:req.body.gapsineducation,
       gapsineducationRhs:null,
       employment:req.body.employment,
       employmentRhs:null,
       jobs:req.body.jobs,
       jobsRhs:null,
       gapsinemployment:req.body.gapsinemployment,
       gapsinemploymentRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"resumechecks"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving resumechecks'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving resumechecks for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   resumechecks.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       education:req.body.education,
       job:req.body.job,
       gapsineducation:req.body.gapsineducation,
...
exports.findOne=(req,res)=>{
   resumechecks.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    resumechecks.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await resumechecks.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await resumechecks.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return resumechecks.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
//        educationRhs:req.body.educationRhs,
//        jobRhs:req.body.jobRhs,
//        gapsineducationRhs:req.body.gapsineducationRhs,
//        employmentRhs:req.body.employmentRhs,
//        jobsRhs:req.body.jobsRhs,
//        gapsinemploymentRhs:req.body.gapsinemploymentRhs,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await resumechecks.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'resumechecks Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//        educationRhs:req.body.data.educationRhs,
//        jobRhs:req.body.data.jobRhs,
//        gapsineducationRhs:req.body.data.gapsineducationRhs,
//        employmentRhs:req.body.data.employmentRhs,
//        jobsRhs:req.body.data.jobsRhs,
//        gapsinemploymentRhs:req.body.data.gapsinemploymentRhs,
...
// 15 Feb 2025	.find({ $or: [{ status: 'INPUTQC-ACCEPTED', verificationAllocatedTo: req.user.user_id }, { status: 'MENTOR-REVIEW-REJECTED', verificationAllocatedTo: req.user.user_id }, { status: 'OUTPUTQC-REJECTED', verificationAllocatedTo: req.user.user_id },{ status: 'WIP', verificationAllocatedTo: req.user.user_id }] })
...
exports.deleteCheck=(req,res)=>{
   resumechecks.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
/*exports.addquicknote = async (req, res) => {
    try {
        const caseData = await resumechecks.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await resumechecks.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
/*exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await resumechecks.findOne({ _id: req.params.case_id })
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await resumechecks.findOne({ _id: req.body._id })
        const caseComments = []
                Object.assign(caseComments, caseData ? caseData.comments : null)
                const commentToPush = { user: mongoose.Types.ObjectId(req.user.user_id), date: new Date(), comment: req.body.comment, colorType: req.body.colorType }
                caseComments.push(commentToPush)
