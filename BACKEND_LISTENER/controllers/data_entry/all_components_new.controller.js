...
const employmentModel = require('../../models/data_entry/employment.model');
const componentModel = require('../../models/administration/component.model');
const SubclientAccess = require("../../models/administration/user_subclient_access.model")
...
const educationModel  = require('../../models/data_entry/education.model');
      ...
      for (let j = 0; j < modelData.length; j++) {
        ...
        if (status === "MENTOR-REVIEW-ACCEPTED") {
          if (
            ...
            currData.status != "DELETED-CHECK" &&
            currData.status != "STOP-CHECK"			  
          ) {
            statusFound = false;
            break;
          }
        ...
        function stopCheck() {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                if (!item) {
                    ...
                    await model.findOneAndUpdate({ case: case_id, serialNumber: serialNumber }, { status: 'STOP-CHECK' })
...
exports.deleteCheck = async (req, res) => {
	...
	if(modelData.status == "MENTOR-REVIEW-ACCEPTED" || modelData.status == "OUTPUTQC-ACCEPTED"){
      ...
      modelData.caseId,
      model,
      modelData._id,                // was 'data._id', now correct
      'Deleting a Check',           // fixed typo
      'DELETED',
      'Check Deleted',
      null,
      null,
...
exports.uploadCandidateDocs=(req,res)=>{
	...
	const extention = path.extname(componentFile.name)
  	...
  	const uploadPath = '/REPO_STORAGE/case_uploads/'+req.body.caseId +'/' + req.body.componentName+ '/' + req.body.componentId + '/candidatedocs/' + req.body.fileName
...
const filePath  = '/REPO_STORAGE/case_uploads/'+req.params.caseId +'/' + req.params.componentName+ '/' + req.params.check_id + '/proofofwork/' 	  
...
exports.deleteCandidateDocs = async (req, res) => {
    console.log("@@@@@@@@@@@@@@@",req.params.caseId)

 try {
   //:caseId/:componentName/:check_id/:fileName
   const { caseId, componentName, check_id, fileName } = req.params;
const filePath  = '/REPO_STORAGE/case_uploads/'+req.params.caseId +'/' + req.params.componentName+ '/' + req.params.check_id + '/candidatedocs/'
   //const filePath = `/REPO_STORAGE/case_uploads/${caseId}/${componentName}/${check_id}/${fileName}`;
     console.log("@@@@@@@@@@@@@@@",filePath)
   if (fs.existsSync(filePath)) {
     fs.rmSync(filePath + "/" +fs.readdirSync(filePath).find(item => item.includes(req.query.fileName)))
...
/*exports.uploadCDF=(req,res)=>{
    ...
    let filePath = '/REPO_STORAGE/case_uploads/' + req.body.caseId + '/' + req.body.componentName + '/' + req.body.componentId + '/candidatedocs/' + file.name;
...
exports.uploadCDF=(req,res)=>{
	    console.log(" req.files.componentFile;", req.files.componentFile);

    let componentFile = req.files;
	console.log("component Files:",componentFile)
       const files = Object.keys(componentFile)
    ...
    let filePath = '/REPO_STORAGE/case_uploads/' + req.body.caseId + '/' + req.body.componentName + '/' + req.body.componentId + '/candidatedocs/';

	files.forEach(async item => {
		await new Promise((resolve,reject)=>{
			componentFile[item].mv(filePath+item, function (err) {
        if (err) {
            res.status(500).send({ message: "Error uploading the file" });
        }
				resolve()
...
/////////////
 exports.insuffRaised = async (req, res) => {
    try {
        ...
        console.log('singleCase == ', singleCase,mailAddress);
        ...
        if (notification.length > 0) {
		    ...
		    console.log("Email",mailAddress, changedSubject, changedMailText)
                    res.end();
                } catch (error) {
...
exports.sendMailForNextFollowUpdate = async (req, res) => {
        try {
            ...
            const Model = require(`../../models/data_entry/${CompName}.model`)
            const details = await Model.find({ nextfollowupdate: todayFormatted });

            let groupedByEmail = {};

            for (let detail of details) {
                let userEmail = await User.findById(detail.verificationAllocatedTo);
                let caseDetails = await Case.findById(detail.case).populate({ path: 'client', select: 'name' });

                let email = userEmail ? userEmail.userId : 'No email found';

                if (caseDetails) {
                    if (!groupedByEmail[email]) {
                        groupedByEmail[email] = { cases: [] };
                    }
                    groupedByEmail[email].cases.push({
                        caseId: caseDetails.caseId || 'No case found',
                        candidateName: caseDetails.candidateName || 'No candidate found',
                        client: caseDetails.client ? caseDetails.client.name : 'No client found',
                        followUpDate: detail.nextfollowupdate || 'No follow-up date found'
                    });
                }
            }


            for (let [emailAddress, caseData] of Object.entries(groupedByEmail)) {
                if (caseData.cases.length > 0) {
                    let subject = `Candidate Follow-up for Today (${todayFormatted})`;
                    let emailContent = `<h2>Follow-up for Today (${todayFormatted})</h2>`;
                    emailContent += `<table border="1" cellpadding="10"><thead><tr><th>Case ID</th><th>Candidate Name</th><th>Client</th><th>Follow-up Date</th></tr></thead><tbody>`;
                    caseData.cases.forEach(caseDetail => {
                        emailContent += `<tr>
                            <td>${caseDetail.caseId}</td>
                            <td>${caseDetail.candidateName}</td>
                            <td>${caseDetail.client}</td>
                            <td>${caseDetail.followUpDate}</td>
                        </tr>`;
                    });

                    emailContent += `</tbody></table>`;

                    await sendMail.sendMail(emailAddress, subject, emailContent);
                }
            }
            ...
            console.log(`Error in sending Mail for NextFollowUpdate --> ${error}`);
            res.status(500).json({ error: "Error in sending Mail for NextFollowUpdate" });
        }
    }
////////////////////////////////New 15Oct2024///////
// venky 10/10
/*exports.getCompanyNameOfEmployee = async(req,res)=>{
    try {
        ///let companyName =  req.params.name;
	  let companyName = decodeURIComponent(req.params.name);
	    console.log("CCCCCMMC",companyName);

        const [employee] = await Promise.all([
            employmentModel.find({ organization: companyName }),
            //empadvanceModel.find({ nameofemployer: companyName }),
            //empbasicModel.find({nameofemployer:companyName})
        ]);
        
	  const response = {
            employees: [...employee].map(emp => ({
                verifiedon: emp?.verifiedonRhs || "No record found",
                organization: emp.organization || "No record found",
                verifiernameRhs: emp.verifiernameRhs || "No record found",
                verifieremailidRhs: emp.verifieremailidRhs || "No record found"
            })),
        };  
        res.json({response})        
    } catch (error) {
        console.log(`Error in getCompanyNameOfEmployee-- ${error}`)
        res.status(500).json({ message: error });
    }
}*/
exports.getCompanyNameOfEmployee = async (req, res) => {
  try {
    const { employername, compName, institutename } = req.body;

    console.log("Request Body:", req.body);

    if (compName === "employment") {
      const companyName = decodeURIComponent(employername);

      const employees = await employmentModel.find({ organization: companyName });
    ...
    } else if (compName === "education") {
      const instituteName = decodeURIComponent(institutename);

      const institutes = await educationModel.find({ institutename: instituteName });

      const response = {
        employees: institutes.map(emp => ({
                verifiedon: emp?.verifiedonRhs || "No record found",
                institutename: emp.institutename || "No record found",
                verifiernameRhs: emp.verifiernameRhs || "No record found",
                verifieremailidRhs: emp.verifieremailidRhs || "No record found"
        })),
      };
      ...
      return res.status(400).json({ message: "Invalid compName. Must be 'employment' or 'education'." });
    }

  } catch (error) {
    console.error("Error in getCompanyNameOfEmployee:", error);
...
exports.findComponentsFor = (req, res) => {
    ...
    if (req.params.for == 'VENDOR-VERIFIED-FOR-ANALYST') {
        query = { $and: [{ $or: [{ status: 'VENDOR-COMPLETED' }, { status: "VENDOR-INSUF" }, { status: "VENDOR-INSUFFICIENCY" }, { status: 'VENDOR-COULD-NOT-VERIFY' }, { status: "VENDOR-VERIFIED" }] }, { verificationAllocatedTo: req.user.user_id }] }
    }

    if (req.params.for == 'FE-VERIFIED-FOR-ANALYST') {
        query = { $and: [{ $or: [{ status: 'FE-COMPLETED' }, { status: "FE-INSUF" }, { status: "FE-INSUFFICIENCY" }, { status: 'FE-COULD-NOT-VERIFY' }] }, { verificationAllocatedTo: req.user.user_id }] }
    }
    ...
    model.find(query)
        ...
        .lean()
        .then(async data => {
          const subclientAccessData = await SubclientAccess.find({user: req.user.user_id})
            const clientAccess = subclientAccessData.map(item => item.subclient.toString())
	let result = []	
		for(let i=0; i<data.length; i++){
			 console.log(data[i].case.subclient._id.toString())
			if(clientAccess.includes(data[i].case.subclient._id.toString())){
				result.push(data[i])
			}
		}
...
///////////////////////////////
exports.countOfWIPChecksForAnalyst = async (req, res) => {
    try {
        let count = 0
        const statuses = ["INPUTQC-ACCEPTED", "MENTOR-REVIEW-REJECTED", "OUTPUTQC-REJECTED"]
        const componentData = await Component.find().lean()
        for (let i = 0; i < componentData.length; i++) {
            const currComp = componentData[i].name
            const model = require(`../../models/data_entry/${currComp}.model`)
            const modelCount = await model.count({ $and: [{ status: { $in: statuses } }, { verificationAllocatedTo: req.user.user_id }, { stage: { $ne: null } }, { stage: { $exists: true } }] })
            count += modelCount
        }
...
exports.countOfRejectedChecksForAnalyst = async (req, res) => {
    try {
        console.log("Rejected Api Hit")
        let count = 0
        const componentData = await Component.find().lean()
        for (let i = 0; i < componentData.length; i++) {
            const currComp = componentData[i].name
            ...
            const modelCount = await model.count({
                $or: [
                    { status: 'MENTOR-REVIEW-REJECTED', verificationAllocatedTo: req.user.user_id },
                    { status: 'OUTPUTQC-REJECTED', verificationAllocatedTo: req.user.user_id }
                ]
            })
            count += modelCount
        }
...
exports.countOfClosedChecksForTheMonth = async (req, res) => {
    try {
        let count = 0
        const componentData = await Component.find().lean()
        ...
        const todayString = `${year}-${month}-${todayDate + 1}`
        const beginningString = `${year}-${month}-1`
        for (let i = 0; i < componentData.length; i++) {

            const currComp = componentData[i].name
            const model = require(`../../models/data_entry/${currComp}.model`)

            const modelCount = await model.count({ $and: [{ verificationCompletionDate: { $gte: beginningString } }, { verificationCompletionDate: { $lt: todayString } }, { verificationAllocatedTo: req.user.user_id }, { status: { $in: ["VERIFICATION-COMPLETED", "MENTOR-REVIEW-ACCEPTED", "OUTPUTQC-ACCEPTED"] } }] })
            count += modelCount
        }
	console.log("countOfClosedChecksForTheMonth:",count)
        return res.json(count)
    } catch (err) {
        console.log(err)
        return res.status(500).json({ error: err.message })
...
exports.countOfInsuffClearedChecksForAnalyst = async (req, res) => {
    try {
        console.log("Rejected Api Hit")
        let count = 0
        const componentData = await Component.find().lean()
        for (let i = 0; i < componentData.length; i++) {
            ...
            const modelCount = await model.count({
                $or: [
                    { status: { $in: ['INSUF-1-CLEARED', 'INSUF-2-CLEARED', 'INSUF-3-CLEARED', 'INSUF-4-CLEARED', 'INSUF-5-CLEARED'] }, verificationAllocatedTo: req.user.user_id }
                    //{ status: {$in:['INSUF-1-CLEARED','INSUF-2-CLEARED','INSUF-3-CLEARED','INSUF-4-CLEARED','INSUF-5-CLEARED']}, verificationAllocatedTo: req.user.user_id }
                ]
            })
            count += modelCount
        }
        console.log("Insuf Cleared: ", count)
        return res.json(count)
    } catch (err) {
        console.log(err)
        return res.status(500).json({ error: err.message })
...
exports.getPendingAndClosedCountForAnalystPerDay = async (req, res) => {
    try {

        const countPerDay = { allocated: {}, completed: {} }
        const componentData = await Component.find().lean()
        const todayDate = new Date().getDate()
        const year = new Date().getFullYear()
        const month = new Date().getMonth() + 1
        ...
        for (let i = 0; i < componentData.length; i++) {
            const currComp = componentData[i].name
            const model = require(`../../models/data_entry/${currComp}.model`)
            console.log("Current Component:", currComp)
            for (let j = 1; j <= todayDate; j++) {
                const currentDateString = `${year}-${month}-${j}`
                const nextDateString = `${year}-${month}-${j + 1}`

                const allocatedCount = await model.count({
                    $and: [
                        { verificationAllocatedTo: req.user.user_id },
                        {
                            $and: [
                                { inputqcCompletionDate: { $gte: currentDateString, $lt: nextDateString } },
                                { $or: [{ verificationCompletionDate: null }, { verificationCompletionDate: { $exists: false } }] }
                            ]
                        }
                    ]
                });
                const completedCount = await model.count({
                    verificationAllocatedTo: req.user.user_id,
                ...
                countPerDay.allocated[j] = countPerDay.allocated[j] ? countPerDay.allocated[j] + allocatedCount : 1
                countPerDay.completed[j] = countPerDay.completed[j] ? countPerDay.completed[j] + completedCount : 1
            }
        }

        return res.json(countPerDay)
...
exports.getNoOfEffortsOnTodayForAnalyst = async (req, res) => {
    try {
        const requiredRoles = ["Analyst - Address", "Analyst - Education", "Analyst - Employment", "Analyst - Special Checks External", "Analyst - Special Checks Internal"];

        // Get role IDs matching required roles
        const roleData = await Role.find({ name: { $in: requiredRoles } }).select('_id');
        const roleIds = roleData.map(role => role._id);

        // Find user roles matching the role IDs
        const userRole = await UserRole.findOne(
            { role: { $in: roleIds }, user: req.user.user_id }
        ).select('user');


        // Aggregate CaseHistory to count documents matching the criteria
        if (userRole) {
            var result = await CaseHistory.aggregate([
                {
                    $match: {
                        status: "EFFORT ADDED",
                        user: userRole.user,
        ...
        const totalCount = result.length ? result[0].count : 0;
...
exports.getNoOfEffortsPerDayForAnalyst = async (req, res) => {
    try {
        const requiredRoles = ["Analyst - Address", "Analyst - Education", "Analyst - Employment", "Analyst - Special Checks External", "Analyst - Special Checks Internal"];
        ...
        const result = [{ id: "no of efforts", "color": "#2eb393", xData: [], yData: [] }];

        while (startDate.isSameOrBefore(today, 'day')) {
            const count = await CaseHistory.countDocuments({
                status: "EFFORT ADDED",
                user: userRole.user,
                date: { $gte: startDate.toDate(), $lt: startDate.clone().endOf('day').toDate() }
            });

            result[0].xData.push(startDate.date());
            result[0].yData.push(count || 0);
        ...
        console.log(error);
        return res.status(500).send("error whilefetching data");
    }
}
...
exports.addNote = async (req, res) => {
    try {
        const { effortType, note, nextfollowupdate, componentName } = req.body;
        const { caseId, componentId } = req.params;
	    console.log("test ",req.params)
const componentData = await Component.findOne({name: componentName}) 
        // Step 1: Get component info from Component model using _id
const ComponentModel = require(`../../models/data_entry/${componentName}.model`)
        ...
        if (nextfollowupdate) {
            updateFields.nextfollowupdate = nextfollowupdate;
	    stage: null	
        } else {
            updateFields.stage = 'WIP';
        }

        // Step 4: Update the component document
        ...
        if (!updatedComponent) {
            return res.status(404).json({ message: "Component document not found" });
        }

        // Step 5: Log history
	    console.log( componentData._id, componentId)
	    const CaseHistory = require("../../controllers/data_entry/case_history.controller")
        CaseHistory.create(
		req.params.caseId, componentData._id, componentId, 'Effort Added', 'EFFORT ADDED', req.body.note, null, null, null, req.user.user_id
        );
        ...
        res.status(500).json({
            message: err.message || "Some error occurred while updating the component"
        });
    }
};
///////////////get Unallocated  bucket 09Aug2025/////////////////////
//const path = require("path");

exports.findAllUnallocatedComponentsForUser = async (req, res) => {
  try {
    const { componentNames } = req.body; // ["employment", "education", "address", ...]

    if (!Array.isArray(componentNames) || componentNames.length === 0) {
      return res.status(400).json({ message: "componentNames must be a non-empty array" });
    }

    const queryTemplate = {
      $or: [
        { status: "INPUTQC-ACCEPTED", branchAllocatedTo: null },
        { status: "REINITIATED", branchAllocatedTo: null },
        { status: "MENTOR-REVIEW-REJECTED", branchAllocatedTo: null},
        { status: "OUTPUTQC-REJECTED", branchAllocatedTo: null },
        { status: "FE-COMPLETED",  branchAllocatedTo: null },
        { status: "FE-INSUF",  branchAllocatedTo: null },
        { status: "FE-COULD-NOT-VERIFY",  branchAllocatedTo: null },
	{ status: 'ON-HOLD',   branchAllocatedTo: null  },
        { status: 'DRAFT',  branchAllocatedTo: null  } ,
        { status: 'WIP' , branchAllocatedTo: null  }      
      ]
    };
    ...
    const results = await Promise.all(
      componentNames.map(async (name) => {
        try {
          // Dynamically require the model file based on name
          const modelPath = path.join(__dirname, `../../models/data_entry/${name.toLowerCase()}.model.js`);
          const Model = require(modelPath);

          const data = await Model.find(queryTemplate)
            .populate({ path: "case", populate: { path: "subclient", populate: { path: "client" } } })
            .populate("allocatedToFE allocatedToVendor verificationAllocatedTo personalDetailsData")
            .lean();

          return { component: name, data };
        } catch (err) {
          console.error(`Component ${name} model not found or error querying`, err);
          return { component: name, data: [], error: true };
        }
      })
    );
    ...
    console.error("Error fetching unallocated components", err);
    res.status(500).json({ message: "Error fetching unallocated components" });
  }
};
///////////////verification bucket/////////////
exports.getAllChecksAllocatedToMeForVerificationMulti = async (req, res) => {
  try {
    const { componentNames } = req.body; // Example: ["employment", "education", "address"]
    ...
    const queryTemplate = {
      $or: [
        { status: "INPUTQC-ACCEPTED", verificationAllocatedTo: req.user.user_id },
        { status: "REINITIATED", verificationAllocatedTo: req.user.user_id },
        { status: "MENTOR-REVIEW-REJECTED", verificationAllocatedTo: req.user.user_id },
        { status: "OUTPUTQC-REJECTED", verificationAllocatedTo: req.user.user_id },
	{ status: 'ON-HOLD', verificationAllocatedTo: req.user.user_id },      
	{ status: 'DRAFT', verificationAllocatedTo: req.user.user_id } ,
        { status: 'WIP', verificationAllocatedTo: req.user.user_id }      
      ]
    };
    ...
    const results = await Promise.all(
      componentNames.map(async (name) => {
	    ...
	    .populate({ path: 'personalDetailsData' })
          ...
          // Return component name + data
          return {  data };
	  const compData =  data.component || null;

         // return { component: compData, data };

        } catch (err) {
          console.error(`Component ${name} model not found or error querying:`, err.message);
          return { component: name, data: [], error: true };
        }
      })
    );
    ...
    console.error("Error fetching checks for verification", err);
    res.status(500).json({ message: "Error fetching checks for verification" });
  }
};
exports.uploadFile = (req, res) => {
      let componentFile = req.files.componentFile;
    componentFile.mv('/REPO_STORAGE/case_uploads/' + req.body.caseId + '/' + req.body.componentName + '/' + req.body._id + '/candidatedocs/' + componentFile.name + '.pdf', function (err) {
        if (err) {
            res.status(500).send({ message: "Error uploading the file" });
        }
        res.json({ message: "File uploaded" });
...
exports.deleteFile = (req, res) => {
     fs.unlink('/REPO_STORAGE/case_uploads/' + req.params.caseId + '/' + req.params.componentName + '/' + req.params.componentId + '/candidatedocs/' + req.params.fileName + '.pdf', function (err) {
            if (err) {
...
exports.deleteCDF = async (req, res) => {
  try {
    ...
    const filePath = `/REPO_STORAGE/case_uploads/${caseId}`;

    if (fs.existsSync(filePath)) {
      const fileToDelete = fs.readdirSync(filePath).find(item => item.includes(req.query.fileName));
      if (fileToDelete) {
        fs.rmSync(`${filePath}/${fileToDelete}`);
        return res.json({ message: "CDF File Deleted Successfully" });
      } else {
        return res.status(404).json({ error: "File not found" });
      }
    } else {
...
/*exports.uploadCDF = (req, res) => {
  try {
    let uploadedFile = req.files.cdfFile;
    const ext = path.extname(uploadedFile.name);

    const uploadPath = `/REPO_STORAGE/case_uploads/${req.body.caseId}/`;
    ...
    const finalPath = uploadPath + req.body.fileName + ext;
    ...
    uploadedFile.mv(finalPath, (err) => {
      ...
      return res.json({ message: "CDF File Uploaded Successfully" });
    });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: err.message });
