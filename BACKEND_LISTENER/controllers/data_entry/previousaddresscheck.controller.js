...
const previousaddresscheck = require('../../models/data_entry/previousaddresscheck.model')
...
if (!req.body.status) {
    ...
    const obj = new previousaddresscheck({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "6723e398f072dffc31fe4906" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await previousaddresscheck.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "previousaddresscheck" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving previousaddresscheck'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    previousaddresscheck
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving previousaddresscheck for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    previousaddresscheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    previousaddresscheck.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsFor = (req, res) => {
    ...
    previousaddresscheck.find(query)
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    previousaddresscheck
        ...
        previousaddresscheck.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await previousaddresscheck.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await previousaddresscheck.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return previousaddresscheck.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateVendorVerificationStatus = (req, res) => {
    previousaddresscheck.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,addressRhs: req.body.addressRhs,cityRhs: req.body.cityRhs,stateRhs: req.body.stateRhs,postalcodeRhs: req.body.postalcodeRhs,landmarkRhs: req.body.landmarkRhs,posRhs: req.body.posRhs,natureofoccupancyRhs: req.body.natureofoccupancyRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await previousaddresscheck.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'previousaddresscheck Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'previousaddresscheck Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    previousaddresscheck.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await previousaddresscheck.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await previousaddresscheck.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await previousaddresscheck.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In previousaddresscheck Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In previousaddresscheck for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        previousaddresscheck.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
