	...
	const personalreference = require('../../models/data_entry/personalreference.model')
	...
	if (!req.body.status) {
	    ...
	    const obj = new personalreference({
		case: req.body.case,
		personalDetailsData: req.body.personalDetails,
		component: req.body.component == null ? "6723e368f072dffc31fe4733" : req.body.component,
		checkId: checkId,
	name: req.body.name,
		    nameRhs: null,
	refereedetails: req.body.refereedetails,
		    refereedetailsRhs: null,
	...
	status: req.body.status,
		...
		.then(data => {
			...
			.then(async caseData => {
			    await personalreference.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
			//.then(caseData => {
			    let checksEntered = 0
		    ...
		    Case
			.update({ _id: req.body.case }, { $push: { actualComponents: "personalreference" } })
			.then(data => {
		...
		.catch(err => {
		    res.status(500).json({
			message: err.message || 'Some error while saving personalreference'
		    })
		})
	};
...
exports.findAllForACase = (req, res) => {
		...
		.catch(err => {
		    res.status(500).json({
			message: err.message || 'Some error while retrieving personalreference for cases'
		    })
		})
	};
	...
	exports.update = (req, res) => {
	    ...
	    personalreference.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
	...
	exports.findOne = (req, res) => {
	    personalreference.findOne({ case: req.params.caseId, _id: req.params.componentId })
		.then(data => {
		    res.json(data)
		})
		.catch(err => {
	...
	exports.findComponentsForVendorManager = (req, res) => {
		...
		personalreference.find({ status: req.params.status })
		.sort({ _id: 1 })
		.skip(offSet)
		.limit(200)
		.populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
  ...
  try {
    ...
    const existing = await personalreference.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await personalreference.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return personalreference.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
	...
	// refereedetailsRhs: req.body.refereedetailsRhs,
	...
	exports.updateFeVerificationStatus = (req, res) => {
	    personalreference.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,refereedetailsRhs: req.body.refereedetailsRhs,contactRhs: req.body.contactRhs,relationshipRhs: req.body.relationshipRhs,sinceRhs: req.body.sinceRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
	status: req.body.status,
	...
	exports.updateVendorVerificationStatus = (req, res) => {
	    personalreference.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
		nameRhs: req.body.nameRhs,refereedetailsRhs: req.body.refereedetailsRhs,contactRhs: req.body.contactRhs,relationshipRhs: req.body.relationshipRhs,sinceRhs: req.body.sinceRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
	status: req.body.status,
    ...
    try {
		...
		//added code nov-07
				...
				if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
					...
					await personalreference.findOneAndUpdate(
						{ case: caseId, _id: componentId },
						{
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'personalreference Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
	...
	// 	personalreference.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,refereedetailsRhs: req.body.data.refereedetailsRhs,contactRhs: req.body.data.contactRhs,relationshipRhs: req.body.data.relationshipRhs,sinceRhs: req.body.data.sinceRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
	// status: req.body.status,
	// 	mentorReviewCompletionDate: new Date(),
	// 		mentorReviewComments: req.body.mentorReviewComments,
	// 		mentorReviewCompletedBy: req.user.user_id
	...
	// 				    let mailText = subclientNotificationData.template.content
	// 				    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'personalreference Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
	// 				    let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    personalreference.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await personalreference.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await personalreference.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await personalreference.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In personalreference Create for CDE")
...
if (!req.body.refereedetails) {
            res.status(400).json({ message: "refereedetails is Mandatory" })
        }
if (!req.body.contact) {
            res.status(400).json({ message: "contact is Mandatory" })
        }
if (!req.body.relationship) {
            res.status(400).json({ message: "relationship is Mandatory" })
        }
if (!req.body.since) {
...
exports.findOneForCde = (req, res) => {
    console.log("In personalreference for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        personalreference.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
