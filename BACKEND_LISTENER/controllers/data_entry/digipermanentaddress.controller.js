...
const digipermanentaddress = require('../../models/data_entry/digipermanentaddress.model')
...
if (!req.body.status) {
    ...
    const obj = new digipermanentaddress({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "68b03a750d62c5e7c5687706" : req.body.component,
        checkId: checkId,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await digipermanentaddress.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "digipermanentaddress" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving digipermanentaddress'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    digipermanentaddress
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving digipermanentaddress for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    digipermanentaddress.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    digipermanentaddress.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsFor = (req, res) => {
    ...
    digipermanentaddress.find(query)
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    digipermanentaddress
        ...
        digipermanentaddress.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await digipermanentaddress.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await digipermanentaddress.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return digipermanentaddress.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
 ...
 return acc;
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await digipermanentaddress.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let mailText = subclientNotificationData.template.content;
		                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'digipermanentaddress Details'</td><td>${data.completeaddress}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.deleteCheck = (req, res) => {
    digipermanentaddress.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await digipermanentaddress.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await digipermanentaddress.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await digipermanentaddress.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In digipermanentaddress Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In digipermanentaddress for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        digipermanentaddress.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
