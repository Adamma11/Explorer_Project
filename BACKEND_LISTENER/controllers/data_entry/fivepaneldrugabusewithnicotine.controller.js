const fivepaneldrugabusewithnicotine = require('../../models/data_entry/fivepaneldrugabusewithnicotine.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   const obj = new fivepaneldrugabusewithnicotine({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "650ac8f3c176fe8818613047":req.body.component,
       name:req.body.name,
       nameRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"fivepaneldrugabusewithnicotine"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving fivepaneldrugabusewithnicotine'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 fivepaneldrugabusewithnicotine
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving fivepaneldrugabusewithnicotine for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   fivepaneldrugabusewithnicotine.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       drugs:req.body.drugs,
       result:req.body.result,
       verifiedby:req.body.verifiedby,
...
exports.findOne=(req,res)=>{
   fivepaneldrugabusewithnicotine.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    fivepaneldrugabusewithnicotine.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    fivepaneldrugabusewithnicotine.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await fivepaneldrugabusewithnicotine.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await fivepaneldrugabusewithnicotine.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return fivepaneldrugabusewithnicotine.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await fivepaneldrugabusewithnicotine.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'fivepaneldrugabusewithnicotine Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//        drugsRhs:req.body.data.drugsRhs,
...
exports.deleteCheck=(req,res)=>{
   fivepaneldrugabusewithnicotine.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await fivepaneldrugabusewithnicotine.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await fivepaneldrugabusewithnicotine.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await fivepaneldrugabusewithnicotine.findOne({ _id: req.params.case_id })
