...
const digitalcurrentaddress = require('../../models/data_entry/digitalcurrentaddress.model')
...
if (!req.body.pin) {
                    return res.status(400).json({ message: "pin is Mandatory" })
                }
...
if (!req.body.status) {
    ...
    const obj = new digitalcurrentaddress({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "68b14638b7d780fb41f137ea" : req.body.component,
        checkId: checkId,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await digitalcurrentaddress.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "digitalcurrentaddress" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving digitalcurrentaddress'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    digitalcurrentaddress
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving digitalcurrentaddress for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    digitalcurrentaddress.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    digitalcurrentaddress.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
mov: req.body.mov,
charges: req.body.charges,
pos: req.body.pos,status: req.body.status,
...
exports.findComponentsFor = (req, res) => {
    ...
    digitalcurrentaddress.find(query)
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    digitalcurrentaddress
        ...
        digitalcurrentaddress.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await digitalcurrentaddress.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await digitalcurrentaddress.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return digitalcurrentaddress.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateFeVerificationStatus = (req, res) => {
    digitalcurrentaddress.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {addressRhs: req.body.addressRhs,cityRhs: req.body.cityRhs,stateRhs: req.body.stateRhs,pinRhs: req.body.pinRhs,landmarkRhs: req.body.landmarkRhs,nocRhs: req.body.nocRhs,geolRhs: req.body.geolRhs,geogRhs: req.body.geogRhs,resultRhs: req.body.resultRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,chargesRhs: req.body.chargesRhs,posRhs: req.body.posRhs,
status: req.body.status,
 ...
 return acc;
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await digitalcurrentaddress.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let mailText = subclientNotificationData.template.content;
		                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'digitalcurrentaddress Details'</td><td>${data.address}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.updateOutputqcStatus = (req, res) => {
        digitalcurrentaddress.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {addressRhs: req.body.data.addressRhs,cityRhs: req.body.data.cityRhs,stateRhs: req.body.data.stateRhs,pinRhs: req.body.data.pinRhs,landmarkRhs: req.body.data.landmarkRhs,nocRhs: req.body.data.nocRhs,geolRhs: req.body.data.geolRhs,geogRhs: req.body.data.geogRhs,resultRhs: req.body.data.resultRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,chargesRhs: req.body.data.chargesRhs,posRhs: req.body.data.posRhs,
status: req.body.status,
        outputqcCompletionDate: new Date(),
...
exports.deleteCheck = (req, res) => {
    digitalcurrentaddress.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await digitalcurrentaddress.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await digitalcurrentaddress.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await digitalcurrentaddress.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In digitalcurrentaddress Create for CDE")
...
if (!req.body.pin) {
            res.status(400).json({ message: "pin is Mandatory" })
        }
if (!req.body.landmark) {
...
exports.findOneForCde = (req, res) => {
    console.log("In digitalcurrentaddress for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        digitalcurrentaddress.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
