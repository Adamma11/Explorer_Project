const additionalidb = require('../../models/data_entry/additionalidb.model')
...
if (!req.body.passportid) {
                    return res.status(400).json({ message: "passportid is Mandatory" })
                }
if (!req.body.validfrom) {
                    return res.status(400).json({ message: "validfrom is Mandatory" })
                }
if (!req.body.validtill) {
                    return res.status(400).json({ message: "validtill is Mandatory" })
                }
if (!req.body.status) {
        res.status(400).json({ message: 'Status required' })
    }
    const obj = new additionalidb({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "67220643dec2aeecb2d5b367" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
...
mov: req.body.mov,
            movRhs: null,
civillitigation: req.body.civillitigation,
            civillitigationRhs: null,
bankruptcy: req.body.bankruptcy,
            bankruptcyRhs: null,
mediasearch: req.body.mediasearch,
            mediasearchRhs: null,
globaldatabase: req.body.globaldatabase,
            globaldatabaseRhs: null,
directorship: req.body.directorship,
            directorshipRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await additionalidb.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "additionalidb" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving additionalidb'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    additionalidb
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving additionalidb for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    additionalidb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    additionalidb.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
mediasearch: req.body.mediasearch,
globaldatabase: req.body.globaldatabase,
directorship: req.body.directorship,status: req.body.status,
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    additionalidb
        ...
        additionalidb.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await additionalidb.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await additionalidb.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return additionalidb.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
// passportidRhs: req.body.passportidRhs,
// validfromRhs: req.body.validfromRhs,
// validtillRhs: req.body.validtillRhs,
// verifiedbyRhs: req.body.verifiedbyRhs,
// verifiedonRhs: req.body.verifiedonRhs,
// movRhs: req.body.movRhs,
// civillitigationRhs: req.body.civillitigationRhs,
// bankruptcyRhs: req.body.bankruptcyRhs,
// mediasearchRhs: req.body.mediasearchRhs,
// globaldatabaseRhs: req.body.globaldatabaseRhs,
// directorshipRhs: req.body.directorshipRhs,
// status: req.body.status,
//             verificationCompletionDate: new Date(),
//             grade: req.body.grade,
//             grade1: req.body.grade,
...
exports.updateFeVerificationStatus = (req, res) => {
    additionalidb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,passportidRhs: req.body.passportidRhs,validfromRhs: req.body.validfromRhs,validtillRhs: req.body.validtillRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,civillitigationRhs: req.body.civillitigationRhs,bankruptcyRhs: req.body.bankruptcyRhs,mediasearchRhs: req.body.mediasearchRhs,globaldatabaseRhs: req.body.globaldatabaseRhs,directorshipRhs: req.body.directorshipRhs,
status: req.body.status,
...
exports.updateMentorReviewStatus = async (req, res) => {
    try {
        ...
        if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
            ...
            await additionalidb.findOneAndUpdate(
                { case: caseId, _id: componentId },
                {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'additionalidb Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         additionalidb.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,passportidRhs: req.body.data.passportidRhs,validfromRhs: req.body.data.validfromRhs,validtillRhs: req.body.data.validtillRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,civillitigationRhs: req.body.data.civillitigationRhs,bankruptcyRhs: req.body.data.bankruptcyRhs,mediasearchRhs: req.body.data.mediasearchRhs,globaldatabaseRhs: req.body.data.globaldatabaseRhs,directorshipRhs: req.body.data.directorshipRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'additionalidb Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    additionalidb.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await additionalidb.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await additionalidb.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await additionalidb.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In additionalidb Create for CDE")
...
if (!req.body.passportid) {
            res.status(400).json({ message: "passportid is Mandatory" })
        }
if (!req.body.validfrom) {
            res.status(400).json({ message: "validfrom is Mandatory" })
        }
if (!req.body.validtill) {
...
if (!req.body.civillitigation) {
            res.status(400).json({ message: "civillitigation is Mandatory" })
        }
if (!req.body.bankruptcy) {
            res.status(400).json({ message: "bankruptcy is Mandatory" })
        }
if (!req.body.mediasearch) {
            res.status(400).json({ message: "mediasearch is Mandatory" })
        }
if (!req.body.globaldatabase) {
            res.status(400).json({ message: "globaldatabase is Mandatory" })
        }
if (!req.body.directorship) {
            res.status(400).json({ message: "directorship is Mandatory" })
        }
if (!req.body.status) {
    res.status(400).json({ message: 'Status required' })
}
...
exports.findOneForCde = (req, res) => {
    console.log("In additionalidb for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        additionalidb.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
