const twelvepaneldrugabusetest = require('../../models/data_entry/twelvepaneldrugabusetest.model')
...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   const obj = new twelvepaneldrugabusetest({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
  checkId: checkId,
   component:req.body.component ==null ? "650a9c41c176fe8818612c4e":req.body.component,
       name:req.body.name,
       nameRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"twelvepaneldrugabusetest"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving twelvepaneldrugabusetest'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 twelvepaneldrugabusetest
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving twelvepaneldrugabusetest for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   twelvepaneldrugabusetest.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       drugs:req.body.drugs,
       result:req.body.result,
       verifiedby:req.body.verifiedby,
...
exports.findOne=(req,res)=>{
   twelvepaneldrugabusetest.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    twelvepaneldrugabusetest.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    twelvepaneldrugabusetest.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await twelvepaneldrugabusetest.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await twelvepaneldrugabusetest.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return twelvepaneldrugabusetest.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await twelvepaneldrugabusetest.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'twelvepaneldrugabusetest Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
exports.deleteCheck=(req,res)=>{
   twelvepaneldrugabusetest.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await twelvepaneldrugabusetest.findOne({ _id: req.body.case_id  })
        const caseComments = []
        ...
        const updatedCaseData = await twelvepaneldrugabusetest.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await twelvepaneldrugabusetest.findOne({ _id: req.params.case_id })
