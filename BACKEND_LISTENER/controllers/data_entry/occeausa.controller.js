...
if (!req.body.status) {
    ...
    const obj = new occeausa({
        ...
        component: req.body.component == null ? "6723d8cff072dffc31fe2c63" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await occeausa.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await occeausa.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await occeausa.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return occeausa.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateFeVerificationStatus = (req, res) => {
    occeausa.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,fathernameRhs: req.body.fathernameRhs,dobRhs: req.body.dobRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await occeausa.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'occeausa Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         occeausa.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,fathernameRhs: req.body.data.fathernameRhs,dobRhs: req.body.data.dobRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'occeausa Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await occeausa.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await occeausa.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await occeausa.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In occeausa Create for CDE")
...
exports.findOneForCde = (req, res) => {
    console.log("In occeausa for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        occeausa.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
