...
const fda = require('../../models/data_entry/fda.model')
...
if (!req.body.firstname) {
                    return res.status(400).json({ message: "firstname is Mandatory" })
                }
if (!req.body.middlename) {
                    return res.status(400).json({ message: "middlename is Mandatory" })
                }
if (!req.body.lastname) {
                    return res.status(400).json({ message: "lastname is Mandatory" })
                }
if (!req.body.dob) {
...
if (!req.body.status) {
    ...
    const obj = new fda({
        ...
        component: req.body.component == null ? "6723ca431272f0f57d27dfd9" : req.body.component,
        checkId: checkId,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await fda.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "fda" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving fda'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    fda
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving fda for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    fda.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    fda.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    fda
        ...
        fda.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await fda.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await fda.findOneAndUpdate(
...
//         fda.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
// firstnameRhs: req.body.firstnameRhs,
// middlenameRhs: req.body.middlenameRhs,
// lastnameRhs: req.body.lastnameRhs,
...
exports.updateFeVerificationStatus = (req, res) => {
    fda.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {firstnameRhs: req.body.firstnameRhs,middlenameRhs: req.body.middlenameRhs,lastnameRhs: req.body.lastnameRhs,dobRhs: req.body.dobRhs,fathernameRhs: req.body.fathernameRhs,numberRhs: req.body.numberRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    fda.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        firstnameRhs: req.body.firstnameRhs,middlenameRhs: req.body.middlenameRhs,lastnameRhs: req.body.lastnameRhs,dobRhs: req.body.dobRhs,fathernameRhs: req.body.fathernameRhs,numberRhs: req.body.numberRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,
status: req.body.status,
    ...
    try {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'fda Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         fda.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {firstnameRhs: req.body.data.firstnameRhs,middlenameRhs: req.body.data.middlenameRhs,lastnameRhs: req.body.data.lastnameRhs,dobRhs: req.body.data.dobRhs,fathernameRhs: req.body.data.fathernameRhs,numberRhs: req.body.data.numberRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'fda Details'</td><td>${data.firstname}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    fda.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await fda.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await fda.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await fda.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In fda Create for CDE")
    let caseId = req.caseId
    if (!req.body.case) {
        res.status(400).json({ message: "Case Id is Mandatory" })
    }
if (!req.body.firstname) {
            res.status(400).json({ message: "firstname is Mandatory" })
        }
if (!req.body.middlename) {
            res.status(400).json({ message: "middlename is Mandatory" })
        }
if (!req.body.lastname) {
            res.status(400).json({ message: "lastname is Mandatory" })
        }
if (!req.body.dob) {
...
exports.findOneForCde = (req, res) => {
    console.log("In fda for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        fda.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
