...
const saveenpaneldrugabusetest = require('../../models/data_entry/saveenpaneldrugabusetest.model')
...
if (!req.body.status) {
    ...
    const obj = new saveenpaneldrugabusetest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "672200bfdec2aeecb2d5a510" : req.body.component,
        checkId: checkId,
name: req.body.name,
            nameRhs: null,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await saveenpaneldrugabusetest.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "saveenpaneldrugabusetest" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving saveenpaneldrugabusetest'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    saveenpaneldrugabusetest
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving saveenpaneldrugabusetest for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    saveenpaneldrugabusetest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    saveenpaneldrugabusetest.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
verifiedon: req.body.verifiedon,
mov: req.body.mov,
result: req.body.result,status: req.body.status,
...
exports.findComponentsFor = (req, res) => {
    ...
    saveenpaneldrugabusetest.find(query)
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    saveenpaneldrugabusetest
        ...
        saveenpaneldrugabusetest.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
exports.updateVerificationStatus = async (req, res) => {
  try {
    ...
    const existing = await saveenpaneldrugabusetest.findOne({ case: caseId, _id: componentId });
    ...
    if (req.body.status === 'VERIFICATION-COMPLETED' || req.body.verificationStatus === 'FE-COMPLETED') {
      ...
      const updated = await saveenpaneldrugabusetest.findOneAndUpdate(
    ...
    if (req.body.status === 'DRAFT') {
      return saveenpaneldrugabusetest.findOneAndUpdate(
        { case: caseId, _id: componentId },
        {
          ...updateData,
          stage: 'WIP'
...
exports.updateFeVerificationStatus = (req, res) => {
    saveenpaneldrugabusetest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.nameRhs,drugsRhs: req.body.drugsRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,resultRhs: req.body.resultRhs,
status: req.body.status,
...
exports.updateVendorVerificationStatus = (req, res) => {
    saveenpaneldrugabusetest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,drugsRhs: req.body.drugsRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,resultRhs: req.body.resultRhs,
status: req.body.status,
    ...
    try {
        ...
        //added code nov-07
                ...
                if (req.body.status === "MENTOR-REVIEW-ACCEPTED") {
                    ...
                    await saveenpaneldrugabusetest.findOneAndUpdate(
                        { case: caseId, _id: componentId },
                        {
        ...
        if (updated.grade == "602f8b3743383ec9a722496f") {
            ...
            if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                ...
                let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'saveenpaneldrugabusetest Details'</td><td>${updated.institutename}</td><td>${updated.gradingComments}</td></tr></table>`;
...
//         saveenpaneldrugabusetest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {nameRhs: req.body.data.nameRhs,drugsRhs: req.body.data.drugsRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,resultRhs: req.body.data.resultRhs,
// status: req.body.status,
//         mentorReviewCompletionDate: new Date(),
// 		mentorReviewComments: req.body.mentorReviewComments,
// 		mentorReviewCompletedBy: req.user.user_id
...
//                                     let mailText = subclientNotificationData.template.content
//                                     let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'saveenpaneldrugabusetest Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
//                                     let changedMailText = mailText.replace('&lt;&lt;DISCREPANCY-TABLE&gt;&gt;', checkDetails)
...
exports.deleteCheck = (req, res) => {
    saveenpaneldrugabusetest.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await saveenpaneldrugabusetest.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await saveenpaneldrugabusetest.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await saveenpaneldrugabusetest.findOne({ _id: req.params.case_id })
...
exports.createForCde = (req, res) => {
    console.log("In saveenpaneldrugabusetest Create for CDE")
...
if (!req.body.drugs) {
            res.status(400).json({ message: "drugs is Mandatory" })
        }
...
exports.findOneForCde = (req, res) => {
    console.log("In saveenpaneldrugabusetest for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        saveenpaneldrugabusetest.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
