...
/*
    API REQUEST = {
        ...
        type: "component name or others"
        "componentFields": [{name:"", type:"", values: "", size:200, mandatory: "MANDATORY", label: "", fieldNumber:"",conditionField:"",condition:"",conditionValue:"",lhsRhs:"BOTH"}]
    }
*/
exports.writeComponent = async (req, res) => {
    try {
...
exports.approveInsuff2 = async (req, res) => {
    ...
    try {
        ...
        const ${componentName}Data = await ${componentName}.findOne({ case: req.params.caseId, _id: req.params.componentId })

        if (!${componentName}tData.secondInsufficiencyRaisedDate) {

            const data = await ${componentName}.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        ...
        } else if (!${componentName}Data.thirdInsufficiencyRaisedDate) {
        ...
        } else if (!${componentName}Data.fourthInsufficiencyRaisedDate) {
        ...
        } else if (!${componentName}Data.fifthInsufficiencyRaisedDate) {
...
exports.searchACaseForInsuffForClient = (req, res) => {
    ...
    console.log("In search for insuff case ${componentName}, case id required is ", req.query.caseId)

    Case
        .findOne({ caseId: req.query.caseId })
        .then(caseData => {
            console.log("Case Data found for ${componentName} insuff")
            ...
            UserSubclientAccess.find({ user: req.user.user_id }, { _id: 0, subclient: 1 })
                .then(userSubclientData => {
                    console.log("In ${componentName} insuff user subclient access is ", userSubclientData)
                    ${componentName}
                        ...
                        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
                        .then(data => {
                            console.log("Found the ${componentName} insuff data ", data)
                            res.json(data)
                        })
                        .catch(err => {
                            res.status(500).json({
                                message: "Error searching insuff for ${componentName}"
                            })
                        })
                })
                .catch(err => {
...
exports.approveInsuffClearance = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == "INSUF-2-CLEARED") {
            ...
            if (!${componentName}Data.secondInsufficiencyClearedDate) {
            ...
            } else if (!${componentName}Data.thirdInsufficiencyClearedDate) {
            ...
            } else if (!${componentName}Data.fourthInsufficiencyClearedDate) {
            ...
            } else if (!${componentName}Data.fifthInsufficiencyClearedDate) {
...
exports.reinitiateCheck = (req, res) => {
    ...
    ${componentName}.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        ...
        for (let i = 0; i < componentFields.length; i++) {
            const currField = componentFields[i]
            controller += `\n${currField.name}Rhs: req.body.${currField.name}Rhs,`
        }
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull = (req, res) => {
    ${componentName}
        ...
        .then(data => {
            data.forEach(item => {
                console.log("About to update subclient of ${componentName}", item._id)
                ${componentName}
                    .findOneAndUpdate({ _id: item._id }, { subclient: item.case.subclient._id, client: item.case.subclient.client._id })
                    .then(data => {
                    })
                    .catch(err => {
                        console.log("Error in updating ${componentName} ", err)
                    })
            })
            res.json({ message: "Completed" })
        })
        .catch(err => {
            console.log("Error reading ${componentName}", err)
            res.status(500).json({
...
exports.createForCde = (req, res) => {
    ...
    console.log("Case Id for ${componentName} CDE ", caseId)
    if (caseId != null) {
        const obj = new ${componentName}({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        ...
        console.log("Object for saving created in CDE for ${componentName}")
        if (req.body.status == 'CDE-COMPLETED') {
            obj.dataEntryCompletionDate = new Date()
        } else {
            obj.insufficiencyComments = req.body.insufficiencyComments
        }
        console.log("About to save ${componentName} for CDE")
        obj
            .save(obj)
            .then(data => {
            ...
            .catch(err => {
                console.log("Error Occurred while creating the ${componentName} for CDE ", err)
                res.status(500).json({
                    message: err.message || 'Some error while saving ${componentName}'
                })
            })
...
router.delete("/deleteproofofwork/:caseId/:componentName/:componentId",${componentName}.deleteProofOfWork);
...
router.put("/clearcostapproval/:caseId/:componentId",${componentName}.clearCostApproval);
...
router.get("/insuff/searchacaseininsuffforclient",${componentName}.searchACaseForInsuffForClient);
