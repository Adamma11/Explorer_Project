...
if(!req.body.case){
   ...
   if(!req.body.doj){
       res.status(400).json({message:'Date of Joining required'})
   }
   if(!req.body.dor){
       res.status(400).json({message:'Date of Reliving required'})
   }
   if(!req.body.designation){
       res.status(400).json({message:'Designation required'})
   }
   if(!req.body.remuneration){
       res.status(400).json({message:'Remuneration required'})
   }
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "65098b23c176fe88186124ea":req.body.component,
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await employment.findOne({ _id: req.params._id })
