...
if(!req.body.case){
   ...
   if(!req.body.address){
       res.status(400).json({message:'Permanent Address required'})
   }
   if(!req.body.pos){
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "650a8c72c176fe88186129de":req.body.component,
       name:req.body.name,
       nameRhs:null,
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await criminalcourtrecordcheckpermanentaddressrecordcheckpermanentaddress.findOne({ _id: req.body._id })
        const caseComments = []
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await criminalcourtrecordcheckpermanentaddress.findOne({ _id: req.params._id })
