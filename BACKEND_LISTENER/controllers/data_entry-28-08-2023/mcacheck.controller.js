const mcacheck = require('../../models/data_entry/mcacheck.model')
...
if(!req.body.case){
   ...
   const obj = new mcacheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645de5a5da4885b59f88fae7":req.body.component,
       companyname:req.body.companyname,
       companynameRhs:null,
       mcafoundforthecompany:req.body.mcafoundforthecompany,
       mcafoundforthecompanyRhs:null,
       ifmcafoundstatusonmca:req.body.ifmcafoundstatusonmca,
       ifmcafoundstatusonmcaRhs:null,
       dateofincorporation:req.body.dateofincorporation,
       dateofincorporationRhs:null,
       alastbalancesheetfiledddressaspermca:req.body.alastbalancesheetfiledddressaspermca,
       alastbalancesheetfiledddressaspermcaRhs:null,
       addressaspermca:req.body.addressaspermca,
       addressaspermcaRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"mcacheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving mcacheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving mcacheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   mcacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       companyname:req.body.companyname,
       mcafoundforthecompany:req.body.mcafoundforthecompany,
       ifmcafoundstatusonmca:req.body.ifmcafoundstatusonmca,
       dateofincorporation:req.body.dateofincorporation,
...
exports.findOne=(req,res)=>{
   mcacheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    mcacheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     mcacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       ifmcafoundstatusonmcaRhs:req.body.ifmcafoundstatusonmcaRhs,
       dateofincorporationRhs:req.body.dateofincorporationRhs,
       alastbalancesheetfiledddressaspermcaRhs:req.body.alastbalancesheetfiledddressaspermcaRhs,
       addressaspermcaRhs:req.body.addressaspermcaRhs,
...
exports.deleteCheck=(req,res)=>{
   mcacheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
