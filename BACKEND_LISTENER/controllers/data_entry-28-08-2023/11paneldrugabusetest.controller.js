const 11paneldrugabusetest = require('../../models/data_entry/11paneldrugabusetest.model')
...
if(!req.body.case){
   ...
   const obj = new 11paneldrugabusetest({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving 11paneldrugabusetest'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 11paneldrugabusetest
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving 11paneldrugabusetest for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   11paneldrugabusetest.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       results:req.body.results,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   11paneldrugabusetest.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    11paneldrugabusetest.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    11paneldrugabusetest.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   11paneldrugabusetest.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
