const 4paneldrugabusecheck = require('../../models/data_entry/4paneldrugabusecheck.model')
...
if(!req.body.case){
   ...
   const obj = new 4paneldrugabusecheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0617da4885b59f88fed8" :req.body.component,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving 4paneldrugabusecheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 4paneldrugabusecheck
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving 4paneldrugabusecheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   4paneldrugabusecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   4paneldrugabusecheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.updateInputqcStatus=(req,res)=>{
 if(req.body.status=='INPUTQC-ACCEPTED'){
     4paneldrugabusecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         drugschecked:req.body.componentData.drugschecked,
       result:req.body.componentData.result,
	 status:req.body.status,
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    4paneldrugabusecheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    4paneldrugabusecheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   4paneldrugabusecheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
