...
if(!req.body.case){
   ...
   if(!req.body.dinnumber){
       res.status(400).json({message:'DIN Number required'})
   }
   if(!req.body.directorshipstatus){
       res.status(400).json({message:'Directorship Status required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new directorshipcheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e006fda4885b59f88fd19":req.body.component,
       name:req.body.name,
       nameRhs:null,
       ...
       anylegalobligationsfound:req.body.anylegalobligationsfound,
       anylegalobligationsfoundRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"directorshipcheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving directorshipcheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving directorshipcheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   directorshipcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       dateofbirth:req.body.dateofbirth,
       fathersname:req.body.fathersname,
       dinnumber:req.body.dinnumber,
...
exports.findOne=(req,res)=>{
   directorshipcheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    directorshipcheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     directorshipcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       anylegalobligationsfoundRhs:req.body.anylegalobligationsfoundRhs,
...
exports.deleteCheck=(req,res)=>{
   directorshipcheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
