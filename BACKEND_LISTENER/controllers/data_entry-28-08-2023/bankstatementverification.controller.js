const bankstatementverification = require('../../models/data_entry/bankstatementverification.model')
...
if(!req.body.case){
   ...
   if(!req.body.nameofthecandidate){
       res.status(400).json({message:'Name of the Candidate required'})
   }
   ...
   if(!req.body.accountnumber){
       res.status(400).json({message:'Account Number required'})
   }
   if(!req.body.bankname){
       res.status(400).json({message:'Bank Name required'})
   }
   if(!req.body.bankbranchlocation){
       res.status(400).json({message:'Bank Branch Location required'})
   }
   if(!req.body.statementstartdate){
       res.status(400).json({message:'Statement Start Date required'})
   }
   if(!req.body.statementenddate){
       res.status(400).json({message:'Statement End Date required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new bankstatementverification({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645de9feda4885b59f88fbc6":req.body.component,
       nameofthecandidate:req.body.nameofthecandidate,
       nameofthecandidateRhs:null,
       fathersname:req.body.fathersname,
       fathersnameRhs:null,
       dateofbirth:req.body.dateofbirth,
       dateofbirthRhs:null,
       accountnumber:req.body.accountnumber,
       accountnumberRhs:null,
       bankname:req.body.bankname,
       banknameRhs:null,
       bankbranchlocation:req.body.bankbranchlocation,
       bankbranchlocationRhs:null,
       statementstartdate:req.body.statementstartdate,
       statementstartdateRhs:null,
       statementenddate:req.body.statementenddate,
       statementenddateRhs:null,
       isthesalarycredited:req.body.isthesalarycredited,
       isthesalarycreditedRhs:null,
       salarycreditdate:req.body.salarycreditdate,
       salarycreditdateRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"bankstatementverification"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving bankstatementverification'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving bankstatementverification for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   bankstatementverification.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameofthecandidate:req.body.nameofthecandidate,
       fathersname:req.body.fathersname,
       dateofbirth:req.body.dateofbirth,
       accountnumber:req.body.accountnumber,
...
exports.findOne=(req,res)=>{
   bankstatementverification.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    bankstatementverification.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    bankstatementverification.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     bankstatementverification.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameofthecandidateRhs:req.body.nameofthecandidateRhs,
       fathersnameRhs:req.body.fathersnameRhs,
       dateofbirthRhs:req.body.dateofbirthRhs,
       accountnumberRhs:req.body.accountnumberRhs,
       banknameRhs:req.body.banknameRhs,
       bankbranchlocationRhs:req.body.bankbranchlocationRhs,
       statementstartdateRhs:req.body.statementstartdateRhs,
       statementenddateRhs:req.body.statementenddateRhs,
       isthesalarycreditedRhs:req.body.isthesalarycreditedRhs,
       salarycreditdateRhs:req.body.salarycreditdateRhs,
...
exports.deleteCheck=(req,res)=>{
   bankstatementverification.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
