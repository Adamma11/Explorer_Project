const msmeudyamcheck = require('../../models/data_entry/msmeudyamcheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.nameofenterprise){
       res.status(400).json({message:'Name of Enterprise required'})
   }
   if(!req.body.enter12digituamno){
       res.status(400).json({message:'Enter 12 digit UAM No required'})
   }
   if(!req.body.majoractivity){
       res.status(400).json({message:'Major Activity required'})
   }
   if(!req.body.socialcategory){
       res.status(400).json({message:'Social Category required'})
   }
   if(!req.body.nictwodigit){
       res.status(400).json({message:'NIC Two Digit required'})
   }
   if(!req.body.nicfourdigit){
       res.status(400).json({message:'NIC Four Digit required'})
   }
   if(!req.body.nicfivedigitcode){
       res.status(400).json({message:'NIC Five Digit Code required'})
   }
   if(!req.body.activitytype){
       res.status(400).json({message:'Activity Type required'})
   }
   if(!req.body.addedtomsmeon){
       res.status(400).json({message:'Added to MSME On required'})
   }
   if(!req.body.dateofcommencement){
       res.status(400).json({message:'Date of commencement required'})
   }
   if(!req.body.dicname){
       res.status(400).json({message:'DIC Name required'})
   }
   if(!req.body.state){
       res.status(400).json({message:'State required'})
   }
   if(!req.body.applieddate){
       res.status(400).json({message:'Applied Date required'})
   }
   if(!req.body.certificateisvalidtill){
       res.status(400).json({message:'Certificate is Valid Till required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new msmeudyamcheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645df2d4da4885b59f88fc15":req.body.component,
       nameofenterprise:req.body.nameofenterprise,
       nameofenterpriseRhs:null,
       enter12digituamno:req.body.enter12digituamno,
       enter12digituamnoRhs:null,
       majoractivity:req.body.majoractivity,
       majoractivityRhs:null,
       socialcategory:req.body.socialcategory,
       socialcategoryRhs:null,
       nictwodigit:req.body.nictwodigit,
       nictwodigitRhs:null,
       nicfourdigit:req.body.nicfourdigit,
       nicfourdigitRhs:null,
       nicfivedigitcode:req.body.nicfivedigitcode,
       nicfivedigitcodeRhs:null,
       ...
       applieddateRhs:null,
       certificateisvalidtill:req.body.certificateisvalidtill,
       certificateisvalidtillRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"msmeudyamcheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving msmeudyamcheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving msmeudyamcheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   msmeudyamcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameofenterprise:req.body.nameofenterprise,
       enter12digituamno:req.body.enter12digituamno,
       majoractivity:req.body.majoractivity,
       socialcategory:req.body.socialcategory,
...
exports.findOne=(req,res)=>{
   msmeudyamcheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    msmeudyamcheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     msmeudyamcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       enter12digituamnoRhs:req.body.enter12digituamnoRhs,
       majoractivityRhs:req.body.majoractivityRhs,
       socialcategoryRhs:req.body.socialcategoryRhs,
       nictwodigitRhs:req.body.nictwodigitRhs,
       nicfourdigitRhs:req.body.nicfourdigitRhs,
       nicfivedigitcodeRhs:req.body.nicfivedigitcodeRhs,
       ...
       certificateisvalidtillRhs:req.body.certificateisvalidtillRhs,
         status:req.body.status,
...
exports.deleteCheck=(req,res)=>{
   msmeudyamcheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
