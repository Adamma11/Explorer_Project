const cpcpoliceverification = require('../../models/data_entry/cpcpoliceverification.model')
...
if(!req.body.case){
   ...
   if(!req.body.dateofliasoningwithpolicestatus){
       res.status(400).json({message:'Date of Liasoning with Police Status required'})
   }
   if(!req.body.dateofverification){
       res.status(400).json({message:'Date of Verification required'})
   }
   if(!req.body.policestation){
       res.status(400).json({message:'Police Station required'})
   }
   if(!req.body.policestationcontactdetails){
       res.status(400).json({message:'Police Station - Contact Details required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new cpcpoliceverification({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e10ebda4885b59f8900af":req.body.component,
       name:req.body.name,
       nameRhs:null,
       ...
       addressRhs:null,
       dateofliasoningwithpolicestatus:req.body.dateofliasoningwithpolicestatus,
       dateofliasoningwithpolicestatusRhs:null,
       dateofverification:req.body.dateofverification,
       dateofverificationRhs:null,
       policestation:req.body.policestation,
       policestationRhs:null,
       policestationcontactdetails:req.body.policestationcontactdetails,
       policestationcontactdetailsRhs:null,
       numberofyearscoveredintheverification:req.body.numberofyearscoveredintheverification,
       numberofyearscoveredintheverificationRhs:null,
       verifiedby:req.body.verifiedby,
       verifiedbyRhs:null,
       cpcresult:req.body.cpcresult,
       cpcresultRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"cpcpoliceverification"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving cpcpoliceverification'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 cpcpoliceverification
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving cpcpoliceverification for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   cpcpoliceverification.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       dateofbirth:req.body.dateofbirth,
       fathersname:req.body.fathersname,
       address:req.body.address,
...
exports.findOne=(req,res)=>{
   cpcpoliceverification.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    cpcpoliceverification.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    cpcpoliceverification.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     cpcpoliceverification.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       dateofliasoningwithpolicestatusRhs:req.body.dateofliasoningwithpolicestatusRhs,
       dateofverificationRhs:req.body.dateofverificationRhs,
       policestationRhs:req.body.policestationRhs,
       policestationcontactdetailsRhs:req.body.policestationcontactdetailsRhs,
       numberofyearscoveredintheverificationRhs:req.body.numberofyearscoveredintheverificationRhs,
       verifiedbyRhs:req.body.verifiedbyRhs,
       cpcresultRhs:req.body.cpcresultRhs,
...
exports.deleteCheck=(req,res)=>{
   cpcpoliceverification.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
