...
if(!req.body.case){
   ...
   if(!req.body.officeaddressfound){
       res.status(400).json({message:'Office Address Found ? required'})
   }
   if(!req.body.doescompanysignboarddisplayed){
       res.status(400).json({message:'Does Company Sign Board Displayed? required'})
   }
   if(!req.body.wasfeallowedtoenterofficepremises){
       res.status(400).json({message:'Was FE Allowed to Enter Office Premises? required'})
   }
   if(!req.body.ifyesprovidetypeofofficelocation){
       res.status(400).json({message:'If Yes, Provide type of Office Location required'})
   }
   if(!req.body.companysizeintermsofemployeecount){
       res.status(400).json({message:'Company Size in terms of Employee Count required'})
   }
   if(!req.body.companyexistingsince){
       res.status(400).json({message:'Company Existing Since ? required'})
   }
   if(!req.body.nameoftheindividualmetduringthevisit){
       res.status(400).json({message:'Name of the Individual met during the visit required'})
   }
   if(!req.body.contactnumberoftheindividualmetduringthevisit){
       res.status(400).json({message:'Contact Number of the Individual met during the visit required'})
   }
   if(!req.body.designationoftheindividualprovideddetails){
       res.status(400).json({message:'Designation of the Individual Provided Details required'})
   }
   if(!req.body.designationoftheindividualwhoprovideddetails){
       res.status(400).json({message:'Designation of the Individual who provided Details required'})
   }
   if(!req.body.officelocationcity){
       res.status(400).json({message:'Office Location - City required'})
   }
   if(!req.body.pinzipcode){
       res.status(400).json({message:'PIN / Zipcode required'})
   }
   if(!req.body.country){
       res.status(400).json({message:'Country required'})
   }
   if(!req.body.landmark){
       res.status(400).json({message:'Land Mark required'})
   }
   if(!req.body.state){
       res.status(400).json({message:'State required'})
   }
   if(!req.body.status){
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e12f1da4885b59f8900f6":req.body.component,
       officeaddressfound:req.body.officeaddressfound,
       officeaddressfoundRhs:null,
       doescompanysignboarddisplayed:req.body.doescompanysignboarddisplayed,
       doescompanysignboarddisplayedRhs:null,
       wasfeallowedtoenterofficepremises:req.body.wasfeallowedtoenterofficepremises,
       wasfeallowedtoenterofficepremisesRhs:null,
       ifyesprovidetypeofofficelocation:req.body.ifyesprovidetypeofofficelocation,
       ifyesprovidetypeofofficelocationRhs:null,
       companysizeintermsofemployeecount:req.body.companysizeintermsofemployeecount,
       companysizeintermsofemployeecountRhs:null,
       companyexistingsince:req.body.companyexistingsince,
       companyexistingsinceRhs:null,
       nameoftheindividualmetduringthevisit:req.body.nameoftheindividualmetduringthevisit,
       nameoftheindividualmetduringthevisitRhs:null,
       contactnumberoftheindividualmetduringthevisit:req.body.contactnumberoftheindividualmetduringthevisit,
       contactnumberoftheindividualmetduringthevisitRhs:null,
       designationoftheindividualprovideddetails:req.body.designationoftheindividualprovideddetails,
       designationoftheindividualprovideddetailsRhs:null,
       ...
       officelocationcity:req.body.officelocationcity,
       officelocationcityRhs:null,
       pinzipcode:req.body.pinzipcode,
       pinzipcodeRhs:null,
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     companyverificationphysical.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       officeaddressfoundRhs:req.body.officeaddressfoundRhs,
       doescompanysignboarddisplayedRhs:req.body.doescompanysignboarddisplayedRhs,
       wasfeallowedtoenterofficepremisesRhs:req.body.wasfeallowedtoenterofficepremisesRhs,
       ifyesprovidetypeofofficelocationRhs:req.body.ifyesprovidetypeofofficelocationRhs,
       companysizeintermsofemployeecountRhs:req.body.companysizeintermsofemployeecountRhs,
       companyexistingsinceRhs:req.body.companyexistingsinceRhs,
       nameoftheindividualmetduringthevisitRhs:req.body.nameoftheindividualmetduringthevisitRhs,
       contactnumberoftheindividualmetduringthevisitRhs:req.body.contactnumberoftheindividualmetduringthevisitRhs,
       designationoftheindividualprovideddetailsRhs:req.body.designationoftheindividualprovideddetailsRhs,
       designationoftheindividualwhoprovideddetailsRhs:req.body.designationoftheindividualwhoprovideddetailsRhs,
       officelocationcityRhs:req.body.officelocationcityRhs,
       pinzipcodeRhs:req.body.pinzipcodeRhs,
