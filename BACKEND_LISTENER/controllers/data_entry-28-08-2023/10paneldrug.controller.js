const 10paneldrug = require('../../models/data_entry/10paneldrug.model')
...
if(!req.body.case){
   ...
   const obj = new 10paneldrug({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e058bda4885b59f88febc" :req.body.component,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving 10paneldrug'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving 10paneldrug for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   10paneldrug.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   10paneldrug.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    10paneldrug.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   10paneldrug.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
