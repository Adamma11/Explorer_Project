const socialmediacheck = require('../../models/data_entry/socialmediacheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.racismandordemonstrationsofintolerance){
       res.status(400).json({message:'Racism and or Demonstrations of Intolerance required'})
   }
   if(!req.body.potentiallyviolentbehaviour){
       res.status(400).json({message:'Potentially violent behaviour required'})
   }
   if(!req.body.potentiallyunlawfulactivity){
       res.status(400).json({message:'Potentially Unlawful Activity required'})
   }
   if(!req.body.sexuallyexplicitmaterial){
       res.status(400).json({message:'Sexually Explicit Material required'})
   }
   if(!req.body.displayintoleranceoverreligiousbeliefs){
       res.status(400).json({message:'Display intolerance over Religious beliefs required'})
   }
   ...
   const obj = new socialmediacheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0e71da4885b59f89005d":req.body.component,
       racismandordemonstrationsofintolerance:req.body.racismandordemonstrationsofintolerance,
       racismandordemonstrationsofintoleranceRhs:null,
       potentiallyviolentbehaviour:req.body.potentiallyviolentbehaviour,
       potentiallyviolentbehaviourRhs:null,
       potentiallyunlawfulactivity:req.body.potentiallyunlawfulactivity,
       potentiallyunlawfulactivityRhs:null,
       sexuallyexplicitmaterial:req.body.sexuallyexplicitmaterial,
       sexuallyexplicitmaterialRhs:null,
       displayintoleranceoverreligiousbeliefs:req.body.displayintoleranceoverreligiousbeliefs,
       displayintoleranceoverreligiousbeliefsRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"socialmediacheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving socialmediacheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving socialmediacheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   socialmediacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       racismandordemonstrationsofintolerance:req.body.racismandordemonstrationsofintolerance,
       potentiallyviolentbehaviour:req.body.potentiallyviolentbehaviour,
       potentiallyunlawfulactivity:req.body.potentiallyunlawfulactivity,
       sexuallyexplicitmaterial:req.body.sexuallyexplicitmaterial,
...
exports.findOne=(req,res)=>{
   socialmediacheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    socialmediacheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     socialmediacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       racismandordemonstrationsofintoleranceRhs:req.body.racismandordemonstrationsofintoleranceRhs,
       potentiallyviolentbehaviourRhs:req.body.potentiallyviolentbehaviourRhs,
       potentiallyunlawfulactivityRhs:req.body.potentiallyunlawfulactivityRhs,
       sexuallyexplicitmaterialRhs:req.body.sexuallyexplicitmaterialRhs,
       displayintoleranceoverreligiousbeliefsRhs:req.body.displayintoleranceoverreligiousbeliefsRhs,
...
exports.deleteCheck=(req,res)=>{
   socialmediacheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
