const resumecheck = require('../../models/data_entry/resumecheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.education){
       res.status(400).json({message:'Education required'})
   }
   if(!req.body.employment){
       res.status(400).json({message:'Employment required'})
   }
   if(!req.body.searchresultsfromjobportals){
       res.status(400).json({message:'Search Results From Job Portals required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new resumecheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0effda4885b59f89006f":req.body.component,
       education:req.body.education,
       educationRhs:null,
       employment:req.body.employment,
       employmentRhs:null,
       searchresultsfromjobportals:req.body.searchresultsfromjobportals,
       searchresultsfromjobportalsRhs:null,
       finalverificationremarks:req.body.finalverificationremarks,
       finalverificationremarksRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"resumecheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving resumecheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving resumecheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   resumecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       education:req.body.education,
       employment:req.body.employment,
       searchresultsfromjobportals:req.body.searchresultsfromjobportals,
       finalverificationremarks:req.body.finalverificationremarks,
...
exports.findOne=(req,res)=>{
   resumecheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    resumecheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     resumecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       searchresultsfromjobportalsRhs:req.body.searchresultsfromjobportalsRhs,
       finalverificationremarksRhs:req.body.finalverificationremarksRhs,
         status:req.body.status,
...
exports.deleteCheck=(req,res)=>{
   resumecheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
