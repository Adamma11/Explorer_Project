const tenpaneldrug = require('../../models/data_entry/tenpaneldrug.model')
...
if(!req.body.case){
  ...
  personalDetailsData:req.body.personalDetails,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"tenpaneldrug"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving tenpaneldrug'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving tenpaneldrug for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   tenpaneldrug.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   tenpaneldrug.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    tenpaneldrug.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   tenpaneldrug.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
