const fivepaneldrug = require('../../models/data_entry/fivepaneldrug.model')
...
if(!req.body.case){
  ...
  personalDetailsData:req.body.personalDetails,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"fivepaneldrug"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving fivepaneldrug'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving fivepaneldrug for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   fivepaneldrug.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   fivepaneldrug.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    fivepaneldrug.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   fivepaneldrug.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
