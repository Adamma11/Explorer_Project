const sevenpaneldrugabusetest = require('../../models/data_entry/sevenpaneldrugabusetest.model')
...
if(!req.body.case){
   ...
   const obj = new sevenpaneldrugabusetest({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"sevenpaneldrugabusetest"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving sevenpaneldrugabusetest'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 sevenpaneldrugabusetest
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving sevenpaneldrugabusetest for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   sevenpaneldrugabusetest.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   sevenpaneldrugabusetest.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    sevenpaneldrugabusetest.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    sevenpaneldrugabusetest.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   sevenpaneldrugabusetest.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
