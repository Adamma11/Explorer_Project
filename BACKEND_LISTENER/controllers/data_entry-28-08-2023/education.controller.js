...
if(!req.body.case){
   ...
   if(!req.body.institute){
       res.status(400).json({message:'Institute required'})
   }
   if(!req.body.university){
       res.status(400).json({message:'University required'})
   }
   if(!req.body.city){
   ...
   if(!req.body.degree){
       res.status(400).json({message:'Degree required'})
   }
   if(!req.body.registrationnumber){
       res.status(400).json({message:'Registration Number required'})
   }
   if(!req.body.yearofpassing){
       res.status(400).json({message:'Year Of Passing required'})
   }
   if(!req.body.status){
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645dead2da4885b59f88fbe2":req.body.component,
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     education.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       instituteRhs:req.body.instituteRhs,
       universityRhs:req.body.universityRhs,
