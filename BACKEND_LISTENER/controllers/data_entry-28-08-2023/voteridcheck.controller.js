const voteridcheck = require('../../models/data_entry/voteridcheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.districtconstituency){
       res.status(400).json({message:'District constituency required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new voteridcheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0174da4885b59f88fdcc":req.body.component,
       name:req.body.name,
       nameRhs:null,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving voteridcheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving voteridcheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   voteridcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       name:req.body.name,
       dateofbirth:req.body.dateofbirth,
       gender:req.body.gender,
       state:req.body.state,
...
exports.findOne=(req,res)=>{
   voteridcheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    voteridcheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   voteridcheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
