...
if(!req.body.case){
   ...
   if(!req.body.emergencycontactnumbertwo){
       res.status(400).json({message:'Emergency Contact Number 2 required'})
   }
   if(!req.body.relationshipemergencycontactnumbertwo){
       res.status(400).json({message:'Relationship (Emergency Contact Number 2) required'})
   }
   if(!req.body.dateofcontact){
       res.status(400).json({message:'Date of Contact required'})
   }
   if(!req.body.modeofverification){
       res.status(400).json({message:'Mode of Verification required'})
   }
   if(!req.body.remarks){
       res.status(400).json({message:'Remarks required'})
   }
   if(!req.body.status){
