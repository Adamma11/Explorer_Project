...
if(!req.body.case){
   ...
   if(!req.body.refereedetails){
       res.status(400).json({message:'Referee details required'})
   }
   if(!req.body.contactdetails){
       res.status(400).json({message:'Contact details required'})
   }
   if(!req.body.relationshipwithreferee){
       res.status(400).json({message:'Relationship with Referee required'})
   }
   if(!req.body.knownrefereesince){
       res.status(400).json({message:'Known Referee Since? required'})
   }
   if(!req.body.status){
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e08a6da4885b59f88ff68":req.body.component,
       refereedetails:req.body.refereedetails,
       refereedetailsRhs:null,
       contactdetails:req.body.contactdetails,
       contactdetailsRhs:null,
       relationshipwithreferee:req.body.relationshipwithreferee,
       relationshipwithrefereeRhs:null,
       knownrefereesince:req.body.knownrefereesince,
       knownrefereesinceRhs:null,
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     personalreference.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       contactdetailsRhs:req.body.contactdetailsRhs,
       relationshipwithrefereeRhs:req.body.relationshipwithrefereeRhs,
       knownrefereesinceRhs:req.body.knownrefereesinceRhs,
         status:req.body.status,
