const fcafinancialconductauthoritydatabasecheck = require('../../models/data_entry/fcafinancialconductauthoritydatabasecheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.firstname){
       res.status(400).json({message:'First Name As per Passport or ID proof submitted required'})
   }
   if(!req.body.middlename){
       res.status(400).json({message:'Middle Name As per Passport or ID proof submitted required'})
   }
   if(!req.body.lastname){
       res.status(400).json({message:'Last Name As per Passport or ID proof submitted required'})
   }
   ...
   const obj = new fcafinancialconductauthoritydatabasecheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0ca0da4885b59f88fff7":req.body.component,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"fcafinancialconductauthoritydatabasecheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving fcafinancialconductauthoritydatabasecheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 fcafinancialconductauthoritydatabasecheck
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving fcafinancialconductauthoritydatabasecheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   fcafinancialconductauthoritydatabasecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       firstname:req.body.firstname,
       middlename:req.body.middlename,
       lastname:req.body.lastname,
       dateofbirth:req.body.dateofbirth,
...
exports.findOne=(req,res)=>{
   fcafinancialconductauthoritydatabasecheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    fcafinancialconductauthoritydatabasecheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    fcafinancialconductauthoritydatabasecheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   fcafinancialconductauthoritydatabasecheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
