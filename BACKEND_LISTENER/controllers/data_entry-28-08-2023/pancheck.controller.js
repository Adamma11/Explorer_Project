const pancheck = require('../../models/data_entry/pancheck.model')
...
if(!req.body.case){
   ...
   const obj = new pancheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e00cada4885b59f88fd2d":req.body.component,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"pancheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving pancheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving pancheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   pancheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameasperpancard:req.body.nameasperpancard,
       fathersnameasperpancard:req.body.fathersnameasperpancard,
       dateofbirth:req.body.dateofbirth,
       remarks:req.body.remarks,
...
exports.findOne=(req,res)=>{
   pancheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    pancheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   pancheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
