...
if(!req.body.case){
   ...
   if(!req.body.address){
       res.status(400).json({message:'Full Address with PIN required'})
   }
   if(!req.body.pin){
       res.status(400).json({message:'PIN required'})
   }
   if(!req.body.city){
       res.status(400).json({message:'City required'})
   }
   if(!req.body.typeofaddress){
       res.status(400).json({message:'Type of Address required'})
   }
   if(!req.body.tenureofstay){
       res.status(400).json({message:'Tenure of Stay (mmm-yy to mmm-yy) required'})
   }
   if(!req.body.primarycontact){
       res.status(400).json({message:'Primary Contact Number required'})
   }
   if(!req.body.status){
  ...
  personalDetailsData:req.body.personalDetails,
   component:req.body.component == null ? "602f71a5767164c5698f4e36":req.body.component,
       ...
       typeofaddressRhs:null,
       tenureofstay:req.body.tenureofstay,
       tenureofstayRhs:null,
       primarycontact:req.body.primarycontact,
       primarycontactRhs:null,
       alternatecontact:req.body.alternatecontact,
       alternatecontactRhs:null,
       typeofstay:req.body.typeofstay,
       typeofstayRhs:null,
       nameofrespondent:req.body.nameofrespondent,
       nameofrespondentRhs:null,
       relationship:req.body.relationship,
       relationshipRhs:null,
   ...
   .then(data=>{
       ...
       Case
       .update({_id:req.body.case},{$push:{actualComponents:"address"}})
       .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving address'
       })
   })
};
...
//13-jan-23
exports.downloadProofOfWorkAsJson = (req,res)=>{
    console.log("In Download Proof of work as json")
    try {

      const data = fs.readFileSync('/REPO_STORAGE/case_uploads/'+req.params.caseId +'/' + req.params.componentName+ '/' + req.params.componentId + '/proofofwork/' + req.query.fileName + '.file', 'utf8');

      res.json({type:'base64',data:data});
...
if(!req.body.case){
   ...
   address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       address:req.body.address,
       landmark:req.body.landmark,
       pin:req.body.pin,
       city:req.body.city,
   ...
   .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Updating Check',req.body.status,'Check Created',null,null,null,req.user.user_id)
       res.json(data)
   })
   .catch(err=>{
       res.status(500).json({
...
exports.findOne=(req,res)=>{
   address.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.updateDataEntryStatus=(req,res)=>{
 if(req.body.status=='DE-COMPLETED'){
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
                        caseHistory.create(req.body.case,req.body.component,data._id,'Updating Check',req.body.status,'Check Updated',null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.updateInputqcStatus=(req,res)=>{
 if(req.body.status=='INPUTQC-ACCEPTED'){
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         address:req.body.componentData.address,
       landmark:req.body.componentData.landmark,
       pin:req.body.componentData.pin,
       city:req.body.componentData.city,
       typeofaddress:req.body.componentData.typeofaddress,
       tenureofstay:req.body.componentData.tenureofstay,
       primarycontact:req.body.componentData.primarycontact,
       alternatecontact:req.body.componentData.alternatecontact,
       typeofstay:req.body.componentData.typeofstay,
       nameofrespondent:req.body.componentData.nameofrespondent,
       relationship:req.body.componentData.relationship,
       contact:req.body.componentData.contact,
       dateofvisit:req.body.componentData.dateofvisit,
       geotag:req.body.componentData.geotag,
	 status:req.body.status,
         inputqcCompletionDate:new Date(),
         branchAllocatedTo:req.body.branch,
         })
         .then(data=>{
                          caseHistory.create(req.body.case,req.body.component,data._id,'Upadint Input QC Status',req.body.status,'Input QC Status Updated',null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
     ...
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .catch(err=>{
                         caseHistory.create(req.body.case,req.body.component,data._id,'Input QC Updation',req.body.status,req.body.inputqcComments,null,null,null,req.user.user_id)
             res.status(500).json({
                 message:err.message | "Error occurred while updating status during input qc"
             })
         })
...
exports.findComponentsFor=(req,res)=>{
 ...
 console.log("Number of CPUs are............................................................................................",numCPUs)	
 let query;
 ...
 if(req.params.for=='VENDOR-VERIFIED'){
     query={$or:[{status:'VENDOR-COMPLETED'},{status:"VENDOR-INSUF"},{status:'VENDOR-COULD-NOT-VERIFY'},{status:"VENDOR-VERIFIED"}]}
 }
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    address.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       landmarkRhs:req.body.landmarkRhs,
       pinRhs:req.body.pinRhs,
       cityRhs:req.body.cityRhs,
       typeofaddressRhs:req.body.typeofaddressRhs,
       tenureofstayRhs:req.body.tenureofstayRhs,
       primarycontactRhs:req.body.primarycontactRhs,
       alternatecontactRhs:req.body.alternatecontactRhs,
       typeofstayRhs:req.body.typeofstayRhs,
       nameofrespondentRhs:req.body.nameofrespondentRhs,
       relationshipRhs:req.body.relationshipRhs,
       contactRhs:req.body.contactRhs,
       dateofvisitRhs:req.body.dateofvisitRhs,
       geotagRhs:req.body.geotagRhs,
         status:req.body.status,
	 ...
	 grade1:req.body.grade,    
         ...
         .then(data=>{
            caseHistory.create(req.body.case,req.body.component,data._id,'Verification Completed',req.body.status,req.body.gradingComments,null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
     ...
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'Insuff 2 Raised',req.body.status,req.body.insufficiencyComments,null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.updateFeVerificationStatus=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       geotagRhs:req.body.geotagRhs,
         ...
         .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'FE Verification Completion',req.body.status,req.body.status == 'FE-INSUF'? req.body.feInsufficiencyComments:"Completed by FE" ,null,null,null,req.user.user_id)
            console.log("Directory to be created for the case ",req.body.caseId)
            console.log("For the check ",req.body._id)
            console.log("Number of photographs to write",req.body.photographs.length)
            if(!fs.existsSync('/REPO_STORAGE/case_uploads/'+req.body.caseId +'/address/' + req.body._id+ '/proofofwork')){
                console.log("directory does not exist, will create")
                fs.mkdirSync('/REPO_STORAGE/case_uploads/' +req.body.caseId +'/address/' +req.body._id + '/proofofwork',{recursive:true})
            }
            console.log("Number of photographs present ",req.body.photographs.length)
            if(req.body.photographs){
               for(let i=0; i < req.body.photographs.length;i++){
                  let photo = req.body.photographs[i]
                  fs.writeFile(`/REPO_STORAGE/case_uploads/${req.body.caseId}/address/${req.body._id}/proofofwork/photo${i}.file`,photo,err=>{
                     if(err){
...
exports.updateVendorVerificationStatus=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       geotagRhs:req.body.geotagRhs,
         ...
         .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'Vendor Verification Completion',req.body.status,req.body.status == 'VENDOR-INSUF'? req.body.vendorInsufficiencyComments:"Completed by FE" ,null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.updateVerifierReviewStatus=(req,res)=>{
	 ...
	 grade1:req.body.grade,    
         ...
         .then(data=>{
            caseHistory.create(req.body.case,req.body.component,data._id,'Verification Status Update',req.body.status,req.body.status == 'VERIFICATION-COMPLETED'? req.body.gradingComments:req.body.verifierReviewComments ,null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.updateMentorReviewStatus=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
                        caseHistory.create(req.body.case,req.body.component,data._id,'Mentor Review',req.body.status,req.body.status == 'MENTOR-REVIEW-REJECTED'? req.body.mentorReviewComments:"Mentor Review Accepted" ,null,null,null,req.user.user_id)
                            ...
                            .then(subclientNotificationData=>{
                                if(subclientNotificationData != null && subclientNotificationData.subclient.email != null){	
                                    ...
                                    .then(caseData=>{
                                        ...
                                        let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'Address Details'</td><td>${data.address}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.updateOutputqcStatus=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
            caseHistory.create(req.body.case,req.body.component,data._id,'Output QC Status Updation',req.body.status,req.body.status == 'OUTPUTQC-REJECTED'? req.body.outputqcComments:"" ,null,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.approveInsuff2=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
               caseHistory.create(req.body.case,req.body.component,data._id,'Insuff Level 2 Approved','INSUF-2-REQ-ACCEPTED',"Insuf 2 request Accepted" ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.rejectInsuff2=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
        caseHistory.create(req.body.case,req.body.component,data._id,'Insuf Request Rejection','Insuf Request Rejected',req.body.insufficiencyRejectionComments ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.clearInsuff1=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'Insuf 1 Cleared','INSUF-1-CLEARED',req.body.insufficiencyClearedComments ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.clearInsuff2=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Insuf 2 Cleared','INSUF-2-CLEARED',req.body.insufficiencyClearedComments ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.rejectInsuff1Clearance=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Insuf 1 Clearance Rejection','INSUF-1-CLEARANCE-REJECTED',req.body.insufficiencyClearanceRejectionComments ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.rejectInsuff2Clearance=(req,res)=>{
 address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
   ...
   .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Insuf 2 Clearance Rejection','INSUF-2-CLEARANCE-REJECTED',req.body.insufficiencyClearanceRejectionComments ,null,null,null,req.user.user_id)
       res.json(data)
    })
   .catch(err=>{
       res.status(500).json({
...
exports.getInsuffForScrutiny=(req,res)=>{
 ...
 address
 .find({$or:[{status:'INSUF-1-CLEARED'},{status:'INSUF-2-CLEARED'},{status:'INSUF-2-REQ'},{status:'CLARIFICATION-REQ'},{status:'COST-APPROVAL-REQ'},{status:'CLARIFICATION-CLEARED'},{status:'COST-APPROVAL-CLEARED'}]})
.populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   address.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
                                              caseHistory.create(req.body.case,req.body.component,data._id,'Deleting a check','Check Deleted','Check Deleted',null,null,null,req.user.user_id)
                         res.json(data)
                    })
                   .catch(err=>{
                         res.json({
...
exports.putItToFeBucket=(req,res)=>{
     ...
     .then(data=>{
                  caseHistory.create(req.body.case,req.body.component,data._id,'Put Check in FE Bucket',req.body.status,'Putting Check in FE Bucket',null,null,null,req.user.user_id)
         res.json(data)
      })
     .catch(err=>{
         res.status(500).json({
...
exports.putItToVendorBucket=(req,res)=>{
     ...
     .then(data=>{
            caseHistory.create(req.body.case,req.body.component,data._id,'Putting Check in Vendor Bucket','Put Check in Venfo Bucket','Check put in vendor bucket' ,null,null,null,req.user.user_id)
         res.json(data)
      })
     .catch(err=>{
         res.status(500).json({
...
exports.allocateCheckToFe=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         allocatedToFE:req.body.allocatedToFE,
         allocatedToVendor:null,
         feAllocationDate:Date.now(),
         })
         .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'Allocating to FE','ALLOCATED-TO-FE','Allocated to FE' ,null,req.user.user_id,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.allocateCheckToVendor=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
             caseHistory.create(req.body.case,req.body.component,data._id,'Allocating to Vendor','ALLOCATED-TO-VENDOR','Check allocated to vendor' ,req.body.vendor,null,null,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.allocateCheckToVerifier=(req,res)=>{
     address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
         ...
         .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Allocating Check to Analyst','Allocated Check to Analyst','Allocated Check to Analyst' ,null,null,req.body.veifier,req.user.user_id)
             res.json(data)
         })
         .catch(err=>{
             res.status(500).json({
...
exports.reinitiateCheck=(req,res)=>{
    ...
    address.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
     ...
     .then(data=>{
         caseHistory.create(req.body.case,req.body.component,data._id,'Reinitiating a check','REINITIATED',req.body.reinitiationComments,null,null,null,req.user.user_id)
         res.json(data)
      })
     .catch(err=>{
         res.status(500).json({
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull= (req,res)=>{
    console.log("Agout to get grades")	
    address
    .find({subclient:null})
    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
    .then(data=>{
         data.forEach(item=>{
           console.log("About to update subclient of address",item._id)		 
           address
...
/*           Case
           .update({_id:item.case._id},{$push:{actualComponents:"address"}})
           .then(data=>{
	      console.log("Updated the case ",data)	   
	   })		 
           .catch(err=>{
...
//	res.json(data)	    
	...
	res.status(500).json({
	   message:"Error Reading Grade to convert"	
	})
    })	
}
exports.getChecksWhereVerifierNotFoundInUser= (req,res)=>{
    console.log("In Get Checks Where Veifiere Not Found in user")
	 ...
	 data.forEach(item=>{
	   ...
	   .then(data=>{
	      ...
	      console.log("Error in updating address ",err)	   
           ...
           Case
           .find({_id:item.case._id})
           .then(caseData=>{
              let compArray = new Array()
              if(caseData.actualComponents != null){
                 if(!caseData.actualComponents.includes("address")){
                    compArray = caseData.actualComponents
	            compArray.push("address")		
                    Case
                   .findOneAndUpdate({_id:item.case._id},{actualComponents:compArray})
                   .then(caseUpdatedData=>{
                   })
                   .catch(err=>{
                     console.log("Error in updating bank stmt")
                   })
                 }
...
//    .catch(err=>{
//        res.status(500).json({
//           message:"Error Reading Verifier to convert"
//        })
//    })
}
...
//       acase.addCheck(req.body.case,req.body.component,data._id)
        Case
        .update({_id:req.body.case},{$push:{actualComponents:"address"}})
        .then(data=>{
           console.log("Updated the case ",data)
...
exports.findOneForCde=(req,res)=>{
    let caseId = req.caseId
    if(caseId != null){
       address.findOne({case:req.params.caseId,serialNumber:req.params.serialNumber})
       .then(data=>{
            res.json(data)
       })
       .catch(err=>{
...
//   modifiedBy:req.user.user_id
   ...
   .then(data=>{
              caseHistory.create(req.body.case,req.body.component,data._id,'Updating Check',req.body.status,'Check Created',null,null,null,null)
       res.json(data)
   })
   .catch(err=>{
       res.status(500).json({
...
exports.deleteAllFilesForCde=(req,res)=>{
   let caseId = authenticateCdeToken(req.query.access_token)
   if(caseId != null){
