const directorproprietorresidencecheck = require('../../models/data_entry/directorproprietorresidencecheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.pinzipcode){
       res.status(400).json({message:'Pin / Zip Code required'})
   }
   if(!req.body.landmark){
       res.status(400).json({message:'Landmark required'})
   }
   if(!req.body.periodofstayinthisaddress){
       res.status(400).json({message:'Period of Stay in this Address required'})
   }
   if(!req.body.natureofoccupancy){
       res.status(400).json({message:'Nature of Occupancy required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new directorproprietorresidencecheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645df577da4885b59f88fc83":req.body.component,
       ...
       landmarkRhs:null,
       periodofstayinthisaddress:req.body.periodofstayinthisaddress,
       periodofstayinthisaddressRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"directorproprietorresidencecheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving directorproprietorresidencecheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 directorproprietorresidencecheck
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving directorproprietorresidencecheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   directorproprietorresidencecheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       completeaddress:req.body.completeaddress,
       city:req.body.city,
       state:req.body.state,
       pinzipcode:req.body.pinzipcode,
...
exports.findOne=(req,res)=>{
   directorproprietorresidencecheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    directorproprietorresidencecheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    directorproprietorresidencecheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   directorproprietorresidencecheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
