const 7paneldrugabusetest = require('../../models/data_entry/7paneldrugabusetest.model')
...
if(!req.body.case){
   ...
   const obj = new 7paneldrugabusetest({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e06e4da4885b59f88ff16"  :req.body.component,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving 7paneldrugabusetest'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving 7paneldrugabusetest for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   7paneldrugabusetest.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   7paneldrugabusetest.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    7paneldrugabusetest.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    7paneldrugabusetest.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   7paneldrugabusetest.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
