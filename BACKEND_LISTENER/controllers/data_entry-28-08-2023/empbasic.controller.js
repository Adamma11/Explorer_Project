...
if(!req.body.case){
   ...
   if(!req.body.doj){
       res.status(400).json({message:'Date of Joining (DD-MMM-YYYY) required'})
   }
   if(!req.body.lwd){
       res.status(400).json({message:'Last Working Day (DD-MMM-YYYY) required'})
   }
  ...
  personalDetailsData:req.body.personalDetails,
   ...
   .then(data=>{
       ...
       .catch(caseErr=>{
        ...
        Case
        .update({_id:req.body.case},{$push:{actualComponents:"empbasic"}})
        .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving empbasic'
       })
   })
};
...
// console.log("data",data)
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving empbasic for cases'
       })
   })
};
...
exports.deleteProofOfWork=(req,res)=>{
   console.log("About to delete Employment pow ",req.params.caseId + "/" + req.params.componentName + "/" + req.params.componentId + "/" + req.query.fileName)
   fs.unlink('/REPO_STORAGE/case_uploads/'+req.params.caseId +'/' + req.params.componentName+ '/' + req.params.componentId + '/proofofwork/' + req.query.fileName + '.pdf',function(err){
       if(err){
	   console.log("Error deleting pow")    
           res.status(500).send({message:"Error deleting the file"});
       }
	   
       console.log("POW Deleted")	   
...
if(!req.body.case){
   ...
   empbasic.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameofemployer:req.body.nameofemployer,
       branch:req.body.branch,
       deputedto:req.body.deputedto,
       empstatus:req.body.empstatus,
...
exports.findOne=(req,res)=>{
   empbasic.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    empbasic.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
                            ...
                            .then(subclientNotificationData=>{
                                if(subclientNotificationData != null && subclientNotificationData.subclient.email != null){	
                                    ...
                                    .then(caseData=>{
                                        ...
                                        let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'Employment Basic Details'</td><td>${data.nameofemployer}</td><td>${data.gradingComments}</td></tr></table>`
...
exports.deleteCheck=(req,res)=>{
   empbasic.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull= (req,res)=>{
       console.log("About to fill subclient to empbasic")	
       empbasic
       .find({subclient:null})
       .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
       .then(data=>{
         data.forEach(item=>{
//           console.log("About to update subclient of address online",item.case.caseId)
           empbasic
           .findOneAndUpdate({_id:item._id},{subclient:item.case.subclient._id,client:item.case.subclient.client._id})
           .then(data=>{
	      console.log("Updated emp basic for case ",item.case.caseId)	   
           })
           .catch(err=>{
              console.log("Error in updating address ",err)
           })
/*           Case
           .update({_id:item.case._id},{$push:{actualComponents:"empbasic"}})
           .then(data=>{
              console.log("Updated the case ",data)
           })
           .catch(err=>{
...
exports.findOneForCde=(req,res)=>{
   ...
   let caseId = req.caseId
   if(caseId != null){
      empbasic.findOne({case:req.params.caseId,serialNumber:req.params.serialNumber})
        .then(data=>{
            res.json(data)
        })
        .catch(err=>{
