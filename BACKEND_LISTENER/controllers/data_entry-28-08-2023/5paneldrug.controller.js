const 5paneldrug = require('../../models/data_entry/5paneldrug.model')
...
if(!req.body.case){
   ...
   const obj = new 5paneldrug({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e05e7da4885b59f88fece" :req.body.component,
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving 5paneldrug'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving 5paneldrug for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   5paneldrug.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       drugschecked:req.body.drugschecked,
       result:req.body.result,
   status:req.body.status,
...
exports.findOne=(req,res)=>{
   5paneldrug.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    5paneldrug.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.deleteCheck=(req,res)=>{
   5paneldrug.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
