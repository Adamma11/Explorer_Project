const formsixteencheck = require('../../models/data_entry/formsixteencheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.employernameformsixteenissuedby){
       res.status(400).json({message:'Employer Name (Form 16 Issued by) required'})
   }
   if(!req.body.financialyearperformsixteen){
       res.status(400).json({message:'Financial Year Per Form 16 required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new formsixteencheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645dffadda4885b59f88fd07":req.body.component,
       candidatename:req.body.candidatename,
       candidatenameRhs:null,
       pannumber:req.body.pannumber,
       pannumberRhs:null,
       employernameformsixteenissuedby:req.body.employernameformsixteenissuedby,
       employernameformsixteenissuedbyRhs:null,
       financialyearperformsixteen:req.body.financialyearperformsixteen,
       financialyearperformsixteenRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"formsixteencheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving formsixteencheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving formsixteencheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   formsixteencheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       candidatename:req.body.candidatename,
       pannumber:req.body.pannumber,
       employernameformsixteenissuedby:req.body.employernameformsixteenissuedby,
       financialyearperformsixteen:req.body.financialyearperformsixteen,
...
exports.findOne=(req,res)=>{
   formsixteencheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    formsixteencheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     formsixteencheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       ...
       employernameformsixteenissuedbyRhs:req.body.employernameformsixteenissuedbyRhs,
       financialyearperformsixteenRhs:req.body.financialyearperformsixteenRhs,
...
exports.deleteCheck=(req,res)=>{
   formsixteencheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
