const webandmediacheck = require('../../models/data_entry/webandmediacheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.fraudcheckfrominternationalmedia){
       res.status(400).json({message:'Fraud Check from International Media required'})
   }
   if(!req.body.socialmediaracismandordemonstrationofintolerance){
       res.status(400).json({message:'Social Media  Racism and or Demonstration of Intolerance required'})
   }
   if(!req.body.socialmediapotentiallyviolentbehavior){
       res.status(400).json({message:'Social Media Potentially Violent Behavior required'})
   }
   if(!req.body.socialmediapotentiallyunlawfulactivity){
       res.status(400).json({message:'Social Media Potentially Unlawful Activity required'})
   }
   if(!req.body.socialmediapostingsexuallyexplicitmaterial){
       res.status(400).json({message:'Social Media Posting Sexually Explicit Material required'})
   }
   if(!req.body.socialmediadisplayintoleranceoverreligiousbeliefs){
       res.status(400).json({message:'Social Media Display intolerance over Religious beliefs required'})
   }
   if(!req.body.misrepresentationofemploymentprofileandcredentials){
       res.status(400).json({message:'Misrepresentation of Employment Profile and Credentials required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new webandmediacheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0e24da4885b59f890049":req.body.component,
       fraudcheckfrominternationalmedia:req.body.fraudcheckfrominternationalmedia,
       fraudcheckfrominternationalmediaRhs:null,
       socialmediaracismandordemonstrationofintolerance:req.body.socialmediaracismandordemonstrationofintolerance,
       socialmediaracismandordemonstrationofintoleranceRhs:null,
       socialmediapotentiallyviolentbehavior:req.body.socialmediapotentiallyviolentbehavior,
       socialmediapotentiallyviolentbehaviorRhs:null,
       socialmediapotentiallyunlawfulactivity:req.body.socialmediapotentiallyunlawfulactivity,
       socialmediapotentiallyunlawfulactivityRhs:null,
       socialmediapostingsexuallyexplicitmaterial:req.body.socialmediapostingsexuallyexplicitmaterial,
       socialmediapostingsexuallyexplicitmaterialRhs:null,
       socialmediadisplayintoleranceoverreligiousbeliefs:req.body.socialmediadisplayintoleranceoverreligiousbeliefs,
       socialmediadisplayintoleranceoverreligiousbeliefsRhs:null,
       misrepresentationofemploymentprofileandcredentials:req.body.misrepresentationofemploymentprofileandcredentials,
       misrepresentationofemploymentprofileandcredentialsRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"webandmediacheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving webandmediacheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving webandmediacheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   webandmediacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       fraudcheckfrominternationalmedia:req.body.fraudcheckfrominternationalmedia,
       socialmediaracismandordemonstrationofintolerance:req.body.socialmediaracismandordemonstrationofintolerance,
       socialmediapotentiallyviolentbehavior:req.body.socialmediapotentiallyviolentbehavior,
       socialmediapotentiallyunlawfulactivity:req.body.socialmediapotentiallyunlawfulactivity,
...
exports.findOne=(req,res)=>{
   webandmediacheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    webandmediacheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     webandmediacheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       fraudcheckfrominternationalmediaRhs:req.body.fraudcheckfrominternationalmediaRhs,
       socialmediaracismandordemonstrationofintoleranceRhs:req.body.socialmediaracismandordemonstrationofintoleranceRhs,
       socialmediapotentiallyviolentbehaviorRhs:req.body.socialmediapotentiallyviolentbehaviorRhs,
       socialmediapotentiallyunlawfulactivityRhs:req.body.socialmediapotentiallyunlawfulactivityRhs,
       socialmediapostingsexuallyexplicitmaterialRhs:req.body.socialmediapostingsexuallyexplicitmaterialRhs,
       socialmediadisplayintoleranceoverreligiousbeliefsRhs:req.body.socialmediadisplayintoleranceoverreligiousbeliefsRhs,
       misrepresentationofemploymentprofileandcredentialsRhs:req.body.misrepresentationofemploymentprofileandcredentialsRhs,
         status:req.body.status,
...
exports.deleteCheck=(req,res)=>{
   webandmediacheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
