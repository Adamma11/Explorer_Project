const professionallicensecertificationcheck = require('../../models/data_entry/professionallicensecertificationcheck.model')
...
if(!req.body.case){
   ...
   if(!req.body.nameoftheinstitutecertificateissuingauthority){
       res.status(400).json({message:'Name of the Institute / Certificate issuing Authority required'})
   }
   if(!req.body.city){
   ...
   if(!req.body.contactnumber){
       res.status(400).json({message:'Contact Number required'})
   }
   if(!req.body.areaofexpertise){
       res.status(400).json({message:'Area of Expertise required'})
   }
   if(!req.body.registrationnumber){
       res.status(400).json({message:'Registration Number required'})
   }
   if(!req.body.monthyearoflicensecertificateissued){
       res.status(400).json({message:'Month Year of License / Certificate Issued required'})
   }
   if(!req.body.validtill){
       res.status(400).json({message:'Valid till required'})
   }
   if(!req.body.status){
       res.status(400).json({message:'Status required'})
   }
   const obj = new professionallicensecertificationcheck({
   case:req.body.case,
  personalDetailsData:req.body.personalDetails,
   component:req.body.component ==null ? "645e0f57da4885b59f89007d":req.body.component,
       nameoftheinstitutecertificateissuingauthority:req.body.nameoftheinstitutecertificateissuingauthority,
       nameoftheinstitutecertificateissuingauthorityRhs:null,
       city:req.body.city,
       cityRhs:null,
       ...
       areaofexpertiseRhs:null,
       registrationnumber:req.body.registrationnumber,
       registrationnumberRhs:null,
       monthyearoflicensecertificateissued:req.body.monthyearoflicensecertificateissued,
       monthyearoflicensecertificateissuedRhs:null,
   ...
   Case
   .update({_id:req.body.case},{$push:{actualComponents:"professionallicensecertificationcheck"}})
   .then(data=>{
   ...
   .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while saving professionallicensecertificationcheck'
       })
   })
};
exports.findAllForACase=(req,res)=>{

 professionallicensecertificationcheck
 ...
 .catch(err=>{
       res.status(500).json({
          message:err.message || 'Some error while retrieving professionallicensecertificationcheck for cases'
       })
   })
};
...
if(!req.body.case){
   ...
   professionallicensecertificationcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameoftheinstitutecertificateissuingauthority:req.body.nameoftheinstitutecertificateissuingauthority,
       city:req.body.city,
       state:req.body.state,
       country:req.body.country,
...
exports.findOne=(req,res)=>{
   professionallicensecertificationcheck.findOne({case:req.params.caseId,_id:req.params.componentId})
    .then(data=>{
         res.json(data)
     })
    .catch(err=>{
...
exports.findComponentsFor=(req,res)=>{
 ...
 if(req.params.for=='UPDATE-LHS'){
    ...
    professionallicensecertificationcheck.find(query)
...
exports.findComponentsForVendorManager=(req,res)=>{
    ...
    professionallicensecertificationcheck.find({status:req.params.status})
                    .sort({_id:1})
                    .skip(offSet)
                    .limit(200)
                    .populate({path:'case',populate:{path:'subclient',populate:{path:'client'}}})
...
exports.updateVerificationStatus=(req,res)=>{
 if(req.body.status=='VERIFICATION-COMPLETED' || req.body.verificationStatus=='FE-COMPLETED'){
     professionallicensecertificationcheck.findOneAndUpdate({case:req.params.caseId,_id:req.params.componentId},{
       nameoftheinstitutecertificateissuingauthorityRhs:req.body.nameoftheinstitutecertificateissuingauthorityRhs,
       ...
       areaofexpertiseRhs:req.body.areaofexpertiseRhs,
       registrationnumberRhs:req.body.registrationnumberRhs,
       monthyearoflicensecertificateissuedRhs:req.body.monthyearoflicensecertificateissuedRhs,
...
exports.deleteCheck=(req,res)=>{
   professionallicensecertificationcheck.findOneAndDelete({case:req.params.caseId,_id:req.params.componentId})
                   .then(data=>{
