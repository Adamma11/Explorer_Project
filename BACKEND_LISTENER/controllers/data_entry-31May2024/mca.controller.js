...
const mca = require('../../models/data_entry/mca.model')
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await mca.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "mca" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving mca'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    mca
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving mca for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    mca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    mca.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    mca
        ...
        mca.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
mcafoundRhs: req.body.mcafoundRhs,
statusRhs: req.body.statusRhs,
doiRhs: req.body.doiRhs,
lbsRhs: req.body.lbsRhs,
adressRhs: req.body.adressRhs,
emailidRhs: req.body.emailidRhs,
emarksRhs: req.body.emarksRhs,
...
status: req.body.status,
        ...
        const mcaData = await mca.findOne({ case: req.params.caseId, _id: req.params.componentId })
        ...
        } else if (!mcaData.secondInsufficiencyRaisedDate) {
        ...
        } else if (!mcaData.thirdInsufficiencyRaisedDate) {
        ...
        } else if (!mcaData.fourthInsufficiencyRaisedDate) {
        ...
        } else if (!mcaData.fifthInsufficiencyRaisedDate) {
...
exports.updateFeVerificationStatus = (req, res) => {
    mca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.nameRhs,mcafoundRhs: req.body.mcafoundRhs,statusRhs: req.body.statusRhs,doiRhs: req.body.doiRhs,lbsRhs: req.body.lbsRhs,adressRhs: req.body.adressRhs,emailidRhs: req.body.emailidRhs,emarksRhs: req.body.emarksRhs,verifiedbyRhs: req.body.verifiedbyRhs,verifiedonRhs: req.body.verifiedonRhs,movRhs: req.body.movRhs,status: req.body.status,
...
exports.updateMentorReviewStatus = (req, res) => {
    mca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
        nameRhs: req.body.data.nameRhs,mcafoundRhs: req.body.data.mcafoundRhs,statusRhs: req.body.data.statusRhs,doiRhs: req.body.data.doiRhs,lbsRhs: req.body.data.lbsRhs,adressRhs: req.body.data.adressRhs,emailidRhs: req.body.data.emailidRhs,emarksRhs: req.body.data.emarksRhs,verifiedbyRhs: req.body.data.verifiedbyRhs,verifiedonRhs: req.body.data.verifiedonRhs,movRhs: req.body.data.movRhs,
	    status: req.body.status,
                        ...
                        if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                            Case
                                ...
                                .then(caseData => {
                                    ...
                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'mca Details'</td><td>${data.name}</td><td>${data.gradingComments}</td></tr></table>`
...
/*exports.approveInsuff2 = async (req, res) => {
    ...
    try {
        ...
        if (!mcatData.secondInsufficiencyRaisedDate) {

            const data = await mca.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.searchACaseForInsuffForClient = (req, res) => {
    ...
    console.log("In search for insuff case mca, case id required is ", req.query.caseId)

    Case
        .findOne({ caseId: req.query.caseId })
        .then(caseData => {
            console.log("Case Data found for mca insuff")
            ...
            UserSubclientAccess.find({ user: req.user.user_id }, { _id: 0, subclient: 1 })
                .then(userSubclientData => {
                    console.log("In mca insuff user subclient access is ", userSubclientData)
                    mca
                        ...
                        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
                        .then(data => {
                            console.log("Found the mca insuff data ", data)
                            res.json(data)
                        })
                        .catch(err => {
                            res.status(500).json({
                                message: "Error searching insuff for mca"
                            })
                        })
                })
                .catch(err => {
...
exports.approveInsuffClearance = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == "INSUF-2-CLEARED") {
            ...
            if (!mcaData.secondInsufficiencyClearedDate) {
            ...
            } else if (!mcaData.thirdInsufficiencyClearedDate) {
            ...
            } else if (!mcaData.fourthInsufficiencyClearedDate) {
            ...
            } else if (!mcaData.fifthInsufficiencyClearedDate) {
...
exports.deleteCheck = (req, res) => {
    mca.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull = (req, res) => {
    mca
        ...
        .then(data => {
            data.forEach(item => {
                console.log("About to update subclient of mca", item._id)
                mca
                    .findOneAndUpdate({ _id: item._id }, { subclient: item.case.subclient._id, client: item.case.subclient.client._id })
                    .then(data => {
                    })
                    .catch(err => {
                        console.log("Error in updating mca ", err)
                    })
            })
            res.json({ message: "Completed" })
        })
        .catch(err => {
            console.log("Error reading mca", err)
            res.status(500).json({
...
exports.createForCde = (req, res) => {
    console.log("In mca Create for CDE")
...
if (!req.body.mcafound) {
            res.status(400).json({ message: "mcafound is Mandatory" })
        }
if (!req.body.status) {
            res.status(400).json({ message: "status is Mandatory" })
        }
if (!req.body.doi) {
            res.status(400).json({ message: "doi is Mandatory" })
        }
if (!req.body.lbs) {
            res.status(400).json({ message: "lbs is Mandatory" })
        }
if (!req.body.adress) {
            res.status(400).json({ message: "adress is Mandatory" })
        }
if (!req.body.emailid) {
            res.status(400).json({ message: "emailid is Mandatory" })
        }
if (!req.body.emarks) {
            res.status(400).json({ message: "emarks is Mandatory" })
        }
...
if (!req.body.status) {
    ...
    console.log("Case Id for mca CDE ", caseId)
    if (caseId != null) {
        const obj = new mca({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
...
status: req.body.status,
        ...
        console.log("Object for saving created in CDE for mca")
        if (req.body.status == 'CDE-COMPLETED') {
            obj.dataEntryCompletionDate = new Date()
        } else {
            obj.insufficiencyComments = req.body.insufficiencyComments
        }
        console.log("About to save mca for CDE")
        obj
            .save(obj)
            .then(data => {
            ...
            .catch(err => {
                console.log("Error Occurred while creating the mca for CDE ", err)
                res.status(500).json({
                    message: err.message || 'Some error while saving mca'
                })
            })
...
exports.findOneForCde = (req, res) => {
    console.log("In mca for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        mca.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await mca.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await mca.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await mca.findOne({ _id: req.params.case_id })
