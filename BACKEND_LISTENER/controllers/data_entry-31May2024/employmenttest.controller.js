...
const employmenttest = require('../../models/data_entry/employmenttest.model')
...
if (!req.body.status) {
    ...
    const obj = new employmenttest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
        component: req.body.component == null ? "660f8da373447ef73213e8a8" : req.body.component,
    ...
    obj
        ...
        .then(data => {
            ...
            Case
                ...
                .then(async caseData => {
                    await employmenttest.findOneAndUpdate({ _id: obj._id }, { client: caseData.client, subclient: caseData.subclient })
		//.then(caseData => {
                    let checksEntered = 0
            ...
            Case
                .update({ _id: req.body.case }, { $push: { actualComponents: "employmenttest" } })
                .then(data => {
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while saving employmenttest'
            })
        })
};
...
exports.findAllForACase = (req, res) => {
    employmenttest
        ...
        .catch(err => {
            res.status(500).json({
                message: err.message || 'Some error while retrieving employmenttest for cases'
            })
        })
};
...
exports.update = (req, res) => {
    ...
    employmenttest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.findOne = (req, res) => {
    employmenttest.findOne({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
            res.json(data)
        })
        .catch(err => {
...
exports.findComponentsForVendorManager = (req, res) => {
    ...
    employmenttest
        ...
        employmenttest.find({ status: req.params.status })
        .sort({ _id: 1 })
        .skip(offSet)
        .limit(200)
        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
...
status: req.body.status,
        ...
        const employmenttestData = await employmenttest.findOne({ case: req.params.caseId, _id: req.params.componentId })
        ...
        } else if (!employmenttestData.secondInsufficiencyRaisedDate) {
        ...
        } else if (!employmenttestData.thirdInsufficiencyRaisedDate) {
        ...
        } else if (!employmenttestData.fourthInsufficiencyRaisedDate) {
        ...
        } else if (!employmenttestData.fifthInsufficiencyRaisedDate) {
                        ...
                        if (subclientNotificationData != null && subclientNotificationData.subclient.email != null) {
                            Case
                                ...
                                .then(caseData => {
                                    ...
                                    let checkDetails = `<table border='1px'><tr><td>Case Id</td><td>Candidate Name</td><td>Component</td><td>Field Value</td><td>Comments</td></tr><tr><td>${caseData.caseId}</td><td>${caseData.candidateName}</td><td>'employmenttest Details'</td><td>${data.organization}</td><td>${data.gradingComments}</td></tr></table>`
...
/*exports.approveInsuff2 = async (req, res) => {
    ...
    try {
        ...
        if (!employmenttesttData.secondInsufficiencyRaisedDate) {

            const data = await employmenttest.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.searchACaseForInsuffForClient = (req, res) => {
    ...
    console.log("In search for insuff case employmenttest, case id required is ", req.query.caseId)

    Case
        .findOne({ caseId: req.query.caseId })
        .then(caseData => {
            console.log("Case Data found for employmenttest insuff")
            ...
            UserSubclientAccess.find({ user: req.user.user_id }, { _id: 0, subclient: 1 })
                .then(userSubclientData => {
                    console.log("In employmenttest insuff user subclient access is ", userSubclientData)
                    employmenttest
                        ...
                        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
                        .then(data => {
                            console.log("Found the employmenttest insuff data ", data)
                            res.json(data)
                        })
                        .catch(err => {
                            res.status(500).json({
                                message: "Error searching insuff for employmenttest"
                            })
                        })
                })
                .catch(err => {
...
exports.approveInsuffClearance = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == "INSUF-2-CLEARED") {
            ...
            if (!employmenttestData.secondInsufficiencyClearedDate) {
            ...
            } else if (!employmenttestData.thirdInsufficiencyClearedDate) {
            ...
            } else if (!employmenttestData.fourthInsufficiencyClearedDate) {
            ...
            } else if (!employmenttestData.fifthInsufficiencyClearedDate) {
...
exports.deleteCheck = (req, res) => {
    employmenttest.findOneAndDelete({ case: req.params.caseId, _id: req.params.componentId })
        .then(data => {
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull = (req, res) => {
    employmenttest
        ...
        .then(data => {
            data.forEach(item => {
                console.log("About to update subclient of employmenttest", item._id)
                employmenttest
                    .findOneAndUpdate({ _id: item._id }, { subclient: item.case.subclient._id, client: item.case.subclient.client._id })
                    .then(data => {
                    })
                    .catch(err => {
                        console.log("Error in updating employmenttest ", err)
                    })
            })
            res.json({ message: "Completed" })
        })
        .catch(err => {
            console.log("Error reading employmenttest", err)
            res.status(500).json({
...
exports.createForCde = (req, res) => {
    console.log("In employmenttest Create for CDE")
...
if (!req.body.status) {
    ...
    console.log("Case Id for employmenttest CDE ", caseId)
    if (caseId != null) {
        const obj = new employmenttest({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
...
status: req.body.status,
        ...
        console.log("Object for saving created in CDE for employmenttest")
        if (req.body.status == 'CDE-COMPLETED') {
            obj.dataEntryCompletionDate = new Date()
        } else {
            obj.insufficiencyComments = req.body.insufficiencyComments
        }
        console.log("About to save employmenttest for CDE")
        obj
            .save(obj)
            .then(data => {
            ...
            .catch(err => {
                console.log("Error Occurred while creating the employmenttest for CDE ", err)
                res.status(500).json({
                    message: err.message || 'Some error while saving employmenttest'
                })
            })
...
exports.findOneForCde = (req, res) => {
    console.log("In employmenttest for CDE Find One")
    let caseId = req.caseId
    if (caseId != null) {
        employmenttest.findOne({ case: req.params.caseId, serialNumber: req.params.serialNumber })
            .then(data => {
                res.json(data)
            })
            .catch(err => {
...
exports.addquicknote = async (req, res) => {
    try {
        const caseData = await employmenttest.findOne({ _id: req.body._id })
        const caseComments = []
        ...
        const updatedCaseData = await employmenttest.findOneAndUpdate({ _id: req.body._id }, { comments: caseComments })
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await employmenttest.findOne({ _id: req.params.case_id })
