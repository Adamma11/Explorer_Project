...
status: req.body.status,
        ...
        const identityData = await identity.findOne({ case: req.params.caseId, _id: req.params.componentId })
        ...
        } else if (!identityData.secondInsufficiencyRaisedDate) {
        ...
        } else if (!identityData.thirdInsufficiencyRaisedDate) {
        ...
        } else if (!identityData.fourthInsufficiencyRaisedDate) {
        ...
        } else if (!identityData.fifthInsufficiencyRaisedDate) {
...
/*exports.approveInsuff2 = async (req, res) => {
    ...
    try {
        ...
        if (!identitytData.secondInsufficiencyRaisedDate) {

            const data = await identity.findOneAndUpdate({ case: req.params.caseId, _id: req.params.componentId }, {
...
exports.searchACaseForInsuffForClient = (req, res) => {
    ...
    console.log("In search for insuff case identity, case id required is ", req.query.caseId)

    Case
        .findOne({ caseId: req.query.caseId })
        .then(caseData => {
            console.log("Case Data found for identity insuff")
            ...
            UserSubclientAccess.find({ user: req.user.user_id }, { _id: 0, subclient: 1 })
                .then(userSubclientData => {
                    console.log("In identity insuff user subclient access is ", userSubclientData)
                    identity
                        ...
                        .populate({ path: 'case', populate: { path: 'subclient', populate: { path: 'client' } } })
                        .then(data => {
                            console.log("Found the identity insuff data ", data)
                            res.json(data)
                        })
                        .catch(err => {
                            res.status(500).json({
                                message: "Error searching insuff for identity"
                            })
                        })
                })
                .catch(err => {
...
exports.approveInsuffClearance = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == "INSUF-2-CLEARED") {
            ...
            if (!identityData.secondInsufficiencyClearedDate) {
            ...
            } else if (!identityData.thirdInsufficiencyClearedDate) {
            ...
            } else if (!identityData.fourthInsufficiencyClearedDate) {
            ...
            } else if (!identityData.fifthInsufficiencyClearedDate) {
...
exports.getChecksWhereGradeOneIsNullAndGradeNotNull = (req, res) => {
    identity
        ...
        .then(data => {
            data.forEach(item => {
                console.log("About to update subclient of identity", item._id)
                identity
        ...
        .catch(err => {
            console.log("Error reading identity", err)
            res.status(500).json({
...
if (!req.body.status) {
    ...
    console.log("Case Id for identity CDE ", caseId)
    if (caseId != null) {
        const obj = new identity({
        case: req.body.case,
        personalDetailsData: req.body.personalDetails,
...
status: req.body.status,
        ...
        console.log("Object for saving created in CDE for identity")
        if (req.body.status == 'CDE-COMPLETED') {
            obj.dataEntryCompletionDate = new Date()
        } else {
            obj.insufficiencyComments = req.body.insufficiencyComments
        }
        console.log("About to save identity for CDE")
        obj
            .save(obj)
            .then(data => {
            ...
            .catch(err => {
                console.log("Error Occurred while creating the identity for CDE ", err)
                res.status(500).json({
                    message: err.message || 'Some error while saving identity'
                })
            })
