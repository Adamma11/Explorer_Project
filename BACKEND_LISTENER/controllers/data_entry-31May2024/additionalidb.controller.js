...
const checkId = `${caseObj.caseId}-${numComponents}`
   ...
   if(!req.body.passportid){
       res.status(400).json({message:'Passport ID required'})
   }
   if(!req.body.validfrom){
       res.status(400).json({message:'Valid From required'})
   }
  ...
  checkId: checkId,
   component:req.body.component ==null ? "650bca9fc176fe88186133e7":req.body.component,
       name:req.body.name,
       nameRhs:null,
...
exports.getAllquicknoteForACheck = async (req, res) => {
    try {
        ...
        const caseData = await additionalidb.findOne({ _id: req.params._id })
