...
exports.getOperationStatsForAnalyst = (req,res)=>{
    let count = 0	
    let query = "" 
    let today = new Date()
    ...
    if(req.query.reportType=='OPENING'){
      query = {initiationDate:{$lt:year + "-" + monthString + "-" + date},status:{$ne:'MENTOR-REVIEW-ACCEPTED'},status:{$ne:'OUTPUTQC-ACCEPTED'},verificationAllocatedTo:req.user.user_id} 
    }	
    if(req.query.reportType=='INITIATED'){
      //query = {inputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date},status:{$ne:'MENTOR-REVIEW-ACCEPTED'},status:{$ne:'OUTPUTQC-ACCEPTED'},verificationAllocatedTo:req.user.user_id}
    //   query = {inputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date},status:{$ne:'MENTOR-REVIEW-ACCEPTED'},status:{$ne:'OUTPUTQC-ACCEPTED'},verificationAllocatedTo:req.user.user_id}
    query = { $and: [{$or:[{ status: { $ne: "MENTOR-REVIEW-ACCEPTED" } }, { status: { $ne: "OUTPUTQC-ACCEPTED" } },{inputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date}}]}],verificationAllocatedTo:req.user.user_id }
    }
    
   //  console.log("query to apply is ",query)
    if(req.query.reportType=='WIP'){
//	query = {inputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date},status:{$ne:'MENTOR-REVIEW-ACCEPTED'},status:{$ne:'OUTPUTQC-ACCEPTED'},verificationAllocatedTo:req.user.user_id}
	query = {$or:[{status:'INPUTQC-ACCEPTED'},{status:"OUTPUTQC-REJECTED"},{status:"MENTOR-REVIEW-REJECTED"}],verificationAllocatedTo:req.user.user_id}

   // query = {$and:[{status:{$ne:'INSUF-1-REQ-ACCEPTED'},status:{$ne:'INSUF-2-REQ-ACCEPTED'},status:{$ne:"INSUF-1-REQUEST"},status:{$ne:"INSUF-2-REQUEST"},status:{$ne:"INSUF-1-REQUEST-CLEARED"},status:{$ne:"INSUF-2-REQUEST-CLEARED"},status:{$ne:"INSUF-2-CLEARANCE-REJECTED"},status:{$ne:"INSUF-1-CLEARANCE-REJECTED"},status:{$ne:"OUTPUTQC-ACCEPTED"},verificationAlloctedTo:req.user.user_id}]}	    
    }	
//    let query = {$and:[{status:{$ne:"MENTOR-REVIEW-ACCEPTED"},status:{$ne:"OUTPUTQC-ACCEPTED"}}]}
    if(req.query.reportType == 'COMPLETED'){
//    query = {$or:[{status:'MENTOR-REVIEW-ACCEPTED',mentorReviewCompletionDate:{$gte:year + "-" + monthString}},{status:'OUTPUTQC-ACCEPTED',outputqcCompletionDate:{$gte:year + "-" + monthString}}],verificationAllocatedTo:req.user.user_id} 	    
   //query = {$or:[{status:'MENTOR-REVIEW-ACCEPTED',mentorReviewCompletionDate:{$gte:year + "-" + monthString}}],verificationAllocatedTo:req.user.user_id}
	//  query = {$and: [{$or:[{status:'MENTOR-REVIEW-ACCEPTED'},{status:'OUTPUTQC-ACCEPTED'}]},{ mentorReviewCompletionDate: {$gte: new Date(year + "-" + monthString + "-" + date), $lte: new Date(year + "-" + monthString + "-" + dateTo)}}],verificationAllocatedTo:req.user.user_id}
    query = { $and: [{ $or: [{ status: 'MENTOR-REVIEW-ACCEPTED' }, { status: 'OUTPUTQC-ACCEPTED' }] ,verificationAllocatedTo:req.user.user_id}] }

    }
    
    if(req.query.reportType=='INSUF'){
      query = {$or:[{status:'INSUF-1-REQ-ACCEPTED'},{status:'INSUF-2-REQ-ACCEPTED'},{status:"INSUF-1-REQUEST"},{status:"INSUF-2-REQUEST"},{status:"INSUF-1-REQUEST-CLEARED",status:"INSUF-2-REQUEST-CLEARED"},{status:"INSUF-2-CLEARANCE-REJECTED"},{status:"INSUF-1-CLEARANCE-REJECTED"}],verificationAllocatedTo:req.user.user_id}	    
    }	
      console.log("query to apply is ",query)
