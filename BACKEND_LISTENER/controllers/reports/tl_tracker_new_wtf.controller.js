...
//    console.log("Workbook constructed")
    const fileStream = fs.createWriteStream(`/REPO_STORAGE/tmp_tl_tracker/new_tl_tracker_${req.user.user_id}.xls`);	
//    const sheet = workBook.addWorksheet('TL Pending')
    fileStream.write("<table border='1'>")
    fileStream.write('<tr>')
    fileStream.write('<td>Case Id</td><td>Candidate Name</td><td>Client</td><td>Subclient</td><td>Component</td><td>Status</td><td>Initiation Date</td><td>Insuff Raised Date</td><td>Insuff Cleared Date</td><td>TAT End Date</td><td>Verifier</td><td>FE</td><td>Vendor</td><td>Verification Completion Date</td><td>Verified By</td><td>Mentor Review Completion Date</td><td>Mentor Review Completed By</td><td>Output QC Completion Date</td><td>Output QC Completed By</td><td>Grading Color</td><td>Field 1</td><td>Field 2</td><td>Field 3</td><td>Field 4</td><td>Field 5</td><td>Field 6</td><td>Field 7</td><td>Field 8</td><td>Field 9</td><td>Field 10</td></tr>')    	
  
//    sheet.addRow(["Case Id","Candidate Name","Client","Subclient","Component","Status","Initiation Date","Insuff Raised Date","Insuff Cleared Date","TAT End Date","Verifier","FE","Vendor","VerificationCompletionDate","Verified By","Mentor Review Completion Date","Mentor Review Completed By","Output QC Completion Date","Output QC Completed By","Grading Color","Field 1","Field 2","Field 3","Field 4","Field 5","Field 6","Field 7","Field 8","Field 9","Field 10"])
    let query = ""
...
/*    let getTatDays = function(client,initiationDate,component){
        return new Promise((resolve,reject)=>{
//           console.log("Trying to get TAT days")		
	   ...
	   .then(data=>{
		...
		.catch(err=>{
		   console.log("Error finding client contract component")	
	           reject(0)		
		})      
	      }	   
	   })
    ...
    let getTatDays = function(client,initiationDate,component,profile,pkg){
        return new Promise((resolve,reject)=>{
//           console.log("Trying to get TAT days")
           if(profile == null && pkg==null){
               ClientContract
               .findOne({client:client,effectiveDate:{$lte:initiationDate},expiryDate:{$gte:initiationDate}})
               .then(data=>{
                  if(data != null){
              ...
              .catch(err=>{
		console.log("Error finding client contract profiles")      
                 resolve(0)
              })
           }else{
              ...
              .catch(err=>{
                console.log("Error finding client contract package")
                 resolve(0)
              })
...
//		   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)	
                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component,item.case.profile,item.case.package)		                  
                   let insuffDays = 0 
                   if(item.insufficiencyRaisedDate != null && item.insufficiencyClearedDate != null){
                     insuffDays = moment(item.insufficiencyClearedDate).diff(item.insufficiencyRaisedDate,'days') + 1 
                   }
                   tatDays = tatDays + insuffDays
			
                   let tempStartDate = new Date(item.case.initiationDate) 
		   let tempEndDateMoment  = moment(tempStartDate,'yyyy-MM-dd').add(tatDays,'days')
  	           let tempEndDate = tempEndDateMoment.toDate()
		   let numberOfWeekends = await getNumberOfWeekends(tempStartDate,tempEndDate)

		   tatDays = tatDays + numberOfWeekends
		   tempStartDate = new Date(item.case.initiationDate)	
		   tempEndDateMoment = moment(tempStartDate,'yyyy-MM-dd').add(tatDays,'days')
		   tempEndDate = tempEndDateMoment.toDate()
		   let numberOfHolidays = await getHolidays(tempStartDate,tempEndDate)

		   tatDays = tatDays + numberOfHolidays

		   tempStartDate = new  Date(item.case.initiationDate)
                   let tatEndDateMoment = moment(tempStartDate,'yyyy-MM-dd').add(tatDays,'days')
                   let tatEndDate = tatEndDateMoment.toDate()
...
/*		   let insuffDays = 0
		   if(item.insufficiencyRaisedDate != null && item.insufficiencyClearedDate != null){
		     insuffDays = moment(item.insufficiencyClearedDate).diff(item.insufficiencyRaisedDate,'days')	
	           }
		   tatDays = tatDays + insuffDays	*/
...
//		   let tatEndDate = moment(tempTatEndDate).add((holidays+weeklyOffs+insuffDays+1),'days').format('DD-MMM-YYYY')	
//	           let tatEndDate = tempTatEndDate
                   fileStream.write('<tr>')		   
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+moment(item.case.initiationDate).format('DD-MMM-YYYY')+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofemployer+'</td><td>'+item.branch+'</td><td>'+item.deputedto+'</td><td>'+item.empstatus+'</td><td>'+item.empid+'</td><td>'+item.doj+'</td><td>'+item.lwd+'</td><td>'+item.designation+'</td><td>'+item.reportingmgr+'</td><td>'+item.reasonforleaving+'</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')	
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofemployer+'</td><td>'+item.branch+'</td><td>'+item.deputedto+'</td><td>'+item.empstatus+'</td><td>'+item.empid+'</td></td>'+item.doj+'</td><td>'+item.lwd+'</td><td>'+item.designation+'</td><td>'+item.reportingmgr+'</td><td>'+item.reasonforleaving+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			 
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+ vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofuniversity+'</td><td>'+item.nameofschool+'</td><td>'+item.cityofstudy+'</td><td>'+item.typeofqualification+'</td><td>'+item.qualification+'</td><td>'+item.specialization+'</td><td>'+item.rollnumber+'</td><td>'+item.yearofjoining+'</td><td>'+item.yearofcompletion+'</td><td>'+item.marks+'</td></tr>')
...
//                   console.log("In Education Advanced......adding to tracker",item.case.caseId)       
	           fileStream.write('<tr>')
	           fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofuniverskty+'</td><td>'+item.nameofschool+'</td><td>'+item.cityofstudy+'</td><td>'+item.typeofqualification+'</td><td>'+item.qualification+'</td><td>'+item.specialization+'</td><td>'+item.rollnumber+'</td><td>'+item.yearofjoining+'</td><td>'+item.yearofcompletion+'</td><td>'+item.marks+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')	
                   fileStream.write('<td>'+ item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.address+'</td><td>'+item.landmark+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenureofstay+'</td><td>'+item.primarycontact+'</td><td>'+item.alternatecontact+'</td><td>'+contactNumber+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+item.component.displayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.fulladdress+'</td><td>'+item.landmark+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenureofstay+'</td><td>'+item.primarycontact+'</td><td>'+item.alternatecontact+'</td></tr>')	
...
//		   fileStream.write('<tr>')
/*		   fileStream.write('<tr>')		   
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+item.component.displayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.fulladdwithpin+'</td><td>'+item.landmark+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenure+'</td><td>'+item.primarycontact+'</td><td>'+item.alternatecontact+'</td></tr>')	*/
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+item.component.displayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.fulladdwithpin+'</td><td>'+item.landmark+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenure+'</td><td>'+item.primarycontact+'</td><td>'+item.alternatecontact+ '</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReivewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.addresswithpin+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenure+'</td><td>'+item._id+'</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')	
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.fulladdress+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.typeofaddress+'</td><td>'+item.tenureofstay+'</td><td>'+item._id+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')	
	           fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofreference+'</td><td>'+item.deisgnation+'</td><td>'+item.contactdetails + '</td></tr>')		
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)		       
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.name+'</td><td>'+item.designation+'</td><td>'+item.contact+ '</td></tr>')    
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
		   fileStream.write('<td>'+ item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.typeofid+'</td><td>'+item.nameasperid+'</td><td>'+item.idnumber+'</td><td>'+item.issuedby+'</td></tr>')			
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
	           ...
	           fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReiviewcompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.taxid+'</td><td>'+item.nameasperpan+'</td><td>'+item.dateofbirth+'</td><td>'+item.issuedby+'</td></tr>')	
			
//                   sheet.addRow([item.case.caseId,item.case.candidateName,item.case.subclient.client.name,item.case.subclient.name,componentDisplayName,item.status,item.case.initiationDate,item.insufficiencyRaisedDate,item.insufficiencyClearedDate,tatEndDate,verifier,fe,vendor,verificationCompletionDate,verifier,mentorReviewCompletionDate,mentorReiviewcompletedBy,outputqcCompletionDate,outputqcCompletedBy,gradingColor,item.taxid,item.nameasperpan,item.dateofbirth,item.issuedby])
                })
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+ item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.pannumber+'</td><td>'+item.nameasperpan+'</td><td>'+item.dateofbirth+'</td><td>'+item.issuedby + '</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
		   ...
		   fileStream.write('<tr>')
		   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.pannumber+'</td><td>'+item.panname+'</td><td>'+item.dobofpan+'</td><td>'+item.gender+'</td><td>'+item.fulladdress+'</td><td>'+item.contact+'</td><td>'+item.altcontact+'</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
		   fileStream.write('<td>'+ item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.searchname + '</td></tr>')	
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofemployee+'</td><td>'+item.fulladdress+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.contact+'</td></tr>')			
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.nameofemployee+'</td><td>'+item.address+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.contact+'</td></tr>')			
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyCleredDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.consent+'</td><td>'+item.fulladdress+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.contact+'</td></tr>')			
			
//                   sheet.addRow([item.case.caseId,item.case.candidateName,item.case.subclient.client.name,item.case.subclient.name,componentDisplayName,item.status,item.case.initiationDate,item.insufficiencyRaisedDate,item.insufficiencyCleredDate,tatEndDate,verifier,fe,vendor,verificationCompletionDate,verifier,mentorReviewCompletionDate,mentorReviewCompletedBy,outputqcCompletionDate,outputqcCompletedBy,gradingColor,item.consent,item.fulladdress,item.pin,item.city,item.contact])
                })
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                  ...
                  fileStream.write('<tr>')
                  fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.dlnumber+'</td><td>'+item.nameperdl+'</td><td>'+item.dob+'</td><td>'+item.issuedate+'</td><td>'+item.dlvalid+'</td><td>'+uniqueId+'</td><td>'+item.case._id+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.directorname+'</td><td>'+item.dinnumber+'</td><td>'+item.dob+'</td><td>'+item.verifiedon+'</td></tr>')			
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			 
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompltionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.epicnumber+'</td><td>'+item.epicname+'</td><td>'+item.fulladdress+'</td><td>'+item.pin+'</td><td>'+item.city+'</td><td>'+item.contactnumber+'</td></tr>')			 
			 
//                   sheet.addRow([item.case.caseId,item.case.candidateName,item.case.subclient.client.name,item.case.subclient.name,componentDisplayName,item.status,item.case.initiationDate,item.insufficiencyRaisedDate,item.insufficiencyClearedDate,tatEndDate,verifier,fe,vendor,verificationCompletionDate,verifier,mentorReviewCompltionDate,mentorReviewCompletedBy,outputqcCompletionDate,outputqcCompletedBy,gradingColor,item.epicnumber,item.epicname,item.fulladdress,item.pin,item.city,item.contactnumber])
                })
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.companyname+'</td><td>'+item.regdadd+'</td><td>'+item.cin+'</td><td>'+item.pan+'</td><td>'+item.gst+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.tenure+'</td><td>'+item.nameofbank+'</td><td>'+item.transaction+'</td></tr>')			
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
	           let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component,item.case.profile,item.case.package)
                  ...
                  fileStream.write('<tr>')
                  fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.applicantname+'</td><td>'+item.dateofbirth+'</td><td>'+item.stcode+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
	           let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component,item.case.profile,item.case.package)
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.candname+'</td><td>'+item.ofac+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                  ...
                  fileStream.write('<tr>')
                  fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.tenureofgap+'</td><td>'+item.reasonforgap+'</td><td>'+item.address+'</td><td>'+item.PIN+'</td><td>'+item.CITY+'</td></tr>')
...
//                   let tatDays = await getTatDays(item.case.subclient.client,item.case.initiationDate,item.component)			
                   ...
                   fileStream.write('<tr>')
                   fileStream.write('<td>'+item.case.caseId+'</td><td>'+item.case.candidateName+'</td><td>'+item.case.subclient.client.name+'</td><td>'+item.case.subclient.name+'</td><td>'+componentDisplayName+'</td><td>'+item.status+'</td><td>'+item.case.initiationDate+'</td><td>'+item.insufficiencyRaisedDate+'</td><td>'+item.insufficiencyClearedDate+'</td><td>'+tatEndDate+'</td><td>'+verifier+'</td><td>'+fe+'</td><td>'+vendor+'</td><td>'+verificationCompletionDate+'</td><td>'+verifier+'</td><td>'+mentorReviewCompletionDate+'</td><td>'+mentorReviewCompletedBy+'</td><td>'+outputqcCompletionDate+'</td><td>'+outputqcCompletedBy+'</td><td>'+gradingColor+'</td><td>'+item.givenname+'</td><td>'+item.lastname+'</td><td>'+item.issuecountry+'</td><td>'+item.nationality+'</td><td>'+item.passportnumber+'</td><td>'+item.expirydate+'</td></tr>')
...
//        await workBook.xlsx.write(fileStream);	  
	  await new Promise(resolve => setTimeout(resolve, 10000));    
            res.download(`/REPO_STORAGE/tmp_tl_tracker/new_tl_tracker_${req.user.user_id}.xls`,(err)=>{
                if(err){
                  res.status(500).send({
