...
exports.getInsuffForClientReport = (req,res)=>{
    ...
    sheet.addRow(["Case Id","Candidate Name","Client","Subclient","Component","Insuff Raised Date","Insuff Comments"])
		...
		.populate({path:'component'})      
                .then(async data=>{
                   ...
                   reqArray.forEach(item=>{
                      sheet.addRow([item.case.caseId,item.case.candidateName,item.case.subclient.client.name,item.case.subclient.name,item.component.displayName,item.insufficiencyRaisedDate,item.insufficiencyComments])
//                 }
                   })
                   reslove(true)			
