...
exports.getQcStatusReport = (req,res)=>{
    const workBook = new ExcelJS.Workbook()
//    console.log("Workbook constructed")
    const sheet = workBook.addWorksheet('DE Status')
//    console.log("Sheet added")
    sheet.addRow(["Case Id","Candidate Name","Client","Subclient","Initiation Date","Outputqc Completion Date","Interim Report Completion Date","Type of Report","Outputqc Completed By","Interim Report Completed By","Report Upload Date","Number of Components","Actual Components Array"])
...
//        let cases = new Array()
            Case
	    .find({reportDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"}})
            .populate({path:'subclient',populate:{path:'client'}})
	    .populate({path:'outputqcCompletedBy'})	
...
/*		     componentCount = componentCount + await getAddressCount(item._id)	
		     ...
		     .catch(err=>{
		       console.log("Error updating the number of checks entered",err)	     
    	             ...
    	             sheet.addRow([item.caseId,item.candidateName,item.subclient.client.name,item.subclient.name,item.initiationDate,item.outputqcCompletionDate,item.interimReportDate,item.reportType,item.outputqcCompletedBy != null ? item.outputqcCompletedBy.name:"",item.interimReportUploadedBy != null ? item.interimReportUploadedBy.name:"",item.reportDate,item.actualComponents.length,item.actualComponents,item.grade])    
		}
		   //item.numberOfChecksEntered 
