...
const ClientContract = ('../../models/administration/client_contract.model')
...
//	console.log("Trying to fetch employment details for case status report for the case ")    
		...
		data.forEach(item=>{
		   ...
		   let totalPrice = 0
		   if(packagePrice == 0){
		     let clientContractComponentDetails = getClientContractComponentDetails(contractDetails._id,item.component._id) 	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.givenname,item.lastname,item.issuecountry,item.nationality,item.passportnumber,item.expirydate])
    ...
    let getRelevantContract = function(acase){
       return new Promise((resolve,reject)=>{
          ClientContract
	  .findOne({client:acase.client,effectiveDate:{$lte:acase.initiationDate},expirtyDate:{$gte:acase.initiationDate}})    
	  .then(data=>{
             resolve(data)		  
	  })     
	  .catch(err=>{
...
//		console.log("About to get employment details")
		...
		let packagePrice = await etPackagePrice(contractDetails,item.package)    
		let personalDetails = await getPersonalDetails(item._id)
                let employmentDetails = await getEmploymentDetails(item._id,item.packagePrice,contractDetails)
                let empbasicDetails = await getEmpBasicDetails(item._id,item.packagePrice,contractDetails)
                let empadvanceDetails = await getEmpAdvanceDetails(item._id,item.packagePrice,contractDetails)
                let educationDetails = await getEducationDetails(item._id,item.packagePrice,contractDetails)
