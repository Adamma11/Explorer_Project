...
//	   let cam = item.cam.name   
	   ...
	   if(casesReceived != 0 || activeInsuf != 0 || wipCases != 0 || casesCompleted != 0){    
		let detailRow2 =   sheet.addRow([clientName,branch,cam,casesReceived,activeInsuf,wipCases,wipBeyondTat,wipWithinTat,casesCompleted,casesCompletedBeyondTat,casesCompletedWithinTat])    
		detailRow2.commit()   
	   }	   
...
//     .find({$or:[{status:"OUTPUTQC-ACCEPTED",subclient:subclient_id,outputqcCompletionDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"}},{status:"OUTPUTQC-ACCEPTED",subclient:subclient_id,outputqcCompletionDate:null}]})
       .find({$and:[{status:"OUTPUTQC-ACCEPTED",subclient:subclient_id,outputqcCompletionDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"}}]})
     .lean()	   
