...
//	console.log("Trying to fetch employment details for case status report for the case ")    
        return new Promise((resolve,reject)=>{
            let componentDetailsArray = new Array()
            employment
            .find({case:acase})
	    .lean()	
		...
		data.forEach(item=>{
		   ...
		   if(item.allocatedToVendor != null){
			...
			details = (item.component != null ? item.component.displayName:"Employment")   
		       ...
		       componentName : (item.component != null ? item.component.displayName:"Employment"),
		       field1 : item.empstatus,
		       field2:item.nameofemployer,	
		       field3:item.branch,
		       status:((item.status == 'MENTOR-REVIEW-ACCEPTED' || item.status=='OUTPUTQC-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
                       completionDate: ((item.status == 'MENTOR-REVIEW-ACCEPTED' || item.status=='OUTPUTQC-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status  == "MENTOR-REVIEW-ACCEPTED" || item.status=='OUTPUTQC-ACCEPTED')? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
		   })
		   componentDetailsArray.push(employmentDetails)	
	    ...
	    .populate({path:'personalDetailsData'})	
            .then(data=>{
			 ...
			 details  = (item.component != null ? item.component.displayName:"Employment - Basic")     
		      ...
		      let componentDetails = ({
		         componentName: (item.component != null ? item.component.displayName:"Employment - Basic"),
			 field1: item.empstatus,
                         field2:item.nameofemployer,
			 field3:item.branch,
			 status:((item.status == 'MENTOR-REVIEW-ACCEPTED' || item.status=='OUTPUTQC-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
			 completionDate: ((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
			 color: rowValues[colCount++] = ((item.status  == "MENTOR-REVIEW-ACCEPTED" || item.status =='OUTPUTQC-ACCEPTED')? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")     
			      
		      })	   
		      componentDetailsArray.push(componentDetails)	   
...
//                       sheet.addRow([item.component.displayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.nameofemployer,item.branch,item.deputedto,item.empstatus,item.empid,item.doj,item.lwd,item.designation,item.reportingmgr,item.reasonforleaving])
    ...
    let getEmpAdvanceDetails = function(acase){
		...
		let details = ""    
		   ...
		   let componentDetails = ({
		       componentName:(item.component != null ? item.component.displayName:"Employment - Advanced"),
		       field1:item.empstatus,
		       field2:item.nameofemployer,
		       field3:item.branch,
		       status:((item.status == 'MENTOR-REVIEW-ACCEPTED' || item.status=='OUTPUTQC-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status  == "OUTPUTQC-ACCEPTED" || item.status=='MENTOR-REVIEW-ACCEPTED')? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
		   })
		   componentDetailsArray.push(employmentDetails)	
...
//	console.log("In education details of tl pending")    
	    ...
	    .populate({path:'personalDetailsData'})
		 ...
		 data.forEach(item=>{
	           ...
	           let componentDetails = ({
		       componentName:(item.component != null ? item.component.displayName:"Education"),
		       field1:item.nameofuniversity,
		       field2:item.qualification,
		       field3:item.specialization,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status =='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
    ...
    let getEducationAdvancedDetails = function(acase){
	    ...
	    .populate({path:'personalDtailsData'})	
		 ...
		 let details  = ""   
		   ...
		   let componentDetails = ({
		       componentName:(item.component != null ? item.component.displayName:"Education - Advanced"),
		       field1:item.nameofuniverskty,
		       field2:item.qualification,
		       field3:item.specialization,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
...
//                   sheet.addRow([item.component.displayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.nameofuniverskty,item.nameofschool,item.cityofstudy,item.typeofqualification,item.qualification,item.specialization,item.rollnumber,item.yearofjoining,item.yearofcompletion,item.marks])
    ...
    let getEducationComprehensiveDetails = function(acase){
        return new Promise((resolve,reject)=>{
	    let componentDetailsArra = new Array()	
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
		       componentName:(item.component != null ?item.component.displayName:"Education - Comprehensive"),
		       field1:item.nameofuniversity,
		       field2:item.qualification,
		       field3:item.specialization,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
...
//                   sheet.addRow([item.component.displayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.nameofuniversity,item.nameofschool,item.cityofstudy,item.typeofqualification,item.qualification,item.specialization,item.rollnumber,item.yearofjoining,item.yearofcompletion,item.marks])
    ...
    let getAddressDetails = function(acase){
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
		       componentName : (item.component != null ?item.component.displayName:"Address - Advanced"),
		       field1:item.address,
		       field2:item.pin,
		       field3:item.city,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTE')?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:(item.status  == "OUTPUTQC-ACCEPTED" ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
		   })	
		   componentDetailsArray.push(employmentDetails)	
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.address,item.landmark,item.pin,item.city,item.typeofaddress,item.tenureofstay,item.primarycontact,item.alternatecontact,contactNumber])
    ...
    let getAddressComprehensiveDetails = function(acase){
		...
		let details = ""    
		   ...
		   let componentDetails = ({
		      componentName:(item.component != null ?item.component.displayName:"Address - Comprehensive"),
		      field1: item.fulladdress,
		      field2: item.pin,
		      field3: item.city,
		      status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		      completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
...
//                   sheet.addRow([item.component.displayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.fulladdress,item.landmark,item.pin,item.city,item.typeofaddress,item.tenureofstay,item.primarycontact,item.alternatecontact])
    ...
    let getAddressOnlineDetails = function(acase){
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
		       componentName : (item.component != null ?item.component.displayName:"Address - Online"),
		       field1:item.fulladdresswithpin,
		       field2:item.pin,
		       field3:item.city,
		       status : ((item.status == 'OUTPUTQC-ACCEPTED' || item.status== 'MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),	   
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
...
//                   sheet.addRow([item.component.displayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.address,item.landmark,item.pin,item.city,item.typeofaddress,item.tenureofstay,item.primarycontact,item.alternatecontact])
    ...
    let getCourtRecordDetails = function(acase){
        return new Promise((resolve,reject)=>{
	    let componentDetails = new Array()	
            courtrecord
            .find({case:acase})
            .lean()		
            .populate({path:'verificationAllocatedTo'})
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.fulladdress,item.pin,item.city,item.typeofaddress,item.tenureofstay])
    ...
    let getReferenceDetails = function(acase){
		...
		let details  = ""    
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.nameofreference,
		       field2:item.designation,
		       field3:item.item.contactdetails,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.name,item.designation,item.contact])
    ...
    let getIdentityDetails = function(acase){
		...
		let details  = ""    
		   ...
		   let componentDetails = ({
		       componentName : componentDisplayName,
		       field1:item.typeofid,
		       field2:item.idnumber,
		       field3:item.issuedby,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
		   })
    ...
    let getGlobaldatabaseDetails = function(acase){
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
	           ...
	           componentDetailsArray.push(componentDetailsArray)		
//                   rowValues[colCount++] = componentDisplayName
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.searchname])
    ...
    let getDrugTestFiveDetails = function(acase){
		...
		let details  = ""    
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.nameofemployee,
		       field2:item.fulladdress,
		       field3:item.contact,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.nameofemployee,item.address,item.pin,item.city,item.contact])
    ...
    let getDlCheckDetails = function(acase){
		...
		let details = ""
		   ...
		   let componentDetails  = ({
		       componentName:componentDisplayName,
		       field1:item.nameasperdl,
		       field2:item.issueddate,
		       field3:item.dlvalid,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.dlnumber,item.nameperdl,item.dob,item.issuedate,item.dlvalid])
    ...
    let getDirectorshipCheckDetails = function(acase){
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.direcotorname,
		       field2:item.dinnumber,
		       field3:item.dob,
		       status: ((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED') ? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.companyname,item.regdadd,item.cin,item.pan,item.gst])
    ...
    let getBankStmtDetails = function(acase){
	    ...
	    .populate({path:'personalDetailsData'})	
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.nameofbank,
		       field2:item.tenure,
		       field3:item.transaction,
		       status:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:((item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')?  (item.mentorReviewCompletionDate !=null ? moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""):""),
		       color:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? (item.grade !=null ? getColorCodeName(item.grade):"Green"):"")	   
		   })	
		   componentDetailsArray.push(componentDetails)	
//                   rowValues[colCount++] = componentDisplayName
 ...
 //                  sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.candname,item.ofac])
    ...
    let getGapVfnDetails = function(acase){
		...
		let details = ""    
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.tenureofgap,
		       field2:item.resonforgap,
		       field3:item.address,
		       status:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:=( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? (item.grade != null ? getColorCodeName(item.grade) : "Green"):"")	   
...
//                   sheet.addRow([componentDisplayName,item.status,item.case.initiationDate,item.case.tatEndDate,verifier,fe,vendor,item.tenureofgap,item.reasonforgap,item.address,item.PIN,item.CITY])
    ...
    let getPassportDetails = function(acase){
		...
		let details = ""    
		   ...
		   let componentDetails = ({
		       componentName:componentDisplayName,
		       field1:item.givenname,
		       field2:item.lastname,
		       field3:item.passportnumber,
		       status:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? "Completed" :(item.status.substring(0,5) == "INSUF" ? "Insufficiency" : "In Progress")),
		       completionDate:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')?  moment(item.mentorReviewCompletionDate).format('DD-MMM-YYYY'):""),
		       color:( (item.status == 'OUTPUTQC-ACCEPTED' || item.status=='MENTOR-REVIEW-ACCEPTED')? (item.grade !=null ? getColorCodeName(item.grade):"Green"):"")	   
		   })	
...
//		console.log(item.actualComponents);	
		...
		let personalDetails = await getPersonalDetails(item._id)    
		    ...
		    //		let personalDetails = await getPersonalDetails(item._id)
                ...
                let caseDetails:({
                    caseId:item.caseId,
                    candidateName:personalDetails == null ? item.candidateName:personalDetails.candidatename,
                    empId:personalDetails == null ? "" : personalDetails.empid,
                    client:item.subclient.client.name,
                    subclient:item.subclient.name,
                    profile:"",
                    initiationDate:moment(item.initiationDate).format("DD-MMM-YYYY"),
                    insufficiencyRaisedDate:item.firstInsufficiencyRaisedDate == null ? "":moment(item.firstInsufficiencyRaisedDate).format("DD-MMM-YYYY"),
                    insufficiencyClearedDate:item.lastInsufficiencyClearedDate == null ? "":moment(item.lastInsufficiencyClearedDate).format("DD-MMM-YYYY"),
                    tatEndDate:moment(item.tatEndDate).add((numberOfHolidays + numberOfWeekends + numberOfInsuffDays + 1),'days').format("DD-MMM-YYYY"),
                    status:item.status == "OUTPUTQC-ACCEPTED" ? "Completed" : "In Progress",
                    color:item.status == "OUTPUTQC-ACCEPTED" ? (item.grade != null ? getColorCodeName(item.grade):""):"",
                    reportType:item.reportType != null ? item.reportType:"",
                    completionDate:item.outputqcCompletionDate,
                    tatDays:tatDays,
                    actualDays:actualDaysTaken - numberOfHolidays - numberOfWeekends - numberOfInsuffDays + 1,
                })
