...
exports.getCaseFlowReport = (req,res)=>{
    ...
    sheet.addRow(["Client","Subclient","Branch","CAM","Cases Received","Active Insuff","WIP Cases","WIP Beyond TAT","WIP Within TAT","Cases Completed","Completed Beyond TAT","Completed Within TAT"])
...
//	   let cam = item.cam.name   
	   ...
	   if(casesReceived != 0 || activeInsuf != 0 || wipCases != 0 || casesCompleted != 0){    
		   sheet.addRow([clientName,branch,cam,casesReceived,activeInsuf,wipCases,wipBeyondTat,wipWithinTat,casesCompleted,casesCompletedBeyondTat,casesCompletedWithinTat])    
	   }	   
...
let writeReport = function(subclientCursor){
    return new Promise((resolve,reject)=>{
      subclientCursor.forEach(async subclientItem=>{
        let casesCompletedBeyondTat = 0
        let casesCompletedWithinTat = 0
	let wipCases = 0      
	let activeInsuff = 0 
	let wipWithinTat = 0
	let wipBeyondTat = 0      
        if(subclientItem.completed_case_details.length > 0){
              for(let i=0; i < subclientItem.completed_case_details.length;i++){
              let item = subclientItem.completed_case_details[i] 	      
              let initiationDate = item.initiationDate
              let completionDate = item.outputqcCompletionDate
              let tatEndDate = item.tatEndDate
              let actualDaysTaken = moment(completionDate).diff(initiationDate,'days')
              ...
              let calculatedTatEndDate = moment(item.tatEndDate).add((holidays+weekends+insuffDays),'days')
              if(completionDate > calculatedTatEndDate.toDate()){
                 casesCompletedBeyondTat = casesCompletedBeyondTat + 1
              }else{
                 casesCompletedWithinTat = casesCompletedWithinTat + 1
              }
	...
	if(subclientItem.in_progress_case_details.length > 0){
           for(let i=0; i < subclientItem.in_progress_case_details.length;i++){
	      let item = subclientItem.in_progress_case_details[i]	   
        ...
        sheet.addRow([subclientItem.client_details[0].name,subclientItem.name,subclientItem.branch_details[0].name,subclientItem.cam_details.length > 0 ? subclientItem.cam_details[0].name:"",subclientItem.initiated_case_details.length > 0 ? subclientItem.initiated_case_details.length : 0,activeInsuff,wipCases,wipBeyondTat,wipWithinTat,subclientItem.completed_case_details.length > 0 ? subclientItem.completed_case_details.length : 0,casesCompletedBeyondTat,casesCompletedWithinTat])
...
//  const url = 'mongodb://3.6.175.176:27017';
  const uri = "mongodb://3.6.175.176:27017/"
  const mongoClient = new MongoClient(uri);
  const dbName = 'cvws-new';
startReport()
async function startReport(){
	...
	},
	{
	   '$lookup':{
	     from:'cases',
	     "let":{subclient_id:"$_id",fromDate:new Date(req.query.fromDate+"T00:00:00.000Z"),toDate:new Date(req.query.toDate+"T23:59:59.999Z")},	   
	     "pipeline":[
		{
		  $match:{
		     $expr:{
			$and:[
			{$eq:["$subclient","$$subclient_id"]},
			{$gte:["$initiationDate","$$fromDate"]},
		        {$lte:["$initiationDate","$$toDate"]}		
			]
		     } 	     
		  }	
		}     
	     ],
	     as:"initiated_case_details"	   
	   }	
	},
        {
           '$lookup':{
             from:'cases',
             "let":{subclient_id:"$_id",fromDate:new Date(req.query.fromDate+"T00:00:00.000Z"),toDate:new Date(req.query.toDate+"T23:59:59.999Z"),caseStatus:"OUTPUTQC-ACCEPTED"},
             "pipeline":[
                {
                  $match:{
                     $expr:{
                        $and:[
			{$eq:["$status","$$caseStatus"]},	
                        {$eq:["$subclient","$$subclient_id"]},
                        {$gte:["$outputqcCompletionDate","$$fromDate"]},
                        {$lte:["$outputqcCompletionDate","$$toDate"]}
                        ]
                     }
                  }
                }
             ],
             as:"completed_case_details"
           }
        },
        {
           '$lookup':{
             from:'cases',
             "let":{subclient_id:"$_id",caseStatus:"OUTPUTQC-ACCEPTED"},
             "pipeline":[
                {
                  $match:{
                     $expr:{
                        $and:[
                        {$ne:["$status","$$caseStatus"]},
                        {$eq:["$subclient","$$subclient_id"]}
                        ]
                     }
                  }
...
//       const db = mongoClient.db(dbName);
       const subclientCursor = await mongoClient.db(dbName).aggregate(pipeline);
       console.log("Got Subclient Curosor ",subclientCursor.length)	
       const done = await writeReport(subclientCursor)	
       console.log("Wrote the report")	
