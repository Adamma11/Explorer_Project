...
exports.getOperationStatsForAnalyst = (req,res)=>{
    ...
    if(req.query.reportType=='INITIATED'){
      query = {inputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date},verificationAllocatedTo:req.user.user_id}
    }
...
//    let query = {$and:[{status:{$ne:"MENTOR-REVIEW-ACCEPTED"},status:{$ne:"OUTPUTQC-ACCEPTED"}}]}
    if(req.query.reportType == 'COMPLETED'){
      query = {$or:[{status:'MENTOR-REVIEW-ACCEPTED',mentorReviewCompletionDate:{$gte:year + "-" + monthString + "-" + date}},{status:'OUTPUTQC-ACCEPTED',outputqcCompletionDate:{$gte:year + "-" + monthString + "-" + date}}],verificationAllocatedTo:req.user.user_id} 	    
    }
