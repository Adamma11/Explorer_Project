...
//	colorMasterDetails = data      
 ...
 let getEmploymentDetails = function(acase){
   ...
   let getEducationAdvancedDetails = function(acase){
    ...
    //new 10feb23
    let getCVAnalysisDetails = function(acase){
        return new Promise((resolve,reject)=>{
            cvanalysis
            .find({case:acase})
	    .lean()	
		  ...
		  let item = data[i]     
                  let det = ({
                     checkName:"CV Analysis",
                     checkType:"",
                     briefDetails:item.candidatename,
                     grade:item.grade
                  })
                  execSumDet.push(det)
                }
   ...
   let getAddressBusinessDetails = function(acase){
    return new Promise((resolve,reject)=>{
        addressbusiness
        .find({case:acase})
    .lean()	
      ...
      let item = data[i]     
              let det = ({
                 checkName:"Address Business",
                 checkType:item.typeofaddress,
                 briefDetails:item.address,
                 grade:item.grade
              })
      console.log("Sending for creating the jpgs of addressbusiness")     
              execSumDet.push(det)
            }
        ...
        .catch(err=>{
            console.log("Error in addressbusiness component ",err)
            reject()
        })
    })
...
// new-entery 
    ...
    let getEPFOCheckDetails = function(acase){
        ...
        let getvddCheckDetails = function(acase){
            console.log("in vddpv Check id details")    
                return new Promise((resolve,reject)=>{
                    vddpv 
                    .find({case:acase})
                .lean()	
                    ...
                    .populate({path:'personalDetailsData'})
                    .then(async data=>{
                   console.log("vddpv  id data is ",data)	    
                       for(let i=0;i < data.length;i++){
                  let item = data[i]     
                          let det = ({
                             checkName:"VDD PV Details",
                             checkType:"",
                   ...
                   resolve(data)	    
                    ...
                    .catch(err=>{
                console.log("Error in reading vddpv id details ",err)    
                        reject()
                    })
                })
        
        }
        let getvdddeclarationCheckDetails = function(acase){
            console.log("in vdddeclaration Check id details")    
                return new Promise((resolve,reject)=>{
                    vdddeclaration 
                    .find({case:acase})
                .lean()	
                    ...
                    .populate({path:'personalDetailsData'})
                    .then(async data=>{
                   console.log("vdddeclaration  id data is ",data)	    
                       for(let i=0;i < data.length;i++){
                  let item = data[i]     
                          let det = ({
                             checkName:"vdddeclaration Details",
                             checkType:"",
                             briefDetails:item.companyname,
                             grade:item.grade
                          })
                   ...
                   resolve(data)	    
                    ...
                    .catch(err=>{
                console.log("Error in reading vdddeclaration id details ",err)    
                        reject()
                    })
                })
        
        }
        let getcaconfirmationCheckDetails = function(acase){
            console.log("in EPFO Check id details")    
                return new Promise((resolve,reject)=>{
                    caconfirmation 
                    .find({case:acase})
                .lean()	
                    ...
                    .populate({path:'personalDetailsData'})
                    .then(async data=>{
                   console.log("caconfirmation  id data is ",data)	    
                       for(let i=0;i < data.length;i++){
                  let item = data[i]     
                          let det = ({
                             checkName:"caconfirmation Details",
                             checkType:"",
                             briefDetails:item.epicnumber,
                             grade:item.grade
                          })
                   ...
                   resolve(data)	    
                    ...
                    .catch(err=>{
                console.log("Error in reading caconfirmation id details ",err)    
                        reject()
                    })
                })
        
        }
        let gettcsvddCheckDetails = function(acase){
            console.log("in tcsvdd Check id details")    
                return new Promise((resolve,reject)=>{
                    tcsvdd 
                    .find({case:acase})
                .lean()	
                    ...
                    .populate({path:'personalDetailsData'})
                    .then(async data=>{
                   console.log("tcsvdd  id data is ",data)	    
                       for(let i=0;i < data.length;i++){
                  let item = data[i]     
                          let det = ({
                             checkName:"tcsvdd Details",
                             checkType:"",
                             briefDetails:item.epicnumber,
                             grade:item.grade
                          })
                   ...
                   resolve(data)	    
                    ...
                    .catch(err=>{
                console.log("Error in reading tcsvdd id details ",err)    
                        reject()
                    })
                })
    ...
    let writeWordDocument = function(addressDetails,addressBusinessDetails,addressComprehensiveDetails,addressOnlineDetails,addressTelephoneDetails,courtRecordDetails,criminalRecordDetails,educationDetails,educationAdvancedDetails,educationComprehensiveDetails,cvanalysisDetails,employmentDetails,empbasicDetails,empadvanceDetails,referenceRecordDetails,refBasicDetails,identityDetails,creditCheckDetails,credittransDetails,creditEquifaxDetails,globaldatabaseDetails,drugtestfiveDetails,drugtestsixDetails,drugtestsevenDetails,drugtesteightDetails,drugtestnineDetails,drugtesttenDetails,dlCheckDetails,directorshipCheckDetails,voterIdDetails,vddAdvanceDetails,bankStmtDetails,siteCheckDetails,psychometricDetails,socialMediaDetails,facisl3Details,ofacDetails,gapvfnDetails,passportDetails,colorBlindnessDetails,exitInterviewDetails,uanDetails,EPFOCheckDetails,vddCheckDetails,vdddeclarationCheckDetails,caconfirmationCheckDetails,tcsvddCheckDetails,twentysixasDetails,colorMasterDetails,camEmailId,caseDetails,personalDetailsFromDb,execSumDet,profileDetails,packageDetails){
        return new Promise((resolve,reject)=>{
           let worker = new Worker('./controllers/reports/write_std_word_report.js',{workerData:{addressDetails:addressDetails,addressBusinessDetails:addressBusinessDetails,addressComprehensiveDetails:addressComprehensiveDetails,addressOnlineDetails:addressOnlineDetails,addressTelephoneDetails:addressTelephoneDetails,courtRecordDetails:courtRecordDetails,criminalRecordDetails:criminalRecordDetails,educationDetails:educationDetails,educationAdvancedDetails:educationAdvancedDetails,educationComprehensiveDetails:educationComprehensiveDetails,cvanalysisDetails:cvanalysisDetails,employmentDetails:employmentDetails,empbasicDetails:empbasicDetails,empadvanceDetails:empadvanceDetails,referenceRecordDetails:referenceRecordDetails,refBasicDetails:refBasicDetails,identityDetails:identityDetails,creditCheckDetails:creditCheckDetails,credittransDetails:credittransDetails,creditEquifaxDetails:creditEquifaxDetails,globaldatabaseDetails:globaldatabaseDetails,drugtestfiveDetails:drugtestfiveDetails,drugtestsixDetails:drugtestsixDetails,drugtestsevenDetails:drugtestsevenDetails,drugtesteightDetails:drugtesteightDetails,drugtestnineDetails:drugtestnineDetails,drugtesttenDetails:drugtesttenDetails,dlCheckDetails:dlCheckDetails,directorshipCheckDetails:directorshipCheckDetails,voterIdDetails:voterIdDetails,vddAdvanceDetails:vddAdvanceDetails,bankStmtDetails:bankStmtDetails,siteCheckDetails:siteCheckDetails,psychometricDetails:psychometricDetails,socialMediaDetails:socialMediaDetails,facisl3Details:facisl3Details,ofacDetails:ofacDetails,gapvfnDetails:gapvfnDetails,passportDetails:passportDetails,colorBlindnessDetails:colorBlindnessDetails,exitInterviewDetails:exitInterviewDetails,uanDetails:uanDetails,EPFOCheckDetails:EPFOCheckDetails,vddCheckDetails:vddCheckDetails,vdddeclarationCheckDetails:vdddeclarationCheckDetails,caconfirmationCheckDetails:caconfirmationCheckDetails,tcsvddCheckDetails:tcsvddCheckDetails,twentysixasDetails:twentysixasDetails,colorMasterDetails:colorMasterDetails,camEmailId:camEmailId,caseDetails:caseDetails,personalDetailsFromDb:personalDetailsFromDb,execSumDet:execSumDet,profileName:profileDetails,packageName:packageDetails}})
	...
	let colorMasterDetails = await getColorMaster()    
        ...
        let addressBusinessDetails = await getAddressBusinessDetails(caseDetails._id)
        ...
        let cvanalysisDetails = await getCVAnalysisDetails(caseDetails._id)
        ...
        let  vddCheckDetails                = await getvddCheckDetails(caseDetails._id)
        let  vdddeclarationCheckDetails    = await getvdddeclarationCheckDetails(caseDetails._id)
        let  caconfirmationCheckDetails    = await getcaconfirmationCheckDetails(caseDetails._id)
        let  tcsvddCheckDetails  = await gettcsvddCheckDetails(caseDetails._id)
        let twentysixasDetails = await gettwentysixasDetails(caseDetails._id)
        let result = await writeWordDocument(addressDetails,addressBusinessDetails,addressComprehensiveDetails,addressOnlineDetails,addressTelephoneDetails,courtRecordDetails,criminalRecordDetails,educationDetails,educationAdvancedDetails,educationComprehensiveDetails,cvanalysisDetails,employmentDetails,empbasicDetails,empadvanceDetails,referenceRecordDetails,refBasicDetails,identityDetails,creditCheckDetails,credittransDetails,creditEquifaxDetails,globaldatabaseDetails,drugtestfiveDetails,drugtestsixDetails,drugtestsevenDetails,drugtesteightDetails,drugtestnineDetails,drugtesttenDetails,dlCheckDetails,directorshipCheckDetails,voterIdDetails,vddAdvanceDetails,bankStmtDetails,siteCheckDetails,psychometricDetails,socialMediaDetails,facisl3Details,ofacDetails,gapvfnDetails,passportDetails,colorBlindnessDetails,exitInterviewDetails,uanDetails,EPFOCheckDetails,vddCheckDetails,vdddeclarationCheckDetails,caconfirmationCheckDetails,tcsvddCheckDetails,twentysixasDetails,colorMasterDetails,camEmailId,caseDetails,personalDetailsFromDb,execSumDet,getProfileDetails(caseDetails.profile),getPackageDetails(caseDetails.package));
