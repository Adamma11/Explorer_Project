...
//	   let cam = item.cam.name   
	   ...
	   if(casesReceived != 0 || activeInsuf != 0 || wipCases != 0 || casesCompleted != 0){    
		let detailRow2 =  ({"clientName":clientName,"subclientName":subclientName,"branch":branch,"cam":cam,"casesReceived":casesReceived,"activeInsuff":activeInsuf,"wipCases":wipCases,"wipBeyondTat":wipBeyondTat,"wipWithinTat":wipWithinTat,"casesCompleted":casesCompleted,"casesCompletedBeyondTat":casesCompletedBeyondTat,"casesCompletedWithinTat":casesCompletedWithinTat})
	        dataArray.push(detailRow2)	   
//		detailRow2.commit()   
	   }	   
...
let getCasesInitiated = function(subclient_id){
   return new Promise((resolve,reject)=>{
     Case
     .count({initiationDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"},subclient:subclient_id})	   
     .then(data=>{
...
let getCasesCompletedDetails = function(subclient_id){
   return new Promise((resolve,reject)=>{
     Case
     .find({$or:[{status:"OUTPUTQC-ACCEPTED",subclient:subclient_id,outputqcCompletionDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"}},{status:"OUTPUTQC-ACCEPTED",subclient:subclient_id,outputqcCompletionDate:null}]})
     .lean()	   
...
let getCaseDetailsAndWriteReport = function(subclient_id){
    return new Promise((resolve,reject)=>{
    Case
    .find({initiationDate:{$gte:req.query.fromDate,$lte:req.query.toDate+"T23:59:59.999Z"},subclient:subclient_id})
    .lean()	    
...
//	   casesReceived = casesReceived + 1	
	   ...
	   if(item.subclient.client._id.toString() != clientId.toString() && item.subclient._id.toString() != subclientId.toString()){
           let detailRow1 =   ({"clientName":clientName,"subclientName":subclientName,"branch":branch,"cam":cam,"casesReceived":casesReceived,"activeInsuff":activeInsuff,"wipCases":wipCases,"wipBeyondTat":wipBeyondTat,"wipWithinTat":wipWithinTat,"casesCompleted":casesCompleted,"casesCompletedBeyondTat":casesCompletedBeyondTat,"casesCompletedWtihinTat":casesCompletedWithinTat})
              dataArray.push(detailRow1)
//	      detailRow1.commit()	  
	      dataArray.push();	   
              clientId = item.subclient.client._id
	      subclientId = item.subclient._id	  
