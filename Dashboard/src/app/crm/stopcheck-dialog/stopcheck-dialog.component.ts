...
@Component({
  selector: 'app-stopcheck-dialog',
  templateUrl: './stopcheck-dialog.component.html',
  styleUrls: ['./stopcheck-dialog.component.scss']
})
export class StopcheckDialogComponent {
  pows:string[]=[];
  displayedPowName!: string;
  powBlob!: Blob;
  showPowDialog:boolean=false;
  insufficiencyClearanceRejectionComments!: string;
  stopCheckForm: FormGroup;
  ...
  constructor(
    ...
    this.stopCheckForm = this.fb.group({
      caseId: [''],
      candidateName: [''],
  ...
  ngOnInit(): void {
    this.stopCheckForm.get('caseId')!.setValue(this.dataFromParent.caseId);
    this.stopCheckForm.get('candidateName')!.setValue(this.dataFromParent.candidateName);
    this.stopCheckForm.get('componentDisplayName')!.setValue(this.dataFromParent.componentDisplayName);
    this.readPows()
  }
  ...
  stopCheckButtonClicked(){
    console.log("Stop check",this.stopCheckForm.value);
    this.dialogRef.close(this.stopCheckForm.value)
  }
